...
| area | files |
|---|---|
| the chart | `MarketPriceChart.tsx` (+381), `MarketPriceChartOverlay.tsx`, `chart/geometry.ts` |
| constants | `src/server/config/limits.ts` (+75 — the staging window and its expiry alarm) |
| tests | `chart-window.test.ts`, `alignment-chain.test.tsx`, `label-anchor.test.tsx` (new, +398), `terminal-markers.test.tsx`, `y-scale.test.tsx` |
| docs | `docs/design/design-canon.md`, `docs/specs/SPEC.1.md`, `docs/plans/CHART-6.md` |
| instrument | `scripts/chart-6-contact-sheet.tsx` (new, +525) |

**CI:** run `33509278417` on `f50c6345`, conclusion **success**.
...
`@code-reviewer` finished and are reviewed by nobody (doctrine F-11). **The one to
read closely is `LABEL_FLIP_RESERVE_PCT = 14`** — the only behaviour change in the
list, made in response to a finding this run then declined on its arithmetic.
