...
- `docs/specs/SPEC.1.md` — the rider: §0 `1.0.23 → 1.0.24` + last-updated `2026-07-31`; §10.8 new paragraph; §20 new row (newest-last).
...
- **The `(cpmm §10.2)` citation** above — operator to rule on the replacement cite. Note the *code* docblock in `format.ts` deliberately cites the accurate source (the charter line-5 property test) rather than inheriting the rider's off-by-a-section pointer; only the verbatim spec text carries `§10.2`.
- **The reviewer's §10.8 carve-out wording** (unratified choice 6) — apply the one-word fix or leave.
