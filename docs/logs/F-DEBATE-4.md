...
10. **Smaller, from the audit:** no explicit `Cache-Control: no-store` on the page, unlike both its siblings, which comment that a cache is a window in which just-removed content could keep serving (L-1); and `safeCaptureMessage`/`safeCaptureException` on the polled path are neither deduped nor sampled, so a deterministic failure emits `images × 4/min × tabs` Sentry events and can mask a real alert (L-3).
