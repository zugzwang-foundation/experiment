...
- **Deviations:** **one residual** — the ErrorState Reload button renders INERT v1 (an RSC cannot pass `onReload`; the client error-boundary file (`(public)/error.tsx`) is outside the slice file list — creating it would also blanket `/m/[slug]` at group level). Purely UX per the auditor (zero diagnostics leak — fail-closed positive); flagged for a named follow-up alongside OQ-1 C.
