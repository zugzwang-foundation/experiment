...
|---|---|---|
| **2m-1** | ⚠⚠ **NO POST ON STAGING CARRIES AN ATTACHMENT, SO THE A7 D-2 MEDIA CAP CANNOT BE OBSERVED — ONLY CONSTRUCTED.** Measured across all eight markets, both sides: **zero** images in any post card. The R-1 cascade's media figures in this round's report come from markup lifted verbatim out of `CommentImage.tsx`'s `fill` arm and mounted into the live card's real cell, against the real compiled stylesheet, with a synthetic 9:16 data URI. | The construction is honest about what it proves — every CLASS resolves for real and only the pixel source is invented — but it is not the same thing as observing the shipped component. It also means `2k-1` (no aspect-ratio reservation) and `2k-2` (the radius divergence) remain **unobservable on staging**, which is why neither has moved for three rounds. ⇒ The fix is data, not code: one post with an attachment on one staging market would make all three measurable at once. `2k-4` records why the local QA environment cannot supply it either (`signRead` mints no URL there). |
