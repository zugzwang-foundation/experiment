...
2. **SPEC.1 §0 — version + change log** (currently **v1.0.13**; re-verify at sweep). Bump + add a change-log row for **A1's §16.5** CSAM-compliance rider (swap-window-closure note), **B3's §7 F-BET-3** rider (oversell `insufficient_shares` pre-check + error row + acceptance rows + the `bet_receipts` durable-backing note, ADR-0031), and **B7a's §7 F-BET-1** C.length rider (lower bound on trimmed / upper on raw / stored ≡ moderated; AUDIT.1 A24 ruling — paired ADR-0015 Patch record, no new ADR). B1 and B2 did not touch SPEC.1.
