...
## SYNC-sweep — spec/doc reconciliation owed by AUDIT-FIX-A1 + AUDIT-FIX-B1 + AUDIT-FIX-B2 + AUDIT-FIX-B3 + AUDIT-FIX-B7-A26 (FOUR targets)

**Originating task:** AUDIT-FIX-A1 (PR #197, squash `4350406`) + AUDIT-FIX-B1 (PR #199, squash `72ce26c`) + AUDIT-FIX-B2 (PR #201, squash `7fc4c60`) + AUDIT-FIX-B3 (rider commit on `fix/audit-fix-b3`; PR# in `docs/logs/AUDIT-FIX-B3.md`) + AUDIT-FIX-B7-A26 (freeze accepted-window ruling, doc-only riders on `docs/b7-a26-freeze-window`) — all landed same-commit spec riders but none ran the §0 version/changelog bump nor the ADR-index/footer count reconciliation (per CLAUDE.md §7 these are a periodic SYNC sweep, not per-task). **FOUR distinct targets** — the sweep must not do the SPEC.2 §0 bump alone and miss the SPEC.1 bump + the ADR-index/footer reconciliation. State verified current 2026-07-05 (B3 extension same day); B7-A26 extension 2026-07-06.
