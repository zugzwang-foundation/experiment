...
1. **SPEC.2 §0 — version + change log** (currently **v1.0.15**). Bump + add §0.1 change-log row(s) covering:
   ...
   - **B7a** riders — SPEC.1 F-BET-1 C.length rider + ADR-0015 Patch record + SPEC.2 §11 transport line; §0 bumps owed (SPEC.1 **and** SPEC.2 this time — B7a touches both specs), no new ADR (ceiling stays 0031).
