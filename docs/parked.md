...
## SYNC-sweep — spec/doc reconciliation owed by AUDIT-FIX-A1 + AUDIT-FIX-B1 + AUDIT-FIX-B2 + AUDIT-FIX-B3 + AUDIT-FIX-B7-A26 + AUDIT-FIX-B7a + AUDIT-FIX-B8 (FOUR targets)

**Originating task:** AUDIT-FIX-A1 (PR #197, squash `4350406`) + AUDIT-FIX-B1 (PR #199, squash `72ce26c`) + AUDIT-FIX-B2 (PR #201, squash `7fc4c60`) + AUDIT-FIX-B3 (rider commit on `fix/audit-fix-b3`; PR# in `docs/logs/AUDIT-FIX-B3.md`) + AUDIT-FIX-B7-A26 (freeze accepted-window ruling, doc-only riders on `docs/b7-a26-freeze-window`) + AUDIT-FIX-B7a (A14 transport bounding + A24 whitespace comment semantics, code-paired riders on `fix/audit-fix-b7a`; PR# in `docs/logs/AUDIT-FIX-B7a.md`) + AUDIT-FIX-B8 (D1–D4 + §16.3⇄§17.6/A17 reconciliation, doc-only riders on `docs/audit-fix-b8`; PR# in `docs/logs/AUDIT-FIX-B8.md`) — all landed same-commit spec riders but none ran the §0 version/changelog bump nor the ADR-index/footer count reconciliation (per CLAUDE.md §7 these are a periodic SYNC sweep, not per-task). **FOUR distinct targets** — the sweep must not do the SPEC.2 §0 bump alone and miss the SPEC.1 bump + the ADR-index/footer reconciliation. State verified current 2026-07-05 (B3 extension same day); B7-A26 extension 2026-07-06; B7a extension 2026-07-06 (same day); B8 extension 2026-07-07 (same day).
...
1. **SPEC.2 §0 — version + change log** (currently **v1.0.15**). Bump + add §0.1 change-log row(s) covering:
   ...
   - **B7-A26** riders (doc-only ruling, NO new ADR — mirrors the R-14.3 close-lag plan-ruling precedent; ruling record `docs/plans/AUDIT-FIX-B7_A26-freeze-window-ruling.md`) — §20.2 (freeze accepted-window clause inserted before **Wire envelope.**) · §19.1 (source-of-truth boundary: freeze instant → **dataset-build point**; three legitimately-post-freeze row classes) · §19.2 (freeze-snapshot replica → build-point replica). **Target 1 only** — no SPEC.1 touch, no ADR, so no §22/footer delta from this task.
   - **B7a** riders — SPEC.1 F-BET-1 C.length rider + ADR-0015 Patch record + SPEC.2 §11 transport line; §0 bumps owed (SPEC.1 **and** SPEC.2 this time — B7a touches both specs), no new ADR (ceiling stays 0031). **Also owed (pre-existing drift surfaced at B7a):** F-BET-1's **Errors** row omits `comment_requires_bet` (the code has thrown it since DEBATE.1 — predates B7a; reconcile the enumeration at sweep).
   - **B8** riders (2026-07-07; doc-only, D1–D4 + item-5 reconciliation) — §5.1 comments-row D2 build-deferred annotation · Appendix B.6 `market_media_id` PENDING-BUILD note · Appendix B.15 +2 rows (`content_type` / `byte_size` SHIP) · §4.3 catalogue nine→eleven (+ `close-due-markets` cron row + `_smoke-error` row + manifest PENDING-BUILD) + §4-closer two-cron fix · §17.6 `request_id` forward-obligation paragraph. **cpmm.md 2.0.0 bumped in-PR with its own §15 row — no cpmm sweep debt.**
...
2. **SPEC.1 §0 — version + change log** (currently **v1.0.13**; re-verify at sweep). Bump + add a change-log row for **A1's §16.5** CSAM-compliance rider (swap-window-closure note), **B3's §7 F-BET-3** rider (oversell `insufficient_shares` pre-check + error row + acceptance rows + the `bet_receipts` durable-backing note, ADR-0031), **B7a's §7 F-BET-1** C.length rider (lower bound on trimmed / upper on raw / stored ≡ moderated; AUDIT.1 A24 ruling — paired ADR-0015 Patch record, no new ADR), and **B8's §16.3 H3** admin-null clarifier (admin-surface rows log `user_id` = JSON `null`; AUDIT.1 A17/B8 ruling, 2026-07-07). B1 and B2 did not touch SPEC.1.
