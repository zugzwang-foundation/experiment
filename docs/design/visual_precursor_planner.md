# Zugzwang — VISUAL pre-cursor planner (v1)

> **Doc:** `docs/design/visual_precursor_planner.md`
> **Status:** v1-draft · planning-thread output · the spine of the VISUAL bucket
...
>
> **The three docs this planner stands on** (read them; this planner does not duplicate them):
> - `design-language.md` — the **what**: tokens, primitives, locked visual constraints, state-shape rules, per-surface primitive map.
> - `design-workflow.md` — the **how we produce**: the per-slot loop, the pin-point CD prompt template, mockup handling, reliability workarounds, usage discipline, the CD-seed kit.
...
- **web Claude** — orchestrator: authors prompts, gates steps, sequences; runs no tools.
...
Each surface gets **its own web Claude chat**. **Discussion precedes all Claude Design action** — the prompt is fully planned before it is pasted (no exploring via half-formed prompts). The loop each slot follows is `design-workflow.md` §2 (read → sketch → discuss & lock layout → draft prompt → generate/iterate → checkpoint → sign off).
...
|---|---|---|---|---|
| **1** | **Discovery / market-list front page** | DESIGN.3 | UI.3 | **Look-anchor.** Originates the market card, category/tab vocabulary, featured/trending rail, app shell/nav. Establishes the high-fi monochrome look. |
...
1. **Slot 1 = Discovery.** Recommended as the look-anchor (most reusable primitives, not the most complex). *Alternative:* Landing first — weaker, since marketing originates few product primitives.
...
5. **Colour model locked:** black = Support/YES, white = Counter/NO (semantic, permanent); grey = default + future-colour placeholder; **one** accent applied in CC. *(Black↔side binding is a default pending DESIGN.1 confirmation.)*
...
8. **Four backbone docs added** to `docs/design/`: `design-language.md`, `design-workflow.md`, `design-handoff.md`, this planner. (Plus `Research_Report.md` — the CD best-practices research — in PK.)
...
The loop is `design-workflow.md` §2. Each slot chat opens with scope-framing (objective + exit criterion, the three roles, numbered kickoff-to-close steps, a NOT-doing list), reads the three backbone docs + this planner + the slot's tracker entry, then waits for the operator's whiteboard sketch. The Slot-1 kickoff is §7.

---

## §7 — Slot-1 kickoff (paste-ready)

> Paste as the **first message** of a new web Claude chat (in the Zugzwang project) to open Slot 1. Send your whiteboard sketch as the **second** message after it acknowledges. *(Finalised for Slot 1 = Discovery; if the Slot-1 fork resolves differently, this repoints.)*
...
ROLES
- You (web Claude): orchestrator — author the Claude Design prompt, gate
  each step, critique. You run no tools and touch no canvas.
...
2. design-workflow.md   (the per-slot loop §2, the CD prompt template §3)
...
- There is NO published Claude Design system yet — Slot 1 runs on default
  styling held to monochrome via the design-language doc + the constraints
  block in the prompt. The seed kit is built FROM this surface after sign-off.
...
D. You draft the pin-point Claude Design prompt (template: workflow §3;
   constraints block §3.2). I ratify it.
E. I paste it into Claude Design; we iterate via chat + batched comments.
...
*End planner v1-draft. Stands on `design-language.md`, `design-workflow.md`, `design-handoff.md`. Drift in §5 is the v12 sweep's input; the tracker is not edited in this thread.*
