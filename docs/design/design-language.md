...
**Reply (depth-1)**
- *Intent:* a Support or Counter bet on a post; itself a stake-backed argument. A Support reply may be **friendly fire** — the author backs the side and contests this argument (ADR-0058).
- *Rule:* flat (one level only); ordered by stake descending within its side; earlier-posted wins ties. A friendly-fire reply stays in its side's lane and sorts by the same ruler.
- *Slots:* side badge · position marker · **friendly-fire tag** (only when flagged; the same outlined marker primitive as Flipped / Exited, text `Friendly fire`; above 640px it carries the markers' hover gloss — *Friendly fire: this reply backs the side but contests the argument above.* — and below 640px none, per D-4 vi) · reply stake · argument text.
...
- *Rule:* **there is no vote control.** No up/down arrows, no `↑N ↓M`. The friendly-fire *vote* is removed entirely; the friendly-fire **toggle** (ADR-0058) is a compose-time switch on a Support reply-bet, not a vote, and **changes nothing on this aggregate** — friendly-fire Đ is inside the Support figure (same pool). Support/Counter are computed, never cast.

**Friendly-fire switch** *(composer; ADR-0058)*
- *Intent:* let a same-side replier contest the argument without leaving the side.
- *Form:* a labelled switch, `Friendly fire`, in the reply composer's **header row** — beside the `Support <author>'s argument` statement, at the row's right end before the close control — default **off**; beneath the header row, one line of helper copy: *Contest this argument without leaving your side. Your stake still backs YES* (or *NO* — the side being bought); above 640px the label carries the markers' hover gloss: *Friendly fire: a Support reply that backs the side but contests this argument. The bet itself is unchanged — your stake still backs YES.* Same type ramp and outline weight as the pills; monochrome; the on-state uses the side pole's fill. *(Placement ratified from screenshots 2026-09-22; the earlier "beneath the Support pill" wording described the lane header, not the composer.)*
...
- **Market card** — a market at a glance. **Locked composition (Slot 1):** image thumb + question · YES/NO split bar · `Đ volume · posts · replies`. *(Category chip, lifecycle marker, and close date are parked off the Discovery card — still in the vocabulary for other renders.)* Reused across Discovery and Profile; **must be identical everywhere.** ⚠ *Amended at HTML-FINISH · DISCOVERY (2026-08-15): the **two-line sparkline** is struck from this composition — the ratified Discovery mockup's `.mcard` is title-block + price bar with nothing between (`surface_discovery_v1_0.html:269-282`), and its `.spark` rule at `:155` styles no element in the document. The card sparkline was decorative (SPEC.1 §22), and composition of decoration is the mockup's jurisdiction. **The "identical everywhere" clause is deliberately retained and is the load-bearing half of this amendment**: the sparkline leaves BOTH renders together, so Discovery and Profile stay one composition rather than being freed to diverge. Paired SPEC.1 amendment at 1.0.30, same ruling.*
...
- Two-line price graph (originated Slot 1) — YES and NO as two always-complementary lines (mirrors crossing at 50%). The pole that matches the ground cannot render on it, so one line is always a grey stand-in — series-bound via tokens, never restyled ad hoc. Current (dark ground, BRIDGE): YES = `--graph-yes` `#737373` (grey stands in for the black pole) · NO = `--graph-no` `#fafafa`. Retired light-era mirror: YES = solid ink, NO = grey standing in for white. Full-size on market panels (the Discovery hero and the market-detail chart slot). ⚠ *"sparkline on cards" struck at HTML-FINISH · DISCOVERY (2026-08-15) — the market card carries no price graph; see the Market card entry above. The colour binding stated here is unchanged and still governs every render that remains.*
...
> **[PK lineage] v0.9-draft (2026-09-22):** **Friendly-fire toggle (ADR-0058, D-51)** — §3.1 Reply gains the friendly-fire tag slot; §3.1 aggregate Rule carries the vote-vs-toggle rider; two new primitives, **Friendly-fire switch** (composer) and **Friendly-fire meter** (post-focus Support lane header); §6 bullet. Card untouched by ruling. Same day, after screenshot ratification: switch placement corrected to the composer header row; hover glosses on the switch label and the tag above 640px.
...
*End design-language v0.8-draft. Remaining blank values designed in Claude Design; derived into `design.md` at DESIGN.SPEC. Living vocabulary — §3 grows additively; §1 and §4 change only by explicit decision.*
