...
>
> ⛔ **SCOPE OF THIS CORRECTION, STATED SO ITS LIMIT IS NOT MISREAD.** The founder ruling scopes it to **§2 and §5's Profile entries only**. **§6 (`replica reply footer …`) and §7 item 5 (`replica D5-synced …`) still say "replica" and are DELIBERATELY LEFT** — they are routed to the founder, not overlooked. This is the exact residue O-5 warns about (a correction not applied at every site that states the superseded position), recorded here so the next reader meets it as a known open item rather than as a contradiction.
