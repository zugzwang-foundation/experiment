...
3. **Right gutter.** The gutter is a **CSS-space element beside the plot**, sized to the widest label plus the gap — not a region of the viewBox. The viewBox reserves only a **dot allowance**: the terminal circle's radius plus a hair, so the dot cannot half-clip at the right edge. Labels are clamped inside the plot's vertical extent and never clip. **The plot's aspect and every container that sizes it stay locked to the viewBox per `C-CHART-1` clause 4**, and that lock is asserted mechanically, not restated. ⚠ **Amended at CHART-2 (2026-08-27).** The clause previously took the gutter *out of the viewBox*, which rendered every `/m/[slug]` plot ≈5.6 % narrower in the same CSS box — recorded at CHART-1's Gate C as a ratified trade and **dissolved here rather than accepted** — and, unnoticed at that Gate C, broke the expanded overlay's container match. It also retires the label advance CHART-1 pinned by hand against a fallback font, because `next/font` could not be fetched offline at measurement time; HTML text is measured by the browser in the real face.
