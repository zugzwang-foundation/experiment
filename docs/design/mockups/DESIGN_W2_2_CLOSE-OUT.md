...
- **Card 1 image = the live PFP avatar** (a **circle** carrying `--avatar-ring`, matching `.navav`/`.idav` **as built**), shown large as a centered hero — no second decorative illustration. Cards 2–6 image = a bundled static illustration.
  - ⚠ **PFP-UI-1, 2026-08-26 — this said "rounded-square at `--imgr`", and the shape is superseded.** `--imgr` was ratified for avatars while the avatar was an abstract empty square nobody had seen (values-log v0_3 `:44`, O6 — *"Empty square in CD is the correct current state"*). The art delivered on 2026-08-25 is a **disc with padding around it**, so a square frame renders a median **24.83%** of the tile as cream margin. Matching the mount to the shape the asset already has cuts that to 6.04%. All eight avatar mounts are now circles.
  - ⚠ **The `.navav`/`.idav` cross-reference still holds, but read it as a claim about the BUILD, not the mockups.** Those mockup classes are locked evidence of what was drawn and are deliberately not amended — six of them still specify `--imgr`. The mounts they describe are circles in the build.
