...
- **2026-07-16 — Fable-5 window re-opened at UI.0.** Session and all four subagents repin `model: claude-fable-5` (PR #228); `effort: max` unchanged. The window carried a standing post-window revert obligation back to `claude-opus-4-8`, and no gate flexed for it. *(Backfilled 2026-07-31 at MODEL-REPIN from the CLAUDE.md §6 pin history + `docs/logs/UI-0.md`; the event was never logged here at the time.)*
