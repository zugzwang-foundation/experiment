...
5. `router.refresh()` re-invokes the **existing** `/m/[slug]` server read path — the route's layout and page both re-execute, `loadDebateView` re-runs its masking gate, `loadViewerMarketContext` re-runs beside it, and React reconciles the new payload into the mounted client tree. This is **not** a fetch against a Route Handler: SPEC.2 §4.3's Route Handler catalogue is closed and this flow adds no entry to it (Invariants, below).
...
*(Amended at CHART-1, SPEC.1 1.0.40, 2026-08-27: the `debate-view::poll-refreshes-price-chart` row is **removed from this table because §17 no longer carries it** — it asserted the behaviour that amendment reverses, and the header sentence above requires every name here to appear verbatim in §17. The poll still re-executes the page's whole server read, so the chart's **terminal point** is recomposed every tick from the live pool price; what it no longer does is re-derive the chart's **history**, floored to `MARKET_SERIES_MIN_WINDOW_MS`. The replacement rows are F-DEBATE-5's, not this flow's, so they are recorded there rather than swapped in here. The client-render case that exercised it is retained under the interval row, which is the behaviour it actually proved.)*

File placement follows the behaviour under test, per SPEC.1 1.0.25 §9: the interval and suspension rules are client-render behaviours (jsdom + `@testing-library/react`, fake timers; visibility proven by stubbing `document.hidden` and dispatching `visibilitychange`, never in a browser). The masking and stop rules are read-path/structural source-grep guards. The stop rule's *client effect* — the interval cleared and never restarted — is additionally exercised in the client-render file as a supporting case under its own name.
