...
| **Patch records consumed** | ADR-0017 P1 (friendly-fire removed entirely), P2 (latest interleave), P3 (filter modes retired from the v1 surface; lane-dominance badges) · **ADR-0039 R4/R5/R6** (every ranking input keys off surviving lot basis; the figure follows the ruler) · **ADR-0039 P2** (self-authored replies do not count as attraction) |
...
|---|---|---|
| `support_count` | number of **other people's** reply-bets on the post's **own** side | reply-bets joined to the post where reply side = post side, **excluding replies by the post's own author** |
| `counter_count` | number of **other people's** reply-bets on the **opposing** side | reply-bets joined to the post where reply side ≠ post side, **excluding replies by the post's own author** |
...
⚠ **Multi-account capture remains OPEN, and it is CHEAPER than the ruling's own sentence implies.** Measured at RANK-2 against the shipped model and auth stack: **one** extra account, not six — `floorLane.n = 5` with `clears()` at `>=`, and no uniqueness on `(user, parent_comment_id)`, so a single sybil's **five** replies clear traction. Google OAuth signup carries **no Turnstile**; the per-IP OTP cap allows ~600 signups/hour and fails open; the identity pool holds 50,000 tuples; and `INITIAL_USER_DHARMA` self-funds the replies, which the fee-less CPMM returns on exit — **net cost ≈ 3 attoDharma, and the attacker keeps the money.** ⇒ #1 outright plus *Most Debated* plus the Discovery hero, permanently, since neither removal nor a ban decrements `n`. It binds only where a post already holds **≥ Đ200 of attracted basis** (the `D` lane then outranks any finite traction ratio) — i.e. **not** in quiet markets or a market's first days. Recorded as an accepted risk **at the corrected price**; raising it (`COUNT(DISTINCT rc.user_id)`, or a per-parent reply cap) is a ranking-semantics ruling, not a patch.

### 2.1 Read-time join (how the substrate is computed)

The four aggregates derive **entirely from existing columns** — `lots.surviving_basis` over `bets.stake`, `bets.side`, `comments.side_at_post_time`, `comments.parent_comment_id`, and **`comments.user_id`** (the RANK-2 self-exclusion: `AND rc.user_id <> p.user_id` on the reply join). The lot reach is `lots.bet_id = bets.id`, a **1:1 unique-index lookup** (`lots_bet_id_uq`), so it can never fan an aggregate out. There is **no frozen `stake_at_post_time` column** (the superseded ADR-0009 model used one; this model does not need it — value is aggregated from reply-bets at read time).
...
- **Per-side grouping** (the four signals) groups a post's reply-comments by `(parent_comment_id, side_at_post_time)` — served by the index **`comments_ranking_idx (parent_comment_id, side_at_post_time)`**. ⚠ Since RANK-2 the join also tests `rc.user_id <> p.user_id`, and **`comments_ranking_idx` does not cover `user_id`**, so that predicate costs a heap fetch per reply row. Measured at RANK-2 on the eight-market Discovery shape: **≈ +0.5 ms per render (~+5%)**, no additional round trips. Named here rather than left to be rediscovered, given PERF-1's history; widening the index is a tuning-pass call, not a correctness one.
