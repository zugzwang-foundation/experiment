...
| | |
|---|---|
| **Status** | v1.1.1-draft (shape locked; numeric constants pin at the 2026-09-01 number-tuning pass) |
...
**THE COUNTS ARE TWO NUMBERS AND THEY ARE NOT INTERCHANGEABLE** (ADR-0039 patch record P3, RANK-3). The `_total` pair is what a surface shows; the other pair is what the lanes rank on and **is never rendered anywhere, on any surface, in any DTO that reaches a client.** Their difference is the self-reply count, and on a post carrying a removed reply that arithmetic attributes the removal to a named pseudonym — which is why the split exists rather than one number being reused.

⚠ This does **not** contradict RANK-1's one-field-two-consumers rule (§7.3). There the badge and the ruler measured the **same** thing, so splitting them would let one lie. Here they measure **different** things — a display fact and a ranking judgement. *What must never split is a number and its own meaning.*

**AND THE RANKING COUNTS PEOPLE, NOT REPLIES** (R-2). `n` is the thesis's `n` in `K · n > C`: **how many people hold the knowledge.** A lane that counted messages measured the wrong noun — one person posting five times is `n = 1`. This is a correction to a definition, not a mitigation, and it changes what the badge means: see §5.2.
...
**Why the whole model moved at once.** The frozen reading was buyable and refundable: the CPMM is fee-less, so a large bet placed to seize a slot could be exited for dust while the slot it bought stayed seized forever. Closing one ruler and leaving another open would only move the same capture one lane over — so all three moved together. **If the money can leave, the rank it bought leaves with it.** (That is the STAKE axis. The COUNT axis is a separate question with a separate answer — immediately below.)

⚠ **THE COUNT INPUTS ARE A SEPARATE AXIS, AND THEY DO NOT DECAY — BY DESIGN.** Three ranking inputs are **counts**, not stakes — traction `n`, the dominance split `lop`, and the contestation badge lane `n^b` — and `support_count` / `counter_count` are counts rather than sums, with **no survival predicate** — and since RANK-3 they are `COUNT(DISTINCT rc.user_id)` over *other people's* replies. `bets` and `comments` are Bucket A, so a count can never move by any mechanism in the system, including moderation (removed items remain counted — SPEC.1 **§23**, *"Removed posts' attracted Dharma counts — the stakes were real"*).

**That was a live single-account capture surface until RANK-2, and it is now closed — but not by making counts decay.** With no self-reply guard and no per-post reply cap, one account could post at the post floor, reply to its **own** post enough times to clear `floor_lane(n)` and the `floor_split` gate, take the market's **#1 `topOrder` slot outright** via the sole-clearer sentinel (§3.3), and then sell everything back through the fee-less CPMM for dust. Measured before the fix: a **Đ10** post with six Đ50 self-replies outranked a **Đ100** post, and wore *Most Debated*.

**The ruling (ADR-0039 patch record P2): self-authored replies do not count as attraction.** A reply whose author is the parent post's author is excluded from the count lanes (`n`, `lop`, `n^b`) and from the attracted-value aggregates (`support_dharma`, `counter_dharma`) — **and from nothing else.** It keeps its own stake, its own lane position (§7), its own ranking weight as an argument, and its place in the author's Arguments count. *A post attracting its own author is not attracting anything, and attraction is what these inputs measure.*

**Why the counts were not simply made to decay**, which is the obvious fix and the wrong one: a decaying count asserts that an argument someone made and later exited never got made, and **R9 makes Sold permanent precisely because it did**. Decay is the right rule for *weight* and the wrong rule for *whether a thing happened*. So the attack's other leg was removed instead.

⚠ **Multi-account capture remains OPEN, but RANK-3 raised its floor from ONE account to THREE.** RANK-2 measured the price at **one** account — `floorLane.n = 5` with `clears()` at `>=`, and no uniqueness on `(user, parent_comment_id)`, so a single sybil's **five** replies cleared traction. **R-2 ends that**: traction is `COUNT(DISTINCT rc.user_id)`, so five replies from one account on one side are `n = 1` and clear nothing.

⚠ **Three, not five — and the difference is the flip path, not a hole.** `n` is `supportCount + counterCount`, and those are two **separate per-side DISTINCT aggregates**, so one `user_id` reaching both sides of a post is counted once in *each*. Reaching both sides is ratified behaviour: `place` rejects only a **held** opposite side (`getHeldPosition` is `quantity > 0`), so selling a lot to zero re-opens the other side, and §INV-3 then freezes each reply on the side it was posted from. **One account buys two people-units.** Measured — `tests/server/lots/count-differential.test.ts`, third case — three accounts × (reply YES → sell to zero → reply NO) give `n = 6`, clear the floor as the **sole** clearer, and take #1 outright; the sold YES halves leave `D = Đ150`, **below** `floorLane.D = 200`, so the capture is bought purely on the count axis. The *Contested* badge is cheaper again: **two** accounts give `n = 4`, `b = 1`, `n^b = 4 ≥ 3`.

⚠ **That is a floor, not a closure** — and the honest statement of what remains is that it is now **signup cost**, not ranking cost: Google OAuth signup carries **no Turnstile**; the per-IP OTP cap allows ~600 signups/hour and fails open; the identity pool holds 50,000 tuples; and `INITIAL_USER_DHARMA` self-funds the replies, which the fee-less CPMM returns on exit — **net cost ≈ slippage, and the attacker keeps the stake.** ⇒ #1 outright plus *Most Debated* plus the Discovery hero, permanently, since neither removal nor a ban decrements `n`. It binds only where a post already holds **≥ Đ200 of attracted basis** (the `D` lane then outranks any finite traction ratio) — i.e. **not** in quiet markets or a market's first days. Recorded as an accepted risk **at the corrected price**; raising it further — a per-`(user, parent_comment_id)` reply cap, or taking `COUNT(DISTINCT rc.user_id)` **once across both sides** and apportioning it — is a ranking-semantics ruling, not a patch, and is open as **RANK-3-D2**. ⚠ **The `COUNT(DISTINCT rc.user_id)` lever named here at RANK-2 WAS taken at RANK-3** — that is what moved the floor from one account to three.

### 2.1 Read-time join (how the substrate is computed)

The six per-side signals (two display counts, two ranking counts, two Dharma sums) derive **entirely from existing columns** — `lots.surviving_basis` over `bets.stake`, `bets.side`, `comments.side_at_post_time`, `comments.parent_comment_id`, and **`comments.user_id`** (the self-exclusion `AND rc.user_id <> p.user_id` — in the reply JOIN at RANK-2, in the aggregate FILTER clauses since RANK-3, so both the display totals and the ranking counts come from one query). The lot reach is `lots.bet_id = bets.id`, a **1:1 unique-index lookup** (`lots_bet_id_uq`), so it can never fan an aggregate out. There is **no frozen `stake_at_post_time` column** (the superseded ADR-0009 model used one; this model does not need it — value is aggregated from reply-bets at read time).
...
- **Per-side grouping** (the four signals) groups a post's reply-comments by `(parent_comment_id, side_at_post_time)` — served by the index **`comments_ranking_idx (parent_comment_id, side_at_post_time)`**. ⚠ Since RANK-3 the aggregate **FILTER** clauses test `rc.user_id <> p.user_id` (the predicate moved out of the JOIN so the display totals could be computed from the same query), and **`comments_ranking_idx` does not cover `user_id`** — so it costs a heap fetch per reply row. RANK-3 additionally makes the ranking counts `COUNT(DISTINCT rc.user_id)`, which is a sort or hash per group rather than a running increment. Named here rather than left to be rediscovered, given PERF-1's history; widening the index is a tuning-pass call, not a correctness one.
...
**The `COALESCE(lots.surviving_basis, bets.stake)` fallback is deliberate and stays.** Every bet mints its lot inside the same W-1 transaction (ADR-0039 R8), so a lot-less bet does not arise. But the fallback's job is what happens if one ever did: without it, an unknown surviving amount would score as **zero** — silently deleting a real contribution from the model — where with it, the row scores as *all of it survives*, which is exactly the pre-LOTS-1 behaviour. Reading "unknown" as "nothing" is the worse failure of the two.

⚠ **Not because it is the louder one — BOTH are silent.** A row scored zero is indistinguishable from a genuine full exit, and a row riding the fallback is indistinguishable from an untouched argument: `sold` reads false, the original equals the current, so there is no tag and no strikethrough. Nothing in the tree separates *"this stake is intact"* from *"we do not know and are assuming it is"*. The fallback wins on consequence, not on visibility — and the consequence of its own firing is that a lot-less row becomes the **only** class holding ranking weight regardless of exit, permanently, with no back-fill permitted (ADR-0039 R8). Unreachable through the write path; reachable through an ADR-0024 promote or rollback window. The owed control is a **detector**, and there isn't one.
...
> ⚠ **The `Top_score` this chain breaks ties in is computed from PEOPLE, not replies** (R-2, RANK-3). Two posts that would once have tied because each drew five messages may now differ, because one drew them from five people and the other from one.
...
> ⚠ **The `D` this lens orders on excludes self-authored replies, and the Support/Counter footer beside it does NOT** (RANK-3). The footer is a display count (`*_count_total`); the ordering signal is the attracted value. They are different quantities on purpose — see §2.
...
| Badge | Lane | Meaning |
|---|---|---|
| **Most Debated** | traction (`n`) | the most **people** arguing — ⚠ **not** the most replies typed. R-2 (RANK-3) redefined `n` as `COUNT(DISTINCT rc.user_id)`: a post drawing twenty replies from three people is less debated than one drawing six from six. |
...
| Read | Index | Status |
|---|---|---|
| Per-side grouping of a post's reply-comments (the `_total` display counts, the `COUNT(DISTINCT rc.user_id)` ranking counts, and the per-side partition of the Dharma sums) | **`comments_ranking_idx (parent_comment_id, side_at_post_time)`** — ⚠ does **not** cover `user_id`, which both the RANK-3 self-exclusion and the DISTINCT now need | Exists (initial schema). **Must survive** — PRECURSOR.4 lock. **This is the live index the ADR-0017 model needs; it is NOT dropped by DEBATE.8.** |
...
| v1.0.0-draft | 2026-08-22 | RANK-2 (ADR-0039 patch record P2; ruling web-authored) | **Self-authored replies do not count as attraction.** Closes the single-account count-lane capture RANK-1's security pass found: a Đ10 post with six Đ50 self-replies took `topOrder` #1 outright via the sole-clearer sentinel and wore *Most Debated*, then sold back for dust — because counts are `COUNT(...)` over Bucket-A rows and can never move. Amended §2 (the substrate table's four aggregate rows + the count-axis block, which previously recorded the capture as OPEN). **Counts were NOT made to decay** — that would assert an argument someone made and exited never got made, contradicting R9. One predicate (`AND rc.user_id <> p.user_id`) in the reply JOIN at four sites; `reply-substrate.ts` deliberately untouched, since a self-reply keeps its own lane position. Multi-account capture remains open by design. Regression test: `tests/server/lots/self-reply-capture.test.ts`. |
| v1.0.0-draft | 2026-08-22 | RANK-3 (ADR-0039 patch record P3; ruling web-authored) | **Two numbers, and the ranking one counts PEOPLE.** (R-1) The displayed reply count is self- **and removed-inclusive** and is the only count permitted to reach a client; the ranking count is never rendered. That closes an SC-1 differential RANK-2 created — the unfiltered lane minus the filtered aggregate WAS the self-reply count, printed side by side in the **public, signed-out** `.md` export, which attributed a removed reply to a named pseudonym. (R-2) Traction, the dominance split and contestation now use `COUNT(DISTINCT rc.user_id)`: `n` is how many PEOPLE hold the knowledge, so one person posting five times is `n = 1`. **"Most Debated" now means most people arguing, not most replies typed** (§5.2). The self-exclusion moved from the reply JOIN into the aggregate FILTERs so both numbers come from one query. Amended §2 (table + both notes), §3.4, §3.6, §5.2, §11, §14. ⚠ **Consequence:** the M2 staging-fixture badge calibration is stale — M2-P2 draws 5 replies from 3 people and M2-P4 4 from 3, so M2 fires ONE badge where the table claims three. ⚠ **Residual corrected against the ruling:** R-3 prices the new floor at five accounts; it is **three**, because `n` is the sum of two per-side DISTINCT aggregates and the ratified flip path (sell to zero → re-enter the other side) puts one `user_id` on both sides, buying two people-units per account. Measured, not reasoned; the *Contested* badge costs two. Open for ruling as **RANK-3-D2** — apportioning one cross-side DISTINCT would change what `lop`'s denominator means. ⚠ **Both count forms hardened past what is reachable today:** the display totals count reply COMMENTS (`COUNT(DISTINCT rc.id)`), not bet rows, so a second bet on a comment cannot make the total exceed the lane length; and both count clauses require `rb.id IS NOT NULL`, so a bet-less comment cannot buy traction the lane never shows. Regression tests: `tests/server/lots/count-differential.test.ts` (three cases) and `tests/unit/ranking/substrate-site-parity.test.ts`. |
