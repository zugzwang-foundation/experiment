...
- **Version:** 2.0.3 (semver; bump major on invariant changes)
...
| `SHARED_VIEW_MIN_WINDOW_MS` | Minimum interval between derivations of a market's **shared read block** — the debate view's comments, ranking, replies, totals and media, and Discovery's per-market totals, media, series and top posts (§9 *Refresh*; ADR-0051). Within the window a derivation is reused and shared by every reader. **Set to 15000** — a design pin, not a tuned economy value. ⛔ **It replaces a key, not a shorter window.** Both blocks were keyed on the pool `reserves`, and a `'use cache'` key is its argument list, so every bet changed the key and forced a full miss for every reader — the cache helped least exactly when load was highest, which is the failure this section names. ⚠ **Equal to `POLL_INTERVAL_MS_DEBATE_VIEW` but not derived from it**: two independent tunables that happen to agree, so retuning the poll cadence never silently retunes freshness for every reader of every market. ⚠ **Shorter than `MARKET_SERIES_MIN_WINDOW_MS`, deliberately** — that one floors a price *history*, which may honestly be a minute old with its live edge recomposed outside the window; this one floors *arguments*, which are the product. ⛔ **It is not what makes a poster see their own argument**: that is a bounded per-viewer bypass at the page (ADR-0051 D-3), so shortening this constant is not a way to improve that case and lengthening it does not harm it. Read from the constant at every call site, never inlined. |
