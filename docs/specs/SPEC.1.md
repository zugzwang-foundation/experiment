...
- **Version:** 1.0.31 (semver; bump major on invariant changes)
...
**Two modes (mirroring the §23 card→overlay pattern).** **Collapsed** — in-header, card-sized, lines only, no axis, no nodes, carrying an expand affordance; this is the default. **Expanded** — a dismissible overlay carrying the **time axis**, the same two lines, and **post nodes** (below).
...
| `debate-view::price-chart-collapsed-no-axis` | §9 F-DEBATE-5 | — |
