...
- **Version:** 2.1.0 (semver; bump major on invariant changes)
...
| **Friendly fire** | A **Support reply-bet whose author contests the parent argument while backing its side** — declared at compose time by a switch, stored as `comments.friendly_fire` (set on INSERT, never updated), and rejected on a Counter or a top-level post. The stake is an ordinary own-side buy; the single-side rule, floors and price impact are untouched. It is **not** the friendly-fire *vote* removed at ADR-0017 P1 (a free standalone up/down with its own table); it is a flag on a staked reply. Surfaced as a **tag** on the reply and a one-sided **meter** at the head of the post's Support lane (`friendly_fire_dharma ÷ support_dharma`, Dharma still held); never as a count, never on the post card, never as a badge. Read by the ranking's balance term only (§9). Ruled D-51; ADR-0058. | `comments.friendly_fire` · `friendly_fire_dharma`, `endorse_count`, `contest_count` (§9 / `RANKING.md` §2) |
