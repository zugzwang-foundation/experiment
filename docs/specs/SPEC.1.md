...
- **Version:** 2.2.0 (semver; bump major on invariant changes)
...
| **Support / Counter** | The two sides of a reply-bet relative to its parent post: **Support** = a reply-bet on the *same* side as the parent's frozen side; **Counter** = a reply-bet on the *opposite* side. Surfaced per post as read-time aggregate counts and Dharma totals (`support_count : Đ support_dharma` / `counter_count : Đ counter_dharma`). Not a vote — there is no standalone Support/Counter affordance; the counts aggregate over reply-bets. A Support reply-bet may additionally carry the **friendly-fire** flag (next row). | derived from `bets.side` + `comments.parent_comment_id` + parent `side_at_post_time` |
| **Friendly fire** | A **Support reply-bet whose author contests the parent argument while backing its side** — declared at compose time by a switch, stored as `comments.friendly_fire` (set on INSERT, never updated), and rejected on a Counter or a top-level post. The stake is an ordinary own-side buy; the single-side rule, floors and price impact are untouched. It is **not** the friendly-fire *vote* removed at ADR-0017 P1 (a free standalone up/down with its own table); it is a flag on a staked reply. Surfaced as a **tag** on the reply; never as a count, never on the post card, never as a badge. (The one-sided Support-lane meter of D-51 R5 is withdrawn — D-52 — pending a redesign.) Read by the ranking's balance term only (§9). Ruled D-51; ADR-0058. | `comments.friendly_fire` · `friendly_fire_dharma`, `endorse_count`, `contest_count` (§9 / `RANKING.md` §2) |
...
- **Pre.** User holds a current non-zero position on their side (or is entering — the reply is then their entry bet). `market.state = Open`. `parent_comment_id` references an existing comment in the same market. Stake clears the **reply floor** (`BET_MIN_STAKE_REPLY` = 50, higher than the post floor per ADR-0018). The request may carry `friendlyFire` (boolean, default `false`); it may be `true` only when the reply is a **Support** — the side being bought equals `parent.side_at_post_time` — which includes an entry reply placed as Support (ADR-0058). The replier must not be the parent's author: nobody replies to their own post, on either side (D-52).
- **System.** A reply is a bet (per §7): single transaction; insert comment row with `parent_comment_id` set and `side_at_post_time` = the replier's held side (frozen at post-time, **not** the parent's); insert bet row; `dharma_ledger` row; price moves. The reply's side relative to the parent's frozen side classifies it at read-time as **Support** (same side as parent) or **Counter** (opposite) — a derived classification, never a stored verdict. A Support reply may additionally carry **`comments.friendly_fire = true`** — a compose-time flag declaring that the replier backs the side but contests *this* argument (D-51, ADR-0058). The flag is written in the same INSERT as `side_at_post_time` and is never updated (Bucket A); it changes nothing about the bet — the stake buys the replier's held side, and the single-side rule, floors and price impact are untouched. The write path rejects it on an opposite-side reply (400 `friendly_fire_requires_support`) and on a top-level post (400 `friendly_fire_requires_reply`, also a DB CHECK), writing nothing in either case. Depth limit `REPLY_DEPTH_MAX = 1` enforced (a reply cannot itself be replied to; flat replies per ADR-0017). Replies are ordered by stake within side per §9.
- **Response.** `{ betId, commentId, side, parentCommentId, friendlyFire }`.
- **Errors.** As F-COMMENT-1 but `below_reply_floor` in place of `below_post_floor`; plus 400 `reply_depth_exceeded`, 404 `parent_comment_not_found`, 400 `friendly_fire_requires_support`, 400 `friendly_fire_requires_reply`, 400 `self_reply_forbidden` (D-52 — checked inside the transaction after the parent read, before friendly-fire eligibility; writes nothing).
- **Invariants.** INV-1, INV-3.
- **Acceptance.** `tests/server/comments/reply.test.ts::reply-is-a-bet-replier-side-not-parent`, `tests/server/comments/reply.test.ts::reply-floor-enforced`, `tests/server/comments/friendly-fire.test.ts::support-with-flag-buys-held-side`, `tests/server/comments/friendly-fire.test.ts::flag-on-counter-rejected-no-rows`, `tests/server/comments/friendly-fire.test.ts::flag-on-top-level-rejected`, `tests/server/comments/friendly-fire.test.ts::entry-reply-support-with-flag`, `tests/server/comments/friendly-fire.test.ts::replay-with-flag-flipped-409`, `tests/server/comments/self-reply.test.ts::author-reply-rejected-no-rows`, `tests/server/comments/self-reply.test.ts::author-reply-rejected-in-transaction`.
...
- **System.** Per C7: the debate view re-invokes its own server read at `POLL_INTERVAL_MS_DEBATE_VIEW`. New posts, new reply-bets, changed markers, re-ranking under the active mode, and the F-DEBATE-5 price series appear on the next poll. The refresh is a client-side interval that re-invokes the existing `/m/[slug]` server read path — `loadDebateView` composed with `loadViewerMarketContext` at the page RSC per ADR-0034 D-2 — and is *not* a fetch against a separate read endpoint for CONTENT. ⚠ **AMENDED AT ADR-0053 (2.0.5): the tick ASKS BEFORE IT REFRESHES.** Each tick consults `GET /m/[slug]/version`, which returns one opaque token over (status, pool reserves, moderation count) and no content whatsoever, and re-invokes the composed read path above ONLY when that token differs from the one the tab last rendered. The prohibition this sentence carries is unchanged in substance — masking still has exactly one home, because the version route reads no comment body and a detected change still runs `loadDebateView` — and the endpoint is admitted to SPEC.2 §4.3 on precisely that basis. The reason it moved: measured on production 2026-09-16, `/m/[slug]` broke at 50 req/s offered with the connection pool pinned at 53 of 60 **while Postgres itself ran only 1-3 queries**, so the refresh-regardless tick was spending the resource the site had run out of on readers for whom nothing had changed. The baseline token is taken at MOUNT, not at the first tick, so a change landing between the server render and that tick is still caught; a failed check is a non-answer and never refreshes. No such handler exists: SPEC.2 §4.3's Route Handler catalogue is closed, and F-DEBATE-4 adds no entry to it. The constraint is structural, not stylistic — a dedicated read endpoint would fork removal-masking, which is keyed solely on `loadRemovedSet` inside `loadDebateView` (ADR-0021; ADR-0034 D-4), into a second implementation, and two masking implementations are two places for masking to diverge. Re-invoking the composed path instead carries the price bar, the debate list and the price series on one payload with masking and viewer-scoping intact by construction, and `loadDebateView` keeps the viewer-independent signature ADR-0034 D-1 pins. Polling is **suspended while the document is hidden, while any bet composer is open on the surface, and while the reader is idle**, resuming with an immediate refresh when the document becomes visible, when the last composer closes, and on the first reader input after idling. **Idle** means no pointer, touch, wheel, scroll or keyboard input for `POLL_IDLE_TIMEOUT_MS_DEBATE_VIEW` (5 minutes); a document becoming visible counts as input. Reader input never overrides the other two suspensions. Idle suspension exists for cost, not correctness: a tab left open and visible was never suspended by the other two rules, and each tick re-executes the route's layout and page on the server for a reader who is not there *(added 2.0.4, POLL-IDLE)*. The post columns' auto-advance on the same surface is a **separate, client-only timer** at `AUTO_ADVANCE_MS_DEBATE_VIEW` (15 seconds); it makes no server read and is neither retuned nor suspended by the poll's rules. A poll must never discard composer input, focus, caret position, or scroll position. Polling **stops permanently once the market leaves the `Open` state** — a market in any non-`Open` state is never polled again, consistent with the freeze posture of F-DEBATE-3. The poll reads `market.status` from the view model and carries **no independent notion of the global conclusion freeze**: `system_state.frozen_at` does not reach the client, and `FREEZE_INSTANT_UTC` compared against a client clock is a guess about a database state flip, not a signal. No SSE / WebSockets in v1.
...
**Refresh — the shared read block.** The same mechanism governs the rest of what a market surface renders, and since ADR-0051 it governs it in the same way. The **shared read block** — the debate view's comments, ranking, replies, totals and media, and Discovery's per-market totals, media, series and top posts — is derived at most once per **`SHARED_VIEW_MIN_WINDOW_MS`** per market and shared verbatim by every reader in that window.

⛔ **The window replaces a key, and the key is what was wrong.** Both blocks previously keyed on the pool `reserves`. A cached function's key is the arguments it is called with, so every bet moved the pool, changed the key, and forced a full re-derivation for **every reader of that market** — invalidation coupled to *activity*, which is precisely the failure this section names two paragraphs above, on the surface where it costs most. The window is not a tolerance for staleness that was previously absent; it is what lets the derivation **coalesce** at all.

⚠ **Nothing priced rides the *shared read block*.** Every figure a surface renders that is a function of the pool — the price bar, `unitToWin`, the chart's terminal point on an `Open` market, and the hero panel's `Đ staked → Đ now` value — is composed **outside** that block, from a pool read the page already performs, at no additional query. ⛔ **This paragraph read "Nothing priced rides the window… A Đ amount on a public surface is never served from a window" and ADR-0055 reverses that sentence for Discovery** — corrected here rather than appended, because an appendix reverses nothing a reader reaches first (`O-5`). Two reasons, both measured on production at `d2af66cd`. **(1) The rule governed composition order, not freshness.** It required the figure to be computed outside the cached BLOCK and said nothing about the PAGE being cached — and `/m/[slug]`, the only surface where a bet is actually placed, is prerendered: measured serving a **thirteen-hour-old** price, corrected only by the F-DEBATE-4 poll. **(2) Discovery was therefore stricter than the surface it links to, at the cost of a database connection per visitor** on the product's highest-traffic page, against a ceiling the load-test campaign showed to be connection supply rather than database work. Discovery's pool read now rides `DISCOVERY_PRICE_MIN_WINDOW_MS` (§16.1) — **5 s, pinned strictly tighter than `SHARED_VIEW_MIN_WINDOW_MS` so a price is never older than the arguments beside it**, and a rounding error against the thirteen hours the betting page serves. The composition rule above is unchanged and still load-bearing: a priced figure never comes from the shared block, which carries no price and would be wrong at any freshness.

⛔ **A participant sees their own argument immediately, and that is a separate mechanism rather than a property of the window.** Because every comment rides a bet (**INV-1**), the superseded `reserves` key returned an author's own post to them as a side effect of their own bet having moved the pool. That was never a designed behaviour and does not survive a clock. The surface therefore reads **uncached** for a viewer who posted in this market inside the last window — bounded by the clock, scoped to that one viewer, and never an input to the shared derivation, whose output must stay identical for every reader. Other readers see a new argument within one window, which is at most one F-DEBATE-4 poll tick.

*(Added at CACHE-KEY-1, 2026-09-12, ADR-0051 — which supersedes ADR-0041 D-2 in part and closes its OQ-1. The founder ruled three things this paragraph records: the window length, the bypass over a client-side optimistic render, and that the hero's Đ figure is computed live rather than cached.)*
...
| `POLL_INTERVAL_MS_DEBATE_VIEW` | Debate-view poll interval in milliseconds (per C7) — the cadence at which `/m/[slug]` re-invokes its server read (§9 F-DEBATE-4). **Pinned provisionally at 30000** (raised from 15000 at 2.0.4, POLL-IDLE, to halve per-tab server renders; idle readers are further suspended by `POLL_IDLE_TIMEOUT_MS_DEBATE_VIEW`). Unlike the pinned design constants above, this one **remains deferred to the number-tuning pass** (SPEC.2 §4.3 assigns the tune to HARDEN.6); the pin exists only because the flow is unbuildable without a value and go-live precedes that pass. Sized against the measured shape of one tick: because the refresh re-executes the route's **layout as well as its page**, a tick costs twelve to fourteen sequential database round-trips per open tab — including **two** session reads — and nothing throttles it per tab. **Three of those round-trips are not constant:** `listMarketComments` carries no `LIMIT` and the price-series replay walks the market's whole event history, so per-tick cost **scales with the market and grows monotonically across the live window**, most steeply on the markets carrying the most viewers. The quantity to size against is therefore **ticks × concurrent tabs × round-trips × O(market events)**, not the interval alone; visibility suspension is the larger lever, and a cap or keyset on `listMarketComments` is a HARDEN.6 **prerequisite**, not an optimisation. The value is read from the constant at every call site and never inlined, so the HARDEN.6 tune is a one-line change. |
| `POLL_IDLE_TIMEOUT_MS_DEBATE_VIEW` | How long a debate-view reader may go without pointer, touch, wheel, scroll or keyboard input before the poll suspends (§9 F-DEBATE-4); the first input afterwards refreshes immediately and resumes the `POLL_INTERVAL_MS_DEBATE_VIEW` cadence. **Set to 300000** (5 minutes) at 2.0.4, POLL-IDLE. Client-side only — it changes no server read and no cache. Must stay longer than `POLL_INTERVAL_MS_DEBATE_VIEW`, or an active reader could idle out between two ticks. Read from the constant at every call site, never inlined. |
| `AUTO_ADVANCE_MS_DEBATE_VIEW` | Cadence at which the debate view's post columns auto-advance to the next card (§9 F-DEBATE-4). **Set to 15000.** A client-only timer with no server read. ⚠ **Independent of `POLL_INTERVAL_MS_DEBATE_VIEW`:** the carousel used to read the poll constant directly, and was split out at 2.0.4 (POLL-IDLE) so that moving the poll to 30000 did not slow the carousel. Read from the constant at every call site, never inlined. |
...
| `SHARED_VIEW_MIN_WINDOW_MS` | Minimum interval between derivations of a market's **shared read block** — the debate view's comments, ranking, replies, totals and media, and Discovery's per-market totals, media, series and top posts (§9 *Refresh*; ADR-0051). Within the window a derivation is reused and shared by every reader. **Set to 15000** — a design pin, not a tuned economy value. ⛔ **It replaces a key, not a shorter window.** Both blocks were keyed on the pool `reserves`, and a `'use cache'` key is its argument list, so every bet changed the key and forced a full miss for every reader — the cache helped least exactly when load was highest, which is the failure this section names. ⚠ **Not derived from `POLL_INTERVAL_MS_DEBATE_VIEW`**: two independent tunables that agreed at 15000 until the poll moved to 30000 at 2.0.4 (POLL-IDLE) and this window did not — which is the point, since retuning the poll cadence must never silently retune freshness for every reader of every market. ⚠ **Shorter than `MARKET_SERIES_MIN_WINDOW_MS`, deliberately** — that one floors a price *history*, which may honestly be a minute old with its live edge recomposed outside the window; this one floors *arguments*, which are the product. ⛔ **It is not what makes a poster see their own argument**: that is a bounded per-viewer bypass at the page (ADR-0051 D-3), so shortening this constant is not a way to improve that case and lengthening it does not harm it. Read from the constant at every call site, never inlined. |
| `SHARED_VIEW_EXPIRE_SEC` | The outer bound, in seconds, on serving a `SHARED_VIEW_MIN_WINDOW_MS` entry **stale** while a revalidation is in flight. **Derived as `MARKET_SERIES_MIN_WINDOW_MS / 1000` (60)**, never pinned separately — so no shared block is ever older than the oldest thing rendered beside it (⚠ the walk ITSELF is the one exception on both its layers, `cached-series.ts`'s own hour ceiling and, since CACHE-COALESCE-2, its fleet-wide entry's TTL — it is history, and this sentence is about the blocks beside it). ⚠ Deliberately **not** the price walk's own `window × 60` ratio, which is right for a history and would let a 15 s window serve fifteen minutes stale in a low-traffic lull. Since CACHE-COALESCE-1 (ADR-0051 P1) it is also the TTL of the fleet-wide shared entry in Upstash, and of the `removed` marker a moderation removal stamps beside deleting it — the marker must outlive any entry or in-flight render that could still carry the body, and this is the longest either can live. |
| `SHARED_VIEW_LOCK_MS` | How long one runtime instance may hold the **render lock** on a market's shared read block (CACHE-COALESCE-1, ADR-0051 P1; since CACHE-COALESCE-2 / P2 the same lock, wait and size bounds govern all THREE per-market blocks — debate view, reserve walk, market data — each under its own window). **Set to 10000.** ⛔ The window above was measured to cost *instances × markets × 4/min* on production — `'use cache'` dedupes inside one instance only — so the lock is what makes a stale window cost ONE render across the fleet. Above any honest render, below the point where a holder that died mid-render would stall its market: the lock releases by expiry, never by hand. |
| `SHARED_VIEW_WAIT_MS` / `SHARED_VIEW_WAIT_POLL_MS` | How long an instance with **no entry to serve** waits for the lock holder's before rendering itself, and the poll cadence while it waits. **Set to 2000 / 100.** A cold-market bound, not a steady-state cost: with an entry present the loser serves it stale and never waits. Every Redis failure on this path degrades to a local render — the cache is never why a page fails. |
| `VERSION_MIN_WINDOW_MS` | How long one fleet-wide version token is served before one caller re-derives it (CACHE-COALESCE-3, ADR-0051 P3). **Set to 2000.** Below the version route's own edge `s-maxage=5`, so the store adds no staleness the CDN was not already adding; at six markets it bounds the database to three reads a second however many tabs poll. ⛔ The route it serves broke first under a burst — R-18 logged 1,000 `EMAXCONN` 500s in thirteen seconds, every one an edge miss asking the pooler for a connection past its 200-client cap. |
| `SHARED_VIEW_MAX_BYTES` | The largest serialized shared entry the store will write (CACHE-COALESCE-1 Gate C, ADR-0051 P1). **Set to 900000** — under Upstash's 1 MB value limit with margin. ⛔ An entry over the limit is not a slow path but an INVISIBLE one: the `SET` fails, no entry ever lands, and every non-holder pays the cold-market wait and then renders anyway, which turns single-flight into a latency tax. Over the bound the store raises an `oversize` flag for `SHARED_VIEW_EXPIRE_SEC` instead — **for every block, regardless of the block's own expire** (CACHE-COALESCE-2 Gate C: an hour-long flag on the walk, which only grows, would have been permanent) — and the fleet renders locally, the pre-patch behaviour, without locking or waiting. |
...
| `DISCOVERY_PRICE_MIN_WINDOW_MS` | Minimum interval between LIVE POOL PRICE reads on Discovery (`/`) — the per-market spot and reserves behind every card, the hero chart's live tail and the hero's `Đ now` figure (§9 *Refresh*; ADR-0055). **Set to 5000.** ⛔ **Not a performance dial:** it is the largest lag that keeps Discovery strictly fresher than `SHARED_VIEW_MIN_WINDOW_MS`, the content window beside it, so a figure a reader can act on is never older than the arguments around it. Raising it toward that constant creates the defect it exists under; lowering it below a second restores the per-visitor read without buying a perceptible freshness. A guard pins the ordering. Read from the constant at every call site, never inlined. |
| `DISCOVERY_PRICE_EXPIRE_SEC` | The outer bound, in seconds, on serving a `DISCOVERY_PRICE_MIN_WINDOW_MS` entry **stale** while a revalidation is in flight. **Set to 30** — deliberately NOT `SHARED_VIEW_EXPIRE_SEC` (60) and not the price walk's `window × 60` ratio, which would hand a 5 s window a 300 s ceiling. This entry holds a price, so its ceiling is the number that bounds how wrong the front page can be. |
...
|---|---|---|---|
| 2026-09-23 | 2.2.0 | **Self-replies refused; friendly-fire meter withdrawn (D-52).** **§8 F-COMMENT-2** Pre (the replier must not be the parent's author), Errors (400 `self_reply_forbidden`), Acceptance (two tests); **§2** glossary and **§9** display paragraph — the D-51 R5 meter withdrawn pending a redesign, the tag stays; **F-DEBATE-1** post-focus sentence. No invariant change (minor bump). | D-52 |
| 2026-09-22 | 2.1.0 | **Friendly-fire toggle (ADR-0058).** A Support reply-bet may carry `friendly_fire = true` — the replier backs the side and contests *this* argument. **§2** glossary row; **§7** reply definition; **§8 F-COMMENT-2** Pre / System / Response / Errors / Acceptance (two error codes, flag rejected on Counter and on top-level); **§8** rider on the vote-removal note (the toggle is a different mechanic); **§9** two stance counts, `b` reads endorse vs contest, meter + tag described, no count and no badge; **F-DEBATE-1** post-focus elements. No invariant change (minor bump). | D-51 |
| 2026-09-17 | 2.0.7 | **ADR-0053 — the poll asks whether anything changed before it refreshes.** §9 F-DEBATE-4's poll re-invoked the whole composed server read every 30 s per open tab, so a reader who was merely PRESENT cost the same as one who was acting. It now fetches a tiny `GET /m/[slug]/version` probe — a hash over status, the two pool reserves and a moderation count — and calls `router.refresh()` only when that token moves. ⚠ **The refresh path itself is unchanged**, which is the half that matters: a changed token re-invokes the SAME composed read with masking applied where it always was, so the probe decides only WHETHER to refresh and never WHAT the refresh returns. §4.3's closed handler catalogue is therefore narrowed to **content-bearing** endpoints rather than abandoned — an endpoint that cannot carry a body cannot fork a masking decision (SC-1). The probe is `s-maxage=5`, so the edge answers it and the origin sees ~1 request per market per 5 s however many tabs are open. | ADR-0053, 2026-09-17 |
| 2026-09-17 | 2.0.6 | **ADR-0055 — Discovery's prices ride a five-second window.** §9 *Refresh*'s ⚠ paragraph asserted *"Nothing priced rides the window… A Đ amount on a public surface is never served from a window"*; that sentence is **reversed for Discovery and corrected in place** (`O-5`). The rule governed composition ORDER, not freshness: it required a priced figure to be computed outside the cached block and said nothing about the PAGE being cached — and `/m/[slug]`, the only surface where a bet is placed, is prerendered and was measured on production serving a **thirteen-hour-old** price. Discovery, where nobody can bet, was paying a live pool read **per visitor** to be stricter than the surface it links to, on the product's busiest page, against a measured ceiling of connection supply. §16.1 mints `DISCOVERY_PRICE_MIN_WINDOW_MS` (5000) and `DISCOVERY_PRICE_EXPIRE_SEC` (30), the first pinned strictly tighter than `SHARED_VIEW_MIN_WINDOW_MS` by a guard so a price is never staler than the arguments beside it. The composition rule itself is unchanged: a priced figure still never comes from the shared read block. | ADR-0055, 2026-09-17 |
...
POLL_INTERVAL_MS_DEBATE_VIEW = 30000  # provisional pin (§9 F-DEBATE-4 — the flow is unbuildable without a value); raised from 15000 at 2.0.4 (POLL-IDLE); STILL deferred to the number-tuning pass / HARDEN.6
POLL_IDLE_TIMEOUT_MS_DEBATE_VIEW = 300000  # 5 min without reader input suspends the poll (§9 F-DEBATE-4; 2.0.4, POLL-IDLE); client-only
AUTO_ADVANCE_MS_DEBATE_VIEW = 15000  # post-column auto-advance cadence (§9 F-DEBATE-4); client-only, independent of the poll (split out at 2.0.4)
...
SHARED_VIEW_MIN_WINDOW_MS   = 15000  # pinned design value (§9 Refresh; the shared read block's window, ADR-0051); not deferred to tuning
SHARED_VIEW_EXPIRE_SEC      = 60     # DERIVED = MARKET_SERIES_MIN_WINDOW_MS / 1000 — the stale-serving ceiling; never pinned separately
DISCOVERY_PRICE_MIN_WINDOW_MS = 5000  # pinned design value (§9 Refresh; ADR-0055); MUST stay < SHARED_VIEW_MIN_WINDOW_MS
DISCOVERY_PRICE_EXPIRE_SEC  = 30     # the stale-serving ceiling on the above; seconds, not milliseconds
