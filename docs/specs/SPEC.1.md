...
| `DISCOVERY_SERIES_MAX_POINTS` | Maximum plotted points on the §22 Discovery price sparkline after uniform-stride downsampling (first and last always retained). **Set to 64** — a design pin for the decorative card/hero sparkline. *(Shipped at UI.A4; recorded here at 1.0.22 to close its §16.1 omission.)* |
...
DISCOVERY_SERIES_MAX_POINTS = 64  # pinned design value (§22 Discovery sparkline; decorative thumbnail); not deferred to tuning
OTP_TTL_MIN = TBD
