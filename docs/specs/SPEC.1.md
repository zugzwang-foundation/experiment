...
- **Version:** 1.0.44 (semver; bump major on invariant changes)
...
- **Acceptance.** §17 rows `debate-view::poll-interval`, `debate-view::poll-preserves-removal-masking`, `debate-view::poll-stops-when-market-leaves-open`, `debate-view::poll-suspends-while-hidden-or-composer-open`, `debate-view::poll-refreshes-price-chart`. File placement follows the behaviour under test — the interval, suspension and chart-refresh rules are client-render tests, the masking and stop rules read-path tests — and is recorded in `docs/specs/flows/F-DEBATE-4.md` at build (SPEC.2 §13.4). This supersedes the prior `tests/server/debate-view/poll.test.ts::interval-respected` cite, which named a file that does not exist and a case-id absent from §17 — a build error under SPEC.2 §13.5.
...
**Refresh.** The series rides the **F-DEBATE-4 polled read** at `POLL_INTERVAL_MS_DEBATE_VIEW` — a chart that lagged the price bar sitting directly beneath it on the same header would be worse than no chart. **Named cost:** each poll replays that market's event history for that viewer. It is bounded by the market's event count and accepted at experiment scale (Discovery already replays up to `DISCOVERY_GRID_SIZE` markets per page load, so one market per poll is the cheaper operation). Should the cost prove material, the mitigation is a **cached-per-load read** (SPEC.2 R-2, the §22 posture) — **not** a stored series.

**Accessibility.** Unlike the §22 sparkline, this chart is **not** `aria-hidden`: it carries an accessible text summary naming the opening price, the current price, and the domain endpoints. `PriceBar` and its existing accessible current-price text are **unchanged** and remain the canonical spot-price readout.
...
- **AGPL-3.0 §13 source link (`I6`).** Surfaced in the Terms of Service body: `Source code available at github.com/zugzwang-foundation/experiment under AGPL-3.0`. Hard legal requirement. Owner: Hrishikesh, before launch. *(Footer mechanism withdrawn 2026-08-02 — the product ships no page-level footer on any surface. The obligation is unchanged; only its home moved.)*
...
- **DPDPA grievance contact.** Email + named officer (per Indian Digital Personal Data Protection Act). Surfaced in the Privacy Policy.
...
| `debate-view::replies-two-slot-best-support-and-counter` | §9 F-DEBATE-1 — the SELECTION rule (read model + `src/lib/ranking`); **not** a market-view card render since 1.0.31 | ADR-0017 |
| `debate-view::replies-expansion-stake-sorted-within-side` | §9 F-DEBATE-1 — the expansion is the **post-focus** full stake-sorted list since 1.0.31; there is no in-card control | ADR-0017 |
...
| `debate-view::poll-preserves-removal-masking` | §9 F-DEBATE-4 | ADR-0021, ADR-0034 |
| `debate-view::poll-stops-when-market-leaves-open` | §9 F-DEBATE-4 | INV-4 |
| `debate-view::poll-suspends-while-hidden-or-composer-open` | §9 F-DEBATE-4 | — |
| `debate-view::poll-refreshes-price-chart` | §9 F-DEBATE-4, F-DEBATE-5 | — |
| `debate-view::price-chart-market-lifetime-domain` | §9 F-DEBATE-5 | — |
| `debate-view::price-chart-single-point-flat-line` | §9 F-DEBATE-5 | — |
| `debate-view::price-chart-downsample-cap` | §9 F-DEBATE-5 | — |
| `debate-view::price-chart-nodes-top-per-utc-day-per-side` | §9 F-DEBATE-5 | — |
| `debate-view::price-chart-nodes-exclude-content-removed` | §9 F-DEBATE-5 | — |
| `debate-view::price-chart-node-side-frozen` | §9 F-DEBATE-5 | INV-3 |
| `debate-view::price-chart-frozen-after-resolution` | §9 F-DEBATE-5 | INV-4 |
| `debate-view::price-chart-collapsed-time-axis` | §9 F-DEBATE-5 | — |
| `debate-view::price-chart-collapsed-no-nodes` | §9 F-DEBATE-5 | — |
...
- **Standalone About / FAQ / help page.** Served by the **onboarding-deck re-show** (the About / Rules tab opens the DESIGN.W2.2 six-card deck). The dedicated **feature-guide page (§21.6) is deferred from v1** (founder has other plans).
...
| 2026-07-23 | 1.0.22 | §0; §9; §16.1; §17; §20; Appendix B | **Market-detail price-history chart amendment (UI.19 blocker).** §9 gains, appended after the Market-media sub-section (append-don't-renumber, per the §21/§22/§23 precedent), **"Market price history — the market-detail chart"** + **F-DEBATE-5**: a full-size **time-scaled** two-line YES/NO probability chart in the Market-Detail header above `PriceBar`, in two modes — **collapsed** (no axis, no nodes, expand affordance, default) and **expanded** (time axis + post nodes) — mirroring the §23 card→overlay pattern. **X = market lifetime** (`market.opened` → last `bet.placed`/`bet.sold` event), deliberately *not* the §23 fixed experiment window (rationale in-section: a market's price is market-scoped, whereas a Dharma trajectory is lifetime-scoped and needs a shared axis for cross-user comparability). **Y = fixed 0–100 %**, no autoscale. **Derivation reuses the §22 replay mechanism unamended** — no stored series, no new table, no new column, no new event type, no DDL; one additive per-market series call on the market-detail read; a materialised series remains a spec change, not an implementation choice. **Downsampling** capped at the new `MARKET_SERIES_MAX_POINTS` (256). **Post nodes** (expanded only): one per (UTC day, side) by **§9 Top** (`RANKING.md`) — no second ranking rule is introduced or permitted; node y = YES price at post timestamp; node side = `side_at_post_time` (**INV-3**); **content-removed excluded from node eligibility** (mirrors §22 F-DISC-2; ADR-0021 decoupling); replies are not nodes; frozen at resolution (**INV-4**). **Sparse rule:** fewer than two points → flat line at the opening price, no empty state. **Refresh** rides the F-DEBATE-4 poll, with the per-poll replay cost named in-section and the mitigation pinned to a cached-per-load read (SPEC.2 R-2), never a stored series. **Accessible text summary required** — this chart is not `aria-hidden`, unlike the §22 sparkline; `PriceBar` unchanged. **K_eff explicitly never plotted** (§3 `G3` + §12.3 restated at the surface). **§22 is untouched** — the Discovery sparkline keeps its decorative, index-spaced rendering. **§16.1** adds `MARKET_SERIES_MAX_POINTS` (256) and, closing a drift, `DISCOVERY_SERIES_MAX_POINTS` (64 — shipped at UI.A4, never recorded in §16.1 or Appendix B). **§17** adds eight `debate-view::price-chart-*` rows. **Appendix B** adds both constants as pinned design values. **§0** → 1.0.22, last-updated 2026-07-23. SPEC.2 untouched (no new store, route, or architectural pattern; grep-verified no same-section touch). | The betting surface had no price history. `/m/[slug]` shipped a `DeferredPlaceholders` stub reading *"Price history — arrives with the ledger-replay read-model"*, and the read model it was waiting for landed at UI.A4 (`discovery/price-series.ts`) and was reused at UI.A5 without anyone returning to it. The UI.19 recon @ `903e185` confirmed the absence, confirmed the §9 gap (§22 governs Discovery only, §23 Profile only, no F-* flow for a market-detail chart, no §17 row), and disqualified the cheap fix: the shipped `PriceSparkline`'s `size` prop is cosmetic (one component at two sizes, identical `viewBox`, no axes) and its X axis is **index-spaced**, discarding the real timestamps each point carries — acceptable on a decorative thumbnail, misleading on the surface where stake is committed. Authored at UI.19's spec-first halt (no-code-before-spec; the §22/#242 and §23/#248 precedent). No new ADR: the amendment applies existing rulings — the §22 replay posture, §9 Top, ADR-0021 removal masking, INV-3 and INV-4 — to a new surface, exactly as §22 and §23 did. | — |
| 2026-07-23 | 1.0.23 | §0; §10.8; §20 | **Đ display-precision rule.** §10.8 gains the **0-dp / ROUND_HALF_UP** display rule for every Đ value rendered to a user, with the scope boundary made explicit (odds multipliers, percentages, and counts are not Đ values), full precision preserved through the ledger, engine, read models, DTOs, the ADR-0025 `.md` export and the public dataset, and `-0` forbidden. Codifies **rounded values are terminal** — never read back into arithmetic, comparison, validation, clamping, or conditional rendering — with displayed-space aggregate identities as the sole exception — the §23 Net P/L tile and the reply split-bar total, where a rendered total derives from its rendered components so the on-screen arithmetic holds and the sell-module input seed as a named, docketed implementation exception. Records that the **basis** is unchanged (Đb), that 0-dp rendering admits ±0.5 Đ divergence at the render, and that **ceiling was rejected** as systematically overstating against both §10.8 and cpmm §10.3. | Staging rendered `9.999999999999999999` on the §23 Positions-value tile, `-0.000000000000000001` on Net P/L, and `Đ 20.666666666666666666` on the composer's TO WIN preview against a Đ 10 stake. The math was correct — Đb is `computeSell(quantity).proceeds` — but §10.8 was titled "Display rules" and specified no precision, and both shared view formatters trimmed trailing zeros without ever rounding, so any non-terminating decimal surfaced all 18 digits. View-layer only: no DDL, no migration, no new event type, no ADR, no read-model or DTO change. | — |
| 2026-07-31 | 1.0.24 | §0; §10.8; §20 | Percentage display — the complement rule. §10.8 gains a whole-percent rule for market prices: YES canonical (ROUND_HALF_UP integer digit-extraction on the canonical decimal string, never float multiplication), NO derived as 100 − YES and never independently rounded. One side-scoped formatter for every price-percent surface, with a marker-allowlisted single-side escape hatch for genuinely single-side readouts (the price chart's opening and current YES points). Amends §10.8's prior scope boundary, which excluded percentages by name at 1.0.23. Governs the ADR-0025 .md export's prose percentages — a deliberate divergence from the Đ export exemption, which existed because a full-precision form existed; integer percentages have none. The export's YAML front-matter yes_price / no_price stay exact 2-dp ROUND_HALF_EVEN and are untouched. Scoped to market price percentages: the reply split bar's Dharma-support percentage is integer-TRUNCATED by design and is expressly not governed. Price-bar fill geometry unchanged and docketed. View-layer only: no DDL, no migration, no new event type, no ADR, no §16.1 constant, no read-model or DTO change. | Independent per-side ROUND_HALF_UP renders 101% at any exact .xx5 tie (0.525 / 0.475 → 53% + 48%), visible in the price bar's labels and aria-label, in both debate column headlines, in both slot headers, and in the published .md export; a second float-based percent formatter (chart/geometry.ts fmtPct, minted at UI.19 slice 1) rendered the same current price a few pixels from the first. No test in the suite exercised a tie. | — |
| 2026-07-31 | 1.0.25 | §0; §9; §16.1; §17; §20; Appendix B | F-DEBATE-4 polled-on-view refresh — mechanism, provisional pin, suspension and stop rules, acceptance. §9's System bullet replaced: the refresh is a client-side interval re-invoking the existing `/m/[slug]` server read path (`loadDebateView` composed with `loadViewerMarketContext` at the page RSC, ADR-0034 D-2), not a fetch against a read endpoint — SPEC.2 §4.3's handler catalogue is closed at eleven and F-DEBATE-4 adds no twelfth. Two behaviour rules added: polling suspends while the document is hidden **or any bet composer is open**, and stops permanently once the market leaves `Open`. The stop rule is scoped to `market.status` alone and the spec now states explicitly that the poll carries no notion of the global conclusion freeze. §16.1 and Appendix B gain a provisional pin of 15000 ms, explicitly still deferred to the HARDEN.6 number-tuning pass and read from the constant at every call site. §9's Acceptance cite replaced — it named a nonexistent file and a case-id absent from §17 — and §17 gains four rows: masking-survives-poll (ADR-0021, ADR-0034), stops-when-market-leaves-open (INV-4), suspends-while-hidden-or-composer-open, and refreshes-price-chart. Two docketed corrections ride this commit where their anchors still hold: §20's 1.0.24 ADR cell and Appendix B's stale version trailer. No DDL, no migration, no new event type, no new ADR, no read-model or DTO change, no Route Handler. | A specified §9 flow with a §17 acceptance row and a §16.1 constant was never built: `/m/[slug]` is a server component whose client tree holds no timer, so a participant watching a live market sees a frozen page until manual reload, and the F-DEBATE-5 price chart is inert in production for the whole 51-day window. The constant had no value, so the flow was unbuildable; §9 specified polling a read endpoint that does not exist in SPEC.2 §4.3; and §9's acceptance cite named a file absent from disk. The suspension rule is keyed on **composer-open rather than composer-dirty** because a refresh does not disturb composer state at all — text, controlled state, focus, caret and scroll all survive, measured — but a refresh whose server render *throws* destroys the entire client tree and the unsaved argument with it, arriving as an HTTP 200 with no error hook, undetectable and unrecoverable. An open-but-empty composer sits in that blast radius identically to a dirty one, so the reader's intent to compose is the correct gate, not a non-empty predicate. The global-freeze clause was dropped rather than implemented because `system_state.frozen_at` reaches no client component and the only client-side alternative is a compile-time date literal compared against the viewer's own clock. | — |
| 2026-08-02 | 1.0.26 | §0; §16.5; §18; §21.6; §20 | **Page-level footer withdrawn.** §16.5 `I6` — the AGPL-3.0 §13 source link moves from footer text on every page to the Terms of Service body; the string is unchanged. §16.5 DPDPA grievance contact — drops its footer arm, surfaced in the Privacy Policy alone. §18 — the trailing "the whitepaper link remains in the footer" is struck from the About / FAQ row. §21.6 — the standalone "The footer whitepaper link remains." line is deleted. §0 — 1.0.25 → 1.0.26. | Founder ruling 2026-08-02: the product ships no page-level footer on any surface — not reduced, not AGPL-only, none. B4 (UI.11) is withdrawn, **not rescoped**. `I6` is a hard legal requirement and **survives unchanged** — only its mechanism moved, alongside the existing ToS and Privacy Policy rows in §16.5. The whitepaper-in-footer commitment, stated identically in §18 and §21.6 and retained through the §21.6 descope at 1.0.10, is struck from both **by entailment**: no footer exists to hold it, and no public whitepaper URL was ever supplied. The whitepaper's own surfacing is undecided and out of scope here. Documentation only: no DDL, no migration, no new event type, no ADR, no §16.1 constant, no read-model change, no §17 acceptance row. The change-log row at 1460 recording "whitepaper-in-footer retained" is historical record and is deliberately untouched. | — |
| 2026-08-03 | 1.0.27 | §0; §20; §21; §21.8 | **§21.8 minted — the signed-in Dharma cluster.** New subsection at the tail of §21 governing the global header's right-zone Đ cluster for an authenticated participant: one glyph, exactly two stats, in a fixed DOM order (Portfolio, then Balance). Portfolio is pinned to the §23 Positions-value quantity on the §10.8 basis, with `payout_events` row existence as the open/settled discriminant and same-source derivation against `loadProfilePositions`, so the header figure and the §23 tile are byte-identical against the same pool state (FI-2); mark-to-market is restated as not a display basis. Balance is pinned as spendable-today, and its divergence from the §23 Wallet-value tile's raw ledger figure is recorded as DELIBERATE rather than as drift — both true, measuring different things, distinguished by their labels with no reconciliation attempted. Placement is bound to the §21.1 register boundary: both stats left of the divider, the identity chip between cluster and divider, the visitor counter the sole element to its right, and a later layout pass may not collapse them adjacent. Absence and zero are separated: no `dharma_ledger` row hides the cluster entirely, no open positions renders Đ 0, and a failed Portfolio read degrades to Balance alone rather than removing the working figure. §21 preamble in-scope clause rewritten: the count moves `Five` → `Six` and the deferred surface is named by number rather than by ordinal — "the sixth … is deferred" became self-contradictory once §21.8 landed. §21.7 left RESERVED for the freeze-banner rider. §0 version bumped to 1.0.27. | The spec-first gate did not clear on spec text: `Portfolio` appeared ZERO times in `docs/specs/`, and no clause placed a Đ figure in the global header at all — Balance shipped into that same hole at #283, and §22's "balance chip" mention is corroboration, never mandate (`UI-A4.md:32` defers it). §21.8 is therefore authored as a same-commit rider travelling with the code it governs — the DROUND (`5035183`) and UI.13 precedent — and is written to cover BOTH stats, discharging Balance's live gap at the same time rather than leaving a shipped surface ungoverned. It fixes the two properties a later pass could otherwise break silently: the FI-2 byte-identity between the header figure and the §23 tile against the same pool state (a second Σ Đb derivation is legal only because it derives on the same sources), and the §21.1 anti-conflation placement (a Dharma figure filed in the same visual bucket as a page-hit count fails with no visible symptom). TYPE-2 throughout — read-only, no engine, ledger, ranking or resolution contact; no DDL, no event type, no ADR, and **no §17 acceptance row** — the surface's acceptance is carried by the task's own test set (the FI-2 byte-identity lock, the §21.1 DOM-order guard, the zero-versus-absence matrix) and registry rows are deferred to the next SPEC.1 touch rather than minted at a merge gate. | — |
| 2026-08-04 | 1.0.28 | §13 F-AUTH-4 · §21.7 | The W2.1 F-AUTH-4 override lapsed — its SPEC.1 sync was never performed and its footer mechanism was withdrawn 2026-08-02, so §13 stands unamended. §21.7 minted as a RESERVED stub for the freeze-banner rider, closing the §21 numbering gap. | Operator ruling — Hrishikesh | — |
| 2026-08-04 | 1.0.29 | §0; §10.8; §20 | **§10.8 gains the digit-grouping rule — Đ values group in threes, product-wide.** Five paragraphs appended to §10.8 between the 0-dp rule and the complement rule, minting a concept the section was previously silent on. Every Đ value rendered to a user groups its integer part in threes with the **literal ASCII comma `,` (U+002C), never a locale-derived separator** — `toLocaleString` and `Intl.NumberFormat` are forbidden on a Đ value, since Đ figures render in both server and client trees and a locale-derived separator resolves differently in the two, producing a hydration mismatch and rendering `1.234` for one thousand two hundred and thirty-four Dharma under a `de-DE` runtime. **Grouping is a property of the single shared display formatter, not a choice made at a call site**: exactly one formatter renders a Đ value, it rounds and groups together, and no ungrouped display variant survives to be selected by mistake — an opt-in convention that must be remembered at thirty render sites will be broken at the thirty-first, which is precisely the defect this closes (the bet composers grouped while the header stats, the §23 Positions-value tile and the discovery staked totals did not). **The export path takes the two-layer rule**: within the ADR-0025 `.md` debate export the prose body groups because it is read by people, while the machine-readable YAML front matter never groups and its `total_stake_dharma` stays exact at full precision — mirroring the split this section already ratifies for percentages — and any future machine-consumed artefact, the §12.2 public conclusion dataset among them, is front-matter-class and never groups. **Grouped values are terminal in the strong sense** — a grouped figure is not merely a string that should not be read back but one that cannot be, `Number("1,234")` being `NaN` — which reinforces rather than relaxes the named sell-module seed exception, whose editable amount input keeps seeding from the exact ungrouped Đb string for the full-exit byte-identity check. **Scope restated**: odds multipliers, percentages, counts and timestamps are not Đ values and are not grouped, and a count must never be routed through the Đ display formatter at all; the bet composer's TO WIN preview is governed as a Đ value, since a winning share pays exactly Đ 1 at resolution and the figure is a Dharma amount a participant would receive — the multiplier exemption covers ratios, never quantities. | PRIMITIVES-1 C3 same-commit rider — the rule lands in the same commit as the code it governs, so the three pre-ruling tests (`market-card.test.tsx:65`, `dharma-cluster.test.tsx:260-263`, `hero-panels.test.tsx:103` — the first two docketed as such in their own comments, the third surfaced at execute) invert against a ratified rule rather than a doctored baseline. D1 narrows "never in export" to machine-readable output: the `.md` prose body is correct as built and the export path takes zero code change. No ADR, no DDL, no event type, no §16.1 constant, no §17 row. | — |
| 2026-08-15 | 1.0.30 | §0; §9; §16.1; §22; Appendix B | **The Discovery card carries no price sparkline — the spec is reconciled to the ratified mockup.** §22's read model (SCL-4) and F-DISC-1 drop the two-line sparkline from the card enumeration; the card is image + question + YES/NO price bar + `Đ staked · posts · replies`. §22 *Price series* restates the graph as hero-only. §9 *Market price history* narrows "the §22 card/hero sparkline is decorative" to "the §22 hero sparkline"; its decorative-versus-time-scaled distinction is untouched, and its §9 Accessibility sentence ("unlike the §22 sparkline") still holds because the hero sparkline remains. §16.1 and Appendix B narrow `DISCOVERY_SERIES_MAX_POINTS`'s description from "card/hero" to "hero" — **the constant and its value (64) are unchanged**. §0 → 1.0.30. Paired **design-language v0.8-draft** (same commit): §3.2 Market card locked composition drops the sparkline and RETAINS "must be identical everywhere"; the two-line-graph entry strikes "sparkline on cards". **design-canon §2 needed no amendment — it already read "Cards: image + title + stats + YES/NO bar", with no sparkline.** | **Founder ruling, HTML-FINISH · DISCOVERY Gate C finding G-1: the spec was stale and the mockup governs.** Row 1 of the ratified HTML-FINISH row table deleted the tile chart; §22 and design-language §3.2 still named it, so the spec described a build that did not exist. The card sparkline was defined BY THIS SPEC as decorative — no axis, `aria-hidden`, index-spaced X — and composition of decoration is the ratified mockup's jurisdiction (`docs/design/mockups/surface_discovery_v1_0.html`, founder-ratified 2026-08-15 as layout authority; `.mcard` is title-block + price bar with nothing between at `:269-282`, and the orphaned `.spark` rule at `:155` styles no element in the document). Canon §2 and the mockup already agreed; SPEC.1 and design-language were the lone stragglers. ⚠ **NOTHING ARCHITECTURAL CHANGED**: no field, no query, no cache behaviour, no event type, no DDL, no ADR, no §17 acceptance row. `DiscoveryMarketView.series` still carries the series and the hero still renders the full-size two-line graph for the active carousel market — the change removes eight RENDERS, zero READS. Doc-only, landed as a same-commit rider on the branch that made it true (PR #334). Prior change-log rows naming the card sparkline (1.0.17, 1.0.18, 1.0.22) are historical record and are deliberately untouched. | — |
...
| 2026-08-19 | 1.0.36 | §0; §21.1; §20 | §21.1 rendered label corrected **"visitors" → "views"**, and the section's own noun for the metric corrected "page visits" → "page views". The counter increments once per page load; under the standard analytics taxonomy that number is page views, so "visitors" overstated it by roughly the average pages-per-session and "visits" would name a session metric the mechanism does not produce. **Metric, mechanism, placement, abuse guard, and the anti-conflation-by-mechanism property are all unchanged** — one integer, no user/session/device representation, nothing read from the ledger. Retained deliberately as internal identifiers, not copy: the §21.1 section heading, all `§21.1` cross-references, the `VisitorCounter` component, the `/api/visits` route, and the `visits:total:${env}` Redis key. **Known divergence recorded:** the LOCKED `DESIGN_W2_4-5-14_global-header_mockup-v0.2.html` and its copy register still render "visitors"; the mockup is frozen and is not edited — this section governs the live string, and an HTML-FINISH parity pass must treat this word as a spec-directed deviation, not a defect. No ADR, no DDL, no new event type, no SPEC.2 change. | Founder ruling 2026-08-19 from live staging observation — the header read 813 then 814 across a single in-session navigation, i.e. the label was measuring page loads while naming people. | No ADR |
| 2026-08-19 | 1.0.37 | §21.9 | **O1-DECK-R2 — the deck's visual pass.** §21.9's anatomy sentence gains the first card's ruled exception: the Zugzwang wordmark in place of a text title, no eyebrow, the wordmark rendered from the header's own component so the two cannot drift, and a visually hidden heading standing in for the absent text title. A new paragraph makes the deck's measure a function of its copy rather than of a fixed frame — the modal is widened until no subtext clause wraps, subject to the viewport — and removes the figure band's container, sizing each figure to the space the measure leaves. Homogeneity across the other six cards is stated normatively. ⚠ **No type size changed.** The founder's seventh refinement asked for one ramp step up; measurement found that **no ratified type scale exists** (`type.scale.*` is unminted in the design language and is explicitly not a token-contract slot), so the sizes stand rather than an arbitrary value being invented. No behavioural change: the gate, the two contexts, the card split, the marker and its writer are all unamended, and no §17 row moves. | O1-DECK-R2 | — |
...
| 2026-08-25 | 1.0.42 | §0; §13 F-AUTH-4 | **ONBOARD-CARD-2 — the acceptance screen loses its second control and its version label.** A second dated **AMENDMENT — 2026-08-25 (second)** block follows the first, superseding two more of F-AUTH-4's structural commitments. **(1) One control, not two.** The Continue/Cancel pair becomes a single control labelled **"Enter Zugzwang"**, still unavailable until the acceptance checkbox is ticked. ⛔ **Cancel's SEMANTICS are not superseded and still govern** — leaving without submitting commits nothing, leaves `tos_accepted_at IS NULL`, keeps the tuple consumed, issues no cookie; every clause of the old bullet describes the *absence* of a write, so the control was a link and nothing more, and the way out it offered survives in the global header's brand cluster. **(2) The version label leaves the screen** and renders on **`/legal`**, beneath the documents it identifies — still "alongside the documents" exactly as the acceptance-evidence paragraph requires; it is the documents that moved and the label that followed. ⚠ **The label is NOT removed from the product**, and its survival is asserted at its destination rather than merely implied by its absence at the origin (`onboarding-page-wiring.test.ts::legal-page::still-renders-the-version-colophon`). Acceptance evidence is untouched — five columns, both hashes, one transaction, with the initial grant — and the 1.0.41 hash-derivation caveat stands unchanged. Per **O-5** the supersession is written into each operative locus: System item 1's element (vi), item 4 (the gating stands, the label moves, and every later "Continue" reference in the section is renamed by that one pointer), item 5 (Cancel), and the acceptance-evidence paragraph's footer-text sentence together with 1.0.40's O-5 pointer on it. One stale cross-reference is repaired in passing: System item 3's "still read and hashed, per that amendment's own warning" pointed at the sentence 1.0.41 replaced. No schema, no migration, no new event type, no §17 row. **§0** → 1.0.42. | Operator ruling — Hrishikesh, 2026-08-25 (ONBOARD-CARD-2 kickoff, amendment text supplied verbatim). Ground: a signup screen with one deliberate action reads as one decision; a second, quieter control beside it invites a choice the flow does not actually offer, since leaving is what every browser already affords. | — |
| 2026-08-26 | 1.0.43 | §16.2 | **RPLY-CLOSE — the composer's blocked-state notices gain a PRECEDENCE and a RESERVED SLOT.** §16.2 gains two paragraphs. **(1) Precedence.** Where more than one blocked-state condition holds the composer renders exactly ONE notice, ordered **floor-above-balance → rate-limit countdown → over-cap** — the floor-above-balance sentence is §16.2-required and describes a *total* block, so a transient rate-limit banner must never displace the only explanation a viewer has for a dead form. **(2) A reserved slot.** The notice replaces the TO-WIN row, whose value is meaningless in every blocked state, and the slot's height is constant across all four states INCLUDING the unblocked one, so the composer does not change height under the viewer. Codifies behaviour built at RPLY-1 · R3 and measured at RPLY-3 (all four states identical per height per arm, both arms, 900/800/750/700/650). Doc-only; the shipped composer already conforms and no gating predicate moves. | The ordering was an unwritten implementation detail of `BetComposer`, and an unwritten precedence is one a later task reorders without knowing it was a decision. The constant height is the property the slot exists for, so it is stated as a requirement rather than left as an artefact of the height that happened to be reserved. | — |
| 2026-08-26 | 1.0.44 | §9 F-DEBATE-1 | **RPLY-CLOSE — the `?post=` bullet gains a CARVE-OUT on its own user-mintable claim.** The bullet asserts that in-view focus mirrors to the URL, "making deep links user-mintable". A removed post's `Open debate →` mints a `?post=<N>` the resolver then refuses (removed-targeting values are one of the five silent-fallback cases named in the same sentence), so the general claim does not hold in that one state. Recorded as an EXCEPTION rather than by weakening the claim, because the claim is true everywhere else and the asymmetry runs in the safe direction — the URL fails closed to the market arm, never to an error surface. **No code change.** ⚠ v1.0 of this amendment cited a non-existent "UI.A2 §3.4"; UI.A2 is a plan/log (`docs/plans/UI-A2.md`, `docs/logs/UI-A2.md`) and has no spec section — the governing text is this bullet, landed at 1.0.16. | A spec that states a general property and is silently false in one reachable state is worse than one that states the exception: the reader who hits the dead link has no way to tell a bug from a boundary. Naming it makes the fallback legible as a design, and records that the no-code-change reading was deliberate rather than deferred. | — |
...
A vanity / traction counter of **total page views — repeat views by the same person counted** — displayed on the product. It is **explicitly not `n`** — not a participant count, not staked, not in K_eff. It reads nothing from the ledger and feeds nothing into the engine.

- **Anti-conflation (hard).** Labelled plainly as **"views"** and **never co-located** with any participant / stake / Dharma count. The risk is narrative, not technical: a traffic number shown as if it were participation muddies the K × n > C story. The label is **"views"** — not "visitors", not "visits". The counter increments once per page load, so under the standard analytics taxonomy the number it produces is page views: "visitors" overstated it by roughly the average pages-per-session, and "visits" would name a session metric this mechanism does not produce and cannot produce without persisting a session identifier it deliberately does not hold.
...
**Balance — spendable today.** The ledger truth at the viewer's latest `seq` plus the day's `DAILY_CREDIT_DHARMA` where unclaimed, because the place path pays the day's credit before the affordability check. This is **deliberately distinct** from the §23 Wallet-value tile, which renders the raw ledger figure: a participant holding Đ 45 raw who has not bet today can spend Đ 55, and a header showing Đ 45 would tell them they are dead-ended under the Đ 50 reply floor. Both figures are true and they measure different things — ledger state versus committable capacity. The distinct labels carry the distinction; no reconciliation is attempted.

**Placement is load-bearing.** Both stats sit **left** of the §21.1 register boundary; the identity chip separates the cluster from the divider; the visitor counter remains the **sole** element to the divider's right. §21.1's anti-conflation is hard, and a Dharma figure filed in the same visual bucket as a page-hit count violates it silently. A later layout pass must not collapse them adjacent.
...
clause wraps, subject to fitting the viewport; where those
...
**The read model (SCL-4).** Discovery lists **open markets** (`market.state = Open`). Each market renders as a **card** with the locked composition (design-language §3.2): the market **image** + the market **question** + the **YES/NO price bar** + the stat line **`Đ staked · posts · replies`**. Every quantity is a read-model derivation over existing data (pool reserves for price, the bet ledger for `Đ staked`, the comment tree for post/reply counts) — **no new tables, no new event types**. The card **image** is the market's **`is_default` `market_media` row** (ADR-0026); because markets always carry media (the §15 F-ADMIN-1 service invariant), a card image **always exists** — this reconciles **SCL-2** (the deferred "market image field") to the existing `market_media` model, adding **no `markets` column**.

⚠ Amended at HTML-FINISH · DISCOVERY (2026-08-15): the card no longer carries a two-line price sparkline. This section's own *Market price history* text defines the card sparkline as DECORATIVE — no axis, `aria-hidden`, X spaced by event index — and composition of decoration is the ratified mockup's jurisdiction (`docs/design/mockups/surface_discovery_v1_0.html`, founder-ratified 2026-08-15 as layout authority; its `.mcard` is title-block + price bar with nothing between, `:269-282`, and its `.spark` rule at `:155` styles no element in the document). ⚠ THE READ MODEL IS UNCHANGED: the hero renders the full-size two-line graph for the active carousel market and `DiscoveryMarketView.series` still carries the series. No field, no query and no cache behaviour changed.

**Price series (no new store).** The two-line price graph (YES/NO, design-language §3.2 — full-size on the hero; **not rendered on cards**, per the HTML-FINISH amendment above) is a **read-model derivation**: the pool YES-price sampled across the market's `bet.placed` / `bet.sold` events. There is **no materialized price-series table** in v1; the cached-per-load model (below) amortizes the replay across the cache lifetime. A stored series is explicitly out of scope for the experiment (§18-class deferral).
...
- **System.** The read model selects open markets (`market.state = Open`) ordered by `markets.created_at` **descending**, capped at `DISCOVERY_GRID_SIZE`. Each selected market becomes a **card**: image (the market's `is_default` `market_media` row), question, YES/NO price bar (pool-derived), and `Đ staked · posts · replies` (read-model aggregates) — **no sparkline** (HTML-FINISH amendment, read model above). Fewer than `DISCOVERY_GRID_SIZE` markets → render only the available cards (no placeholders); zero markets → empty state (no grid). Served **cached per-load** (SPEC.2 R-2); the carousel rotates the loaded set **client-side** (design-canon §5) with no re-fetch. A card click navigates to that market's detail view (`/m/[slug]`, §9).
...
**What Profile is.** The public-by-default permanent record of one participant, served at **`/u/[pseudonym]`** inside the ADR-0023 `(public)/` shell. Anonymous and authenticated visitors see the same page (D7): the **identity block** (PFP + pseudonym; `Banned` label when `users.banned_at` is set, per D8), the **six account tiles**, the cross-market **positions table**, and the **argument list**. The owner additionally sees **Sell affordances** (F-PROF-3) — the only viewer-dependent payload delta besides the parked Daily-Credit history (below). The admin has no `users` row and therefore no profile — structurally (B5, F-AUTH-ADMIN).
...
- **Net profit/loss** — **lifetime**: net worth − Σ issuance, where issuance = the user's `initial_grant` + `daily_allowance` ledger amounts. (Identity: net worth = Wallet value + Positions value. A user who never bets shows 0; the tile is the thesis-relevant track record.)
...
DISCOVERY_SERIES_MAX_POINTS = 64  # pinned design value (§22 Discovery HERO sparkline; decorative); not deferred to tuning
OTP_TTL_MIN = TBD
