...
⚠ **Nothing priced rides the window.** Every figure a surface renders that is a function of the live pool — the price bar, `unitToWin`, the chart's terminal point on an `Open` market, and the hero panel's `Đ staked → Đ now` value — is composed **outside** the block from a live pool read the page already performs, at no additional query. A Đ amount on a public surface is never served from a window.
