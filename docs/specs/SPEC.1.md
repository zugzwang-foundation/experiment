...
- **Version:** 1.0.6 (semver; bump major on invariant changes)
...
| **B** | Graphic violence; threats; hate speech; harassment | Hide from public; queue for admin: **Approve** / **Discard** / **Block** (ADR-0020 — content removal decoupled from user ban) | Yes |
...
**Content removal and user ban are decoupled (ADR-0020).** Across both the held queue (Track B) and reactive removal of live (Track C) content, hiding a comment from public view and banning its author are independent actions — either may be applied without the other. Held (Track B) items offer **Approve / Discard / Block**; live (Track C) content the admin catches as a false-negative offers **Remove / Ban**. Every action — including a no-ban removal and a Discard — writes an append-only `mod_actions` row carrying a reason code that distinguishes content-removal from user-ban (§16.4); there is no silent suppression. No action touches the bet or ledger (INV-1 / INV-2 / INV-3): a banned or removed author's positions ride to resolution — **ban removes voice, not balance**. The pre-commit gate, vendor, and verdict mapping (ADR-0014) are unchanged; ADR-0020 refines only the admin consequence actions downstream of the verdict.
...
- **System.** Held content hidden from all public surfaces. Author sees `Comment under review` on their own profile only. The admin reviews held items in the moderation feed at `/admin/moderation` — real-time, market-filterable, with a pinned held-count and an oldest-first held-only view (the decision queue), per §15.2. Each held row shows the comment, AI flag categories with confidence scores, user pseudonym, user Dharma, user prior-flag count. Held actions are Approve / Discard / Block (ADR-0020 / F-MOD-3).
- **Acceptance.** `tests/server/moderation/track-b.test.ts::queue-and-visibility`.

### F-MOD-3 — Admin held-item decision (Approve / Discard / Block)

- **System.** Three actions on a held (Track B) item, per ADR-0020 — content removal is decoupled from user ban:
  - **Approve** — the argument is published and the staked bet settles. *(The held-bet execution model — post-time vs approve-time price — is owned by the DEBATE.7 plan.)*
  - **Discard** — content stays hidden, author untouched, no ban. Because a Track B verdict never opens the bet+comment transaction (ADR-0014), Discard closes the held `mod_actions` row: no comment row, no bet, no stake committed.
  - **Block** — content stays hidden and the author is banned (Track A mechanics). No appeal.
  The prior "no middle option" stance is replaced by **Discard** (the no-ban content removal). Each action writes an append-only `mod_actions` row with a reason code distinguishing content-removal from user-ban; **Discard is a decision, not a no-op**. Two surfaces, same backend: the hub feed at `/admin/moderation` and the inline affordance on debate views (F-ADMIN-4). Both call the same endpoint, write the same `mod_actions` shape, and apply the same audit discipline.
- **Acceptance.** Three-option held-decision coverage (Approve / Discard / Block) is minted in the DEBATE.7 plan (§17 moderation:: rows), which owns the consequence wiring.
...
- **Inline = content-level remediation.** Specifically: removing a comment with or without banning its author (Remove / Ban, ADR-0020), and approving or discarding a Track B held comment. Removal now includes live (Track C) content the admin catches as a false-negative — not only Track A/B. These are small, targeted, comment-scoped actions where the admin is already reading the comment in context.
...
The exact set of inline admin affordances on a market page is: on a visible comment, **Remove** or **Ban** (content removal vs content-removal-plus-ban, ADR-0020); on a Track B held comment, **Approve** or **Discard**. **Nothing else.** No "trigger resolution" button on the market page even when state=Closed. No "void market" button. No bulk-action selector. Standalone user-record banning not tied to a specific comment action remains hub-only. Those route the admin to the hub.
...
The `/admin` route lands on a dashboard surfacing what needs the admin's attention, organised around the admin's two in-experiment pillars — **moderation** and **resolution** — plus operational tripwires:

1. **Moderate (attention zone).** The live held-count with age-of-oldest; click-through to the held-only view of the moderation feed (§15.2 feed / F-ADMIN-4).
2. **Resolve (attention zone).** Market-state counts (`Draft / Open / Closed / Resolving / Resolved / Voided`), with **`Closed` broken out as the action item** ("N markets awaiting resolution") and click-through to the Needs-resolution view in the Markets tab. As 2026-11-05 approaches, a **freeze countdown** surfaces here and becomes prominent — every `Closed` market must be Resolved or Voided before 23:59 UTC or it freezes unresolved.
3. **Identity-pool depth (tripwire).** Current `identity_pool` unassigned count as a percentage of total; alarm red below 5% (a depleted pool silently halts signups via F-AUTH-3).

The signup-rate *trend* and the suspicious-pattern-indicator widget from the prior four-widget design are **dropped / deferred**: the signup trend is telemetry that drifts toward analytics (the analytics deliverable is the Nov 6 public dataset, not an in-product surface), and the suspicious-pattern widget (velocity outliers, same-IP clustering — heuristic, no automated bans) is build-heavy moderation *support* not required for a sole-admin v1; reactive investigation runs through the audit-search surface (F-ADMIN-5). Either may be revisited later.

**Real-time reach.** The two attention counters (held-count, needs-resolution-count) are **live**, because staleness there actively misleads. The remaining indicators (pool depth) and the rest of the hub follow the platform's polled-on-view default (per `C7`). This is a deliberate, scoped exception to the prior "no live websocket" rule — real-time where staleness misleads, polled where it does not.
...
- **Markets** (`/admin/markets`) — market list with state filters + a pinned **Needs-resolution** view (`Closed` markets awaiting resolution, with the freeze countdown on the final day); entry point to F-ADMIN-1, F-ADMIN-2, F-ADMIN-3. *(Market creation / pool-seed — F-ADMIN-1/2 — is the pre-live market-maker workflow; the in-experiment Markets work is resolution and void.)*
- **Moderation** (`/admin/moderation`) — the real-time, market-filterable moderation feed (F-ADMIN-4) and audit log search (F-ADMIN-5).
...
- **Confirmation.** The resolution evidence note is mandatory — the action cannot be submitted without it; the note is immutable and ships in the Nov 6 dataset — and a single restate-and-confirm names the winning side and that the action is permanent (corrections only via F-RESOLVE-2 clawback; no re-open, edit, or un-resolve). Before commit, a **dry-run consequence preview** shows the impact — winning bets to be paid, total payout, pool-unwind amount — without committing; this requires a compute-without-commit path additional to the built `resolveMarketAction` (build item, additive). The same preview applies to F-RESOLVE-2 corrections (Markets tab), which additionally display the resolution-event lineage — the append-only chain of corrections.
...
### F-ADMIN-4 — Moderation actions (Hub: real-time feed; Inline: market pages)

- **Pre.** A post or reply exists in one of three states (derived from its OpenAI track): **Live** (Track C, posted), **Held** (Track B, `review_status = pending`, hidden), or **Auto-actioned** (Track A, blocked). Per ADR-0020, content removal and user ban are decoupled.
- **System.** The admin works in a single **real-time, market-filterable feed** at `/admin/moderation` showing every post and reply, with a state filter and a pinned held-count (`N held · oldest Xh`) so held decisions never scroll out of reach; the held-only view is the oldest-first decision queue. Each row shows the content (text + image), post-vs-reply (replies carry a collapsed parent snippet), market, side, author pseudonym, Dharma, prior-flag count, timestamp, state badge, and — for held / auto-actioned items — AI flag categories with confidence. Actions are state-specific:
  - **Held (Track B): Approve / Discard / Block** (per F-MOD-3). Approve publishes + settles the bet; Discard keeps hidden, no ban; Block keeps hidden + bans.
  - **Live (Track C): Remove / Ban.** Remove hides the already-public comment, author untouched, the author's bet rides to resolution (replies under it remain — other users' stake-backed arguments — and the parent renders a "removed by moderator" placeholder, thread intact). Ban additionally bans the author. This is the false-negative coverage role: reactive removal extends to Track-C content, not only Track A/B.
...
- **Response.** Comment visibility and/or author account-state updated; an append-only `mod_actions` row written for **every** action (Approve, Discard, Block, Remove, Ban), with a reason code distinguishing content-removal from user-ban; on a ban, `users.banned_at` set. No action touches the bet or ledger (INV-1 / INV-2 / INV-3); ban removes voice, not balance.
- **Surface — hub.** The real-time feed above is the primary moderation workspace. Rapid per-item triage (including keyboard-driven action) is supported; **each action remains a single, explicitly-chosen, individually-audited per-comment decision — this is not bulk-action or multi-select**, which remain out of scope (one comment, one decision, one `mod_actions` row).
- **Surface — inline.** When an authenticated admin views `/markets/<id>` or any debate view, the same actions render inline (admin-only): Remove / Ban on a visible comment, Approve / Discard on a Track B held comment (with a `pending review` badge). Same backend endpoints as the hub. **The server independently verifies a valid `admin_sessions` row on every request — the inline affordance is a UI convenience, not a privilege path.**
- **Inline scope explicitly:** on a visible comment, Remove or Ban; on a Track B held comment, Approve or Discard. **Nothing else.** No "trigger resolution" button on the market header. No "void market" button. No bulk-select. Standalone user-record banning not tied to a specific comment action is hub-only.
- **Acceptance.** Moderation acceptance coverage (the three-option held queue, the Remove/Ban decoupling, the Track-C reactive-removal scope, per-action `mod_actions` audit, and the inline parity above) is reconciled and minted in the **DEBATE.7 plan** (§17 moderation:: rows), which owns the consequence wiring these actions invoke.
