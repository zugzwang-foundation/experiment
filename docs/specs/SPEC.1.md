...
- **Version:** 1.0.11 (semver; bump major on invariant changes)
...
- **Errors.** 400 `insufficient_dharma`, 400 `error_market_closed_at`, 400 `comment_too_long`, 400 `comment_track_a_blocked`, 400 `comment_track_b_blocked`, 409 `market_resolving`, 403 `banned_user`.
...
1. **Moderate (attention zone).** Click-through to the **reactive review feed** — the live stream of passed (Track C) content the admin reviews post-publication (§15.3 Moderation tab / F-ADMIN-4). There is **no held-count** (nothing is held; ADR-0021); the feed's delivery mechanism and any unreviewed-since-last-visit indicator are owned by the admin-dashboard stratum.
...
**Real-time reach.** The needs-resolution counter is **live**, because staleness there actively misleads as the freeze approaches. The remaining indicators (pool depth) and the rest of the hub follow the platform's polled-on-view default (per `C7`). The reactive review feed's delivery mechanism (live vs polled) is an admin-dashboard-stratum decision (ADR-0021); the held-count exception is removed (no held queue). This remains a deliberate, scoped exception to the prior "no live websocket" rule — real-time where staleness misleads, polled where it does not.
...
- **Moderation** (`/admin/moderation`) — the market-filterable **reactive review feed** of live (passed) content (F-ADMIN-4) and audit log search (F-ADMIN-5).
...
- **System.** The admin works in a **market-filterable reactive review feed** at `/admin/moderation` showing live (passed) posts and replies in **chronological order — no pre-publication step, no admin-side filter beyond market, no ranking** (ADR-0021: the admin reviews the stream by eye). Each row shows the content (text + image), post-vs-reply (replies carry a collapsed parent snippet), market, side, author pseudonym, Dharma, prior-flag count, timestamp, and state badge. Actions are state-specific:
