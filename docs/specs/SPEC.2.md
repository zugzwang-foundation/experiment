...
> **Status:** 1.0.23 · v1.0 locked at PRECURSOR.4 (2026-06-03, fresh-session writer/reviewer review per CLAUDE.md) · §0–§23 + Appendices A–B complete · backend-complete as of the BC (Backend-Consolidation) sweep (2026-07-01): ENGINE .0–.16, DEBATE incl. .4 render (the DEBATE.4 tracker stratum — the public render; NOT SPEC.1 §9's F-DEBATE-4 polling flow, which is a separate row), EXPORT.1, MEDIA.1 all landed on `main` · migration head 0024, ADRs 0003–0038 (36) folded (current through SYNC-4, 2026-08-20; ADR-0033 is index-row only — its substance is folded into neither spec) · ⚠ **the on-disk ADR ceiling is `0038`** (re-measured at SYNC-4, 2026-08-20 by `ls docs/adr/`, superseding the SYNC-3 reading of `0037`): **`ADR-0035`, `ADR-0036`, `ADR-0037` and `ADR-0038` are now folded into BOTH this §0 count AND the §22.1 index — the `N5` backlog is discharged.** ⚠ **Two successive versions of this annotation went stale by one within eleven days of being written**, which is why SYNC-4 rebuilt the index normatively instead of continuing to correct it in place. **Read `ls docs/adr/` (`O-2`); never take a ceiling from this line.** · ADR-0009 superseded-by-0017 · **reply-as-bet model** (every comment rides a bet; friendly-fire + `friendly_fire_events` + `stake_at_post_time` removed entirely; Support/Counter are read-time aggregates over reply-bets) consistent with SPEC.1 (1.0.25) · K_eff dashboard struck per PRECURSOR.2-B D4 (no live in-product surface; K_eff(t) derived post-hoc from the 2026-11-06 public dataset per SPEC.1 G3 + §12.2)
> **Repo path:** `zugzwang-foundation/experiment/docs/specs/SPEC.2.md`
> **Companion files:** `SPEC.1.md` (product), `cpmm.md` (math), `RANKING.md` (ranking function), `PSEUDONYM.md` (pseudonym pool spec), `design.md` (visual system) — `cpmm.md` and `RANKING.md` are on disk at `docs/specs/`; `PSEUDONYM.md` and `design.md` remain pending on their gating tasks (see §1.4); 36 ADRs at `docs/adr/0003–0038` (full index at §22.1)
...
| **Gates downstream** | 36 ADRs (`ADR-0003` through `ADR-0038`; see §22.1 for per-ADR task attribution) + all `SCAFFOLD.*`, `ENGINE.*`, `DEBATE.*`, `DESIGN.*`, `EXPORT.*`, `MEDIA.*`, `TESTING.*`, `HARDEN.*` tracker tasks |
...
Twenty-five tables in v1 across eleven domains. Ten strictly append-only (Bucket A); three append-only with one whitelisted column transition (Bucket B); twelve mutable with no append-only trigger (Bucket C). Total protected by §6's append-only enforcement contract: thirteen.
...
| **C** — mutable | 12 | No append-only trigger (constraint-driven validation only) | `users`, `markets`, `pools`, `positions`, `sessions`, `accounts`, `verifications`, `admin_sessions`, `watermark_state`, `cron_alarms`, `market_media`, `bookmarks` |
...
**Per-domain schema-file split.** Tables are grouped into domain files at `src/db/schema/<domain>.ts` per ADR-0008 §4 with a barrel re-export at `src/db/schema/index.ts`. Eleven domains in v1: `auth`, `markets`, `bets`, `bookmarks`, `comments`, `dharma`, `events`, `identity`, `image-uploads`, `audit`, `system`. The `auth` domain spans two ADR ownerships (ADR-0004 for the four Better Auth tables; ADR-0010 for `admin_sessions`); both groups share the same schema file because they share the auth surface conceptually.
...
| Concern | Source-of-truth file |
|---|---|
| Per-ADR substance | `docs/adr/<NNNN>-<slug>.md` (37 ADRs — 36 ADR files + ADR-0012 in-flight; 34 accepted + 2 superseded + 1 in-flight; ADR-0001 admitted at BC.2; ADR-0034 folded at BOOKMARK-ADD-WIRE; ADR-0035/0036/0037/0038 folded at SYNC-4; ADR-0033 index-row only — its substance is folded into neither spec, a docketed follow-up) |
