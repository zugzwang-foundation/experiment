...
*Reset at 2.0.0 (D-30). The `v0.1-outline`–`1.0.27` log is in git history; last 1.0.x commit `073f65ea`.*
...
5. Moderation dispatch        — per §10 (every comment-bearing bet; after the transaction commits)
...
**Ban is request-time enforcement, not logout.** A banned participant's `sessions` row is NOT deleted at the moment of ban; the ban is enforced at the next request via `users.banned_at IS NOT NULL` check at the Server Action / Route Handler entry. This is deliberate — pre-ban audit trail is preserved, and the ban-enforcement check rides on the same `auth.api.getSession` call that already runs at every handler entry. Track A automatic ban (per ADR-0014 — superseded by ADR-0046 + SPEC.1 §14 F-MOD-1) and Track B admin manual ban (per F-ADMIN-4) both write `users.banned_at`; neither deletes `sessions` rows.
...
5. Moderation dispatch (per §10 — after the transaction commits, off the request path)
...
Six-step orchestration consuming §10 + §11 + §12 jointly. The R2 object exists from step 3 onward regardless of the screening outcome; the `image_uploads` row tracks committed, orphaned, and dropped.
...
| `target_market_id` | uuid \| null | SHIP | FK to `markets.id` — the market the flagged submission targeted (a gate-block has no comment/bet row to JOIN); powers F-ADMIN-5 market search + the dashboard market filter (DEBATE.7). NULL when a comment/bet target is present |
...
| `actor_id` | text | SHIP | `'system'` for every classifier-written row (Track A autoban, Track B block, the `sexual_minors_text_blocked` carve-out); `'admin-singleton'` for reactive Remove/Ban (dashboard stratum) |
| `created_at` | timestamptz | SHIP | |
