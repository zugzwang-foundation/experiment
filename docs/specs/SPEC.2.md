...
|---|---|---|---|
| 2.2.2 | 2026-09-25 | HMH | **ADR-0060 — production readiness on AWS (AWS-MIGRATION-3).** §15.4 gains two codes: `error_bet_timeout` (503, a `statement_timeout` inside the bet transaction, rolled back, not retried server-side, client retry replays the durable receipt) and `error_writes_paused` (503, every write while the cutover write-pause is on). Catalogue 39 → 41. Readiness (`/api/ready`) becomes the load-balancer health check beside `/api/health`; the task carries a bounded V8 heap and a keep-alive above the ALB idle timeout; one task with a hard maximum of one until a fleet-wide cache invalidation exists. No schema change. *(refs: ADR-0060; AWS-MIGRATION-3)* |
| 2.2.1 | 2026-09-24 | HMH | **ADR-0059 — RDS for PostgreSQL replaces Supabase as the database host.** Partial supersession of ADR-0006 (vendor) and ADR-0024 (#1 two Supabase projects, #8 pooler mode); everything else in both inherited. Infrastructure only — `infra/lib/database-stack.ts`, isolated DB subnets and a group-referenced 5432 rule in `network-stack.ts`, a parameter group preloading `pg_cron` for the named database. **No schema change, no application change**; §6's migration set and append-only contract travel unchanged, and the migration guards (ADR-0022) keep their code — only the ref-fragment VALUE moves to the RDS endpoint at cutover. ⚠ Every Supabase reference in §4 / §21 describes the estate **until cutover**; they are not corrected here because they are still true, and `docs/reports/AWS-MIGRATION-REPORT.html` §09 owns the sequence. *(refs: ADR-0059; AWS-MIGRATION-1)* |
...
| `retry_after` | Client MUST wait at least `retry_after` seconds before retrying. Codes: `error_rate_limit_exceeded`, `error_idempotency_in_flight`, `error_idempotency_unavailable`, `error_bet_serialization_exhausted`, `error_bet_timeout`, `error_writes_paused`. |
...
| **ADR-0060 / AWS-MIGRATION-3** (production readiness on AWS) | 2 | `error_bet_timeout` (HTTP 503, `error_type: unavailable`, `retry_semantics: retry_after`, `retry_after: 2` — a `statement_timeout` (SQLSTATE 57014) inside the bet transaction, rolled back, not retried server-side; the client's retry replays the durable receipt), `error_writes_paused` (HTTP 503, `error_type: unavailable`, `retry_semantics: retry_after` — every write while `ZUGZWANG_WRITES_PAUSED=paused`, answered by `proxy.ts` before any reservation) |
| **Total** | **41** | |
