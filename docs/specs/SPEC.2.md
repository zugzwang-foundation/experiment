...
| **Gates downstream** | 30 ADRs (`ADR-0003` through `ADR-0032`; see §22.1 for per-ADR task attribution) + all `SCAFFOLD.*`, `ENGINE.*`, `DEBATE.*`, `DESIGN.*`, `EXPORT.*`, `MEDIA.*`, `TESTING.*`, `HARDEN.*` tracker tasks |
...
| `addBookmarkAction(commentId)` | F-BM | `src/server/bookmarks/add.ts` | No A6 surface — the add-icon on the debate view (`ArgProfile`) + other-user Profile cards is the named follow-on **BOOKMARK-ADD-WIRE** (§11); the action + its F-BM-1 tests land at A6 so the wiring invents no logic | F-BM-1 |
...
| Concern | Source-of-truth file |
|---|---|
| Per-ADR substance | `docs/adr/<NNNN>-<slug>.md` (31 ADRs — 30 ADR files + ADR-0012 in-flight; 28 accepted + 2 superseded + 1 in-flight; ADR-0001 admitted at BC.2; ADR-0032 folded at UI-A6) |
