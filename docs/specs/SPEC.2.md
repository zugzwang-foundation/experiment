...
6. **Branch on the image verdict** — the post is already live either way:
   - **`pass`** — the image publishes with the post; UPDATE `image_uploads` SET `terminal_state = 'committed'`.
   - **`track_a` / `track_b`** — write a `mod_actions` row in a standalone transaction; UPDATE `image_uploads` SET `terminal_state = 'blocked'` (the shipped enum value; renamed at MOD-1). The bet+comment transaction has already committed; the post is live and the image is not served.
