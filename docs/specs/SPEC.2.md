...
> **Status:** 1.0.22 · v1.0 locked at PRECURSOR.4 (2026-06-03, fresh-session writer/reviewer review per CLAUDE.md) · §0–§23 + Appendices A–B complete · backend-complete as of the BC (Backend-Consolidation) sweep (2026-07-01): ENGINE .0–.16, DEBATE incl. .4 render (the DEBATE.4 tracker stratum — the public render; NOT SPEC.1 §9's F-DEBATE-4 polling flow, which is a separate row), EXPORT.1, MEDIA.1 all landed on `main` · migration head 0024, ADRs 0003–0034 (32) folded (current through BOOKMARK-ADD-WIRE, 2026-07-30; ADR-0033 is index-row only — its substance is folded into neither spec) · ⚠ **the on-disk ADR ceiling is `0037`, not `0034`** (re-measured at SYNC-3, 2026-08-19 by `ls docs/adr/`, superseding the SYNC-1 reading of `0036`): **`ADR-0035`, `ADR-0036` and `ADR-0037` exist on `main` and are folded into NEITHER this §0 count NOR the §22.1 index.** ⚠ **The count is now THREE, and this annotation was itself stale by one within eleven days of being written** — which is the argument for `N5` being a normative index rebuild rather than a running correction. `SPEC.1` already cites ADR-0037; this file does not name it anywhere. **Read `ls docs/adr/` (`O-2`); never take a ceiling from this line.** · ADR-0009 superseded-by-0017 · **reply-as-bet model** (every comment rides a bet; friendly-fire + `friendly_fire_events` + `stake_at_post_time` removed entirely; Support/Counter are read-time aggregates over reply-bets) consistent with SPEC.1 (1.0.25) · K_eff dashboard struck per PRECURSOR.2-B D4 (no live in-product surface; K_eff(t) derived post-hoc from the 2026-11-06 public dataset per SPEC.1 G3 + §12.2)
> **Repo path:** `zugzwang-foundation/experiment/docs/specs/SPEC.2.md`
> **Companion files:** `SPEC.1.md` (product), `cpmm.md` (math), `RANKING.md` (ranking function), `PSEUDONYM.md` (pseudonym pool spec), `design.md` (visual system) — `cpmm.md` and `RANKING.md` are on disk at `docs/specs/`; `PSEUDONYM.md` and `design.md` remain pending on their gating tasks (see §1.4); 32 ADRs at `docs/adr/0003–0034` (full index at §22.1)
...
| **Gates downstream** | 32 ADRs (`ADR-0003` through `ADR-0034`; see §22.1 for per-ADR task attribution) + all `SCAFFOLD.*`, `ENGINE.*`, `DEBATE.*`, `DESIGN.*`, `EXPORT.*`, `MEDIA.*`, `TESTING.*`, `HARDEN.*` tracker tasks |
...
| Concern | Source-of-truth file |
|---|---|
| Per-ADR substance | `docs/adr/<NNNN>-<slug>.md` (33 ADRs — 32 ADR files + ADR-0012 in-flight; 30 accepted + 2 superseded + 1 in-flight; ADR-0001 admitted at BC.2; ADR-0034 folded at BOOKMARK-ADD-WIRE; ADR-0033 index-row only — its substance is folded into neither spec, a docketed follow-up) |
