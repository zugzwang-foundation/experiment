...
| 1.0.28 | 2026-09-06 | HMH | **LIQ-1 / ADR-0047 registry row — index only, no substantive amendment here.** **§22.1** gains the ADR-0047 row (asymmetric market open + signup-pegged liquidity injection), and the section carries a new standing annotation that the index is **seven rows behind disk**: `ADR-0040` through `ADR-0046` exist at `docs/adr/`, are cited elsewhere in this document, and have no row in §22.1. That backlog is **named, not paid** — folding seven ADRs is a SYNC sweep, because each row is a reading of its own ADR, and doing it as a rider on an unrelated commit is how the `N5` backlog formed the first two times. The heading count (`38`), the §22 inventory prose and §22.5 are therefore **left stating what they have always stated — the number of rows listed here** — with the annotation pointing every reader at `ls docs/adr/` (`O-2`) for the live inventory: 45 files, ceiling `0047`, `0002`/`0012` unused, next free `0048` (measured 2026-09-06). ⚠ Restating a corrected total in three count sites would have asserted per-ADR statuses for 0040–0046 that this pass did not read, which is the failure the annotation exists to prevent. **ADR-0047's substantive spec amendments do not land here**: `cpmm.md` §7.1/§7.3 ride its Phase 1 PR and `cpmm.md` §7.4/§14 + SPEC.1 §10.6/§10.5/§16.1 ride Phase 2, each same-commit with its code per CLAUDE.md §5.12. **§0** version → 1.0.28, date → 2026-09-06. |
| 1.0.29 | 2026-09-06 | HMH | **LIQ-1 Phase 1 (ADR-0047) — §19.4.1 same-commit rider for the extended `market.opened` payload.** The row's SHIP declaration now covers BOTH payload variants: the legacy symmetric `{marketId, seedAmount}` and the asymmetric `{marketId, yesReserves, noReserves, openingPriceYes, backingMinted, discardedYes, discardedNo}` that `openMarket` emits from this phase forward. No STRIP target is added — none of the six new keys is PII-class; they are public CPMM state. Rides the schema change (`src/server/events/schemas.ts`, a `z.union` of the two variants — the legacy arm is kept because every historical row and every fixture carries it) per §19.4.1's own amendment rule, and lands in Phase 1 rather than Phase 2 (ADR-0047 §F, corrected same-day) because the export rule keys on the PAYLOAD SHAPE, not on the event type — the shape ships now, so its declaration must too. `EVENT_TYPES` is unchanged at 24; `pool.liquidity_added` and its own row are Phase 2. No migration: `event_type` is `text` and `payload` is `jsonb`. |
...
⚠ **This index is SEVEN rows behind the files on disk, and the heading's `38` counts only what is listed here.** `ADR-0040` through `ADR-0046` exist at `docs/adr/`, are cited elsewhere in this document, and have **no row below** — the `N5`-shaped backlog this section has run twice before. ADR-0047 is added here (LIQ-1, 2026-09-06) rather than waiting for that backlog to clear, because its Phase 1 amends `cpmm.md` in the same commit and an ADR that amends a spec must be findable from the index that spec's readers use. **The canonical inventory is `ls docs/adr/` (`O-2`), never the count in a heading** — measured 2026-09-06: 45 files, ceiling `0047`, `0002` and `0012` unused, next free `0048`. Folding 0040–0046 is a SYNC sweep, not a rider: each row is a reading of its own ADR.
...
| **0047** | LIQ-1 | `0047-asymmetric-open-and-signup-pegged-liquidity.md` | Asymmetric market open + signup-pegged liquidity injection — a constant-product market's depth is fixed at seed while participant flow is not, so `flow ÷ depth` climbs for the whole window and the closing price measures the crowd's size rather than the question (measured: 0.787 at 5,000 signups/hour, 0.994 at 40,000, same market). Two decisions on one primitive **A** — `addCpmmLiquidityFixedP` lifted with attribution from the Manifold reference, `L' = L + a`, `S' = S + a·(S/L)`, price-preserving to 18 dp and the excess on the short side **discarded**: (i) **asymmetric open**, `openMarket` takes `openingPriceYes` + `tank` and writes two reserves (90,000 / 10,000 for D-14's 10% YES), retiring the operator-account slate that was the only other way to reach a non-50% open — no operator positions, no arguments the operator does not hold; (ii) a `pg_cron` **injector** (Phase 2) sizing depth to `max(FLOOR, COEFF × count(*) FROM users)`, parameters in an append-only `liquidity_policy` table so tuning is one INSERT and never a deploy. Mints the **backing identity** `Y + H_yes + D_yes == N + H_no + D_no == total Đ deposited`, where `D` is cumulative discards summed from events — today `D = 0`, which is exactly why `voidMarket` throws on any asymmetric seed and `settleMarket` under-reports the NO-outcome residual by `D_no`. `k` stops being preserved across a market's life (it was never asserted constant — INV-C2 is `k′ ≥ k`) and becomes: `k` changes only through a named door. The bet transaction is **not** modified. Amends `cpmm.md` §7.1/§7.3 (Phase 1) and §7.4/§14 + SPEC.1 §10.6/§10.5/§16.1 (Phase 2); ships in two PRs, 8 and 12 September | accepted | 2026-09-06 |
