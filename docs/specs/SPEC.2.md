...
> **Status:** version is in the §0 table below and in the last changelog row; this line deliberately carries no number (`O-15`) · v1.0 locked at PRECURSOR.4 (2026-06-03, fresh-session writer/reviewer review per CLAUDE.md) · §0–§23 + Appendices A–B complete · backend-complete as of the BC (Backend-Consolidation) sweep (2026-07-01): ENGINE .0–.16, DEBATE incl. .4 render (the DEBATE.4 tracker stratum — the public render; NOT SPEC.1 §9's F-DEBATE-4 polling flow, which is a separate row), EXPORT.1, MEDIA.1 all landed on `main` · migration head 0026, ADRs 0003–0039 (37) folded (current through PHASE-0, 2026-08-21; ADR-0033 is index-row only — its substance is folded into neither spec) · ⚠ **the on-disk ADR ceiling is `0039`** (re-measured at PHASE-0, 2026-08-21 by `ls docs/adr/`, superseding the SYNC-4 reading of `0038`, which superseded the SYNC-3 reading of `0037`): **`ADR-0035`, `ADR-0036`, `ADR-0037` and `ADR-0038` were folded into BOTH this §0 count AND the §22.1 index at SYNC-4 — the `N5` backlog is discharged — and `ADR-0039` (per-argument lot accounting) joins both at PHASE-0.** ⚠ **THREE successive versions of this annotation have now gone stale by one, the first two within eleven days of being written**, which is why SYNC-4 rebuilt the index normatively instead of continuing to correct it in place — and why the count is quoted here only as of a named task, never as a standing fact. **Read `ls docs/adr/` (`O-2`); never take a ceiling from this line.** · ADR-0009 superseded-by-0017 · **reply-as-bet model** (every comment rides a bet; friendly-fire + `friendly_fire_events` + `stake_at_post_time` removed entirely; Support/Counter are read-time aggregates over reply-bets) consistent with SPEC.1 (1.0.25) · K_eff dashboard struck per PRECURSOR.2-B D4 (no live in-product surface; K_eff(t) derived post-hoc from the 2026-11-06 public dataset per SPEC.1 G3 + §12.2)
> **Repo path:** `zugzwang-foundation/experiment/docs/specs/SPEC.2.md`
> **Companion files:** `SPEC.1.md` (product), `cpmm.md` (math), `RANKING.md` (ranking function), `PSEUDONYM.md` (pseudonym pool spec), `design.md` (visual system) — `cpmm.md` and `RANKING.md` are on disk at `docs/specs/`; `PSEUDONYM.md` and `design.md` remain pending on their gating tasks (see §1.4); 37 ADRs at `docs/adr/0003–0039` (full index at §22.1)
...
| **Gates downstream** | 37 ADRs (`ADR-0003` through `ADR-0039`; see §22.1 for per-ADR task attribution) + all `SCAFFOLD.*`, `ENGINE.*`, `DEBATE.*`, `DESIGN.*`, `EXPORT.*`, `MEDIA.*`, `TESTING.*`, `HARDEN.*` tracker tasks |
...
| 1.0.5 | 2026-06-13 | HMH | **ENGINE.15 riders R-15-A..R-15-D — HTTP/cron/admin wiring stratum same-commit amendments** (docs/plans/ENGINE.15.md, founder-ratified 2026-06-13; paired with SPEC.1 → 1.0.5, riders R-15-E + R-15-G). **§4.2** (R-15-A): Server-Action names → `*Action`; resolution-action paths repathed `src/server/resolution/*` → the as-built wire files `src/server/admin/markets/{resolve,correct,void}.ts`; `resolveMarketAction` marked composed (F-ADMIN-3 + F-RESOLVE-1, trigger → settle); standalone `triggerResolution` row struck (absorbed into the composed action; `trigger-resolution.ts` never created); `closeMarketAction` manual row added; invocation surfaces → the R-15.1 admin pages. **§4.3 + §4-closer** (R-15-B): the stale per-endpoint `src/server/bets/origin-check.ts` / `ALLOWED_ORIGINS` residues repointed to the canonical cross-cutting `src/server/middleware/origin-allowlist.ts` (BETTER_AUTH_URL-derived, §4.1; origin-check.ts not on disk). **§3.4 + §12.6** (R-15-C): "No other Vercel Cron jobs in v1" → two jobs (second = `GET /api/cron/close-due-markets`, per-minute class, tuning HARDEN, R-14.3 close-lag); §12.6 "single job" restatement + stale "§3.5" cross-ref → §3.4. **§0** (R-15-D): blockquote status + version table → 1.0.5, date → 2026-06-13. |
...
| 1.0.15 | 2026-07-01 | HMH | **BC.2 — prescriptive-spec drift reconciliation (backend-consolidation sweep).** §0 status line rewritten current (was 1.0.5, ten patches stale) + companion-files (`cpmm.md`/`RANKING.md` now on disk, only `PSEUDONYM.md`/`design.md` pending) + gates-downstream (17→25 ADRs, `VISUAL.*`→`DESIGN.*`, `EXPORT.*`/`MEDIA.*` added). §1.4 `cpmm.md` forward-reference closed out (on disk, `src/server/cpmm/` built ENGINE.2–12, no longer greenfield). §4.3/§4.6/§4-SSOT/§12.10/Appendix A: phantom `POST /api/admin/uploads/sign` (F-ADMIN-4 mislabel) removed — 4 loci deleted, 1 prose locus corrected, route-handler count Ten→Nine; the real `/admin/markets/media/sign` route (already correctly documented at MEDIA.1) is unaffected. §3.2 W-1 + Appendix A + Appendix B.6: `comments.bet_id` NOT-NULL mis-description corrected to deliberately-nullable (INV-1 verified still enforced via `bets.comment_id NOT NULL` + the atomic W-1 tx — no code change; DEBATE.8 had already fixed §5.1/§14.1 but missed these three). §22.0/§22.1/§22.5/§23.4: ADR-0001 admitted to the index (25→26 ADRs, 22→23 accepted) — this section's own prior claim that ADR-0001 "was never authored" was itself wrong; it's `0001-license-choice.md` (Accepted), not the "brand architecture" topic previously described. §3.8/§19.4.1: `market.created` payload prose brought current to the MEDIA.1 extension (`media[]`, `mediaVideoUrl`). `tracker_v11.html`→`tracker_v15.html` swept across every live-pointer reference; the two point-in-time changelog mentions of v11 (1.0.0/1.0.1 rows) are left as historical record. §0 → 1.0.15. |
| 1.0.16 | 2026-07-06 | HMH | **AUDIT-FIX-A22 — signup/sign-in event completeness + §3.5 spec-vs-built reconciliation** (same-commit with the three participant-auth emit sites). §3.5 rewritten to the *built* signup architecture (option i): pool consumption folded into Better Auth's `user.create.before` hook via `src/server/identity-pool/consume.ts` (its own default-isolation tx; `FOR UPDATE SKIP LOCKED` row-lock is the double-assignment guard), Better Auth owning the subsequent `users` insert — the phantom single-SERIALIZABLE `identity/assign.ts` transaction removed. Three missing §8.8 participant events (`user.oauth_signed_in`, `user.otp_signed_in`, `user.pseudonym_assigned`) now emit via new §7.5.1 sub-case-(b) post-commit carve-outs (Better Auth 1.6.11 exposes only post-commit after-hooks), each with a mandatory verify-then-emit fabrication guard (the after-hook drains on rollback). §3.5 records two pre-existing observations (pool-consume/user-insert non-atomicity; default-vs-SERIALIZABLE isolation), both filed as separate follow-ups. Stale-loci sweep to the built path: §3-SSOT, §3.7 CI-lint dir (`identity`→`identity-pool`), Appendix A row, §8.3 onboarding-loop + cancellation-safety, §16 F-AUTH-4 evidence (SERIALIZABLE→default); §3.5 `{ data: false }`→`ONBOARDING_REQUIRED` throw drift corrected. §8.8 unchanged; §19.4.1: two rows added (`user.oauth_signed_in` / `user.otp_signed_in` → `payload.userId` strip — closing the re-identification gap these now-emitting events would otherwise open at export); no migration; no new ADR. **§0** → 1.0.16. |
| 1.0.17 | 2026-07-07 | HMH | **SYNC sweep — §0/§0.1 version + changelog catch-up for seven rider tasks (A1 · B1 · B2 · B3 · B7-A26 · B7a · B8) + §19.3/§22 reconciliation** (paired with SPEC.1 → 1.0.14; pays the `docs/parked.md` SYNC-sweep entry, now a PAID tombstone). The riders below landed same-commit with their tasks (CLAUDE.md §5.12); only the version/changelog/index metadata is swept here (CLAUDE.md §7 periodic reconciliation). **A1** (PR #197, `4350406`; ADR-0028): §12.2/§12.3 write-once `If-None-Match: *` conditional PUT + §10 pre-moderation `HeadObject` verify + the §22.1 ADR-0028 row. **B1** (PR #199, `72ce26c`): §17.2 row 2 `events_default_nonempty` drain-side transport + row 4 `openai_moderation_upstream_failure` pin + new row 9 `bet_handler_internal_error` (count-prose eight→nine) · §17.3 6c `headObject` fourth `r2_unavailable` source. **B2** (PR #201, `7fc4c60`; ADR-0029 + ADR-0030): §5.1 row 2 `dharma_ledger` `seq` clause · Appendix B.7 `seq` row · §6 intro + §6.1 clauses 1–2 + §6.2 addendum (`enforce_bucket_a_no_truncate()`, 25 statement-level triggers, forward obligation) + §6.5 owner-privilege reconciliation. **B3** (PR #202, `6244dce`; ADR-0031 + ADR-0015 Patch): §3.1 durable pre-check · §3.2 W-1 receipt · §5.1 row 10 `bet_receipts` + §5.2 (Bucket A 9→10, protected 12→13) · §6.2 (10 tables, 20+26 triggers) · §7.3 third idempotency layer · §9 receipt append · §11 durable pre-check step 3a + ownership-checked never-throws release + alarm-6b two-site · §15.4 → 39 codes (`error_position_conflict`; `insufficient_shares` joins the aggregated participant set) · §17.2 alarm-9 sibling tags + §17.3 6b two-site · §19.3 `bet_receipts` excluded-entirely (2→3). **B7-A26** (PR #209, `ae489a6`; doc-only ruling, no new ADR): §20.2 freeze accepted-window clause · §19.1 dataset-build-point boundary (three legitimately-post-freeze row classes) · §19.2 build-point replica. **B7a** (PR #211, `7dabdc9`; ADR-0015 Patch record, no new ADR): §11 Upstash transport-bounding line (A14). **B8** (PR #216, `f0be380`; doc-only, D1–D4 + item-5 reconciliation): §5.1 comments-row D2 build-deferred annotation · Appendix B.6 `market_media_id` PENDING-BUILD note · Appendix B.15 +2 rows (`content_type`/`byte_size` SHIP) · §4.3 catalogue nine→eleven (`close-due-markets` cron row + `_smoke-error` row + manifest PENDING-BUILD) + §4-closer two-cron fix · §17.6 `request_id` forward-obligation paragraph. **Swept in this pass — pre-existing enumeration drift:** §19.3 enumerates `market_media` (ADR-0026; omitted since 1.0.12 — 20→21 dataset-relevant, shipped 15→16, ships in full per Appendix B.16) and the §19.3 closing recap now names all three excluded-entirely tables (`watermark_state`, `cron_alarms`, `bet_receipts`). **§22 reconciliation:** three new rows (ADR-0029/0030/0031); heading "26-row" → "30-row"; inventory 26 → **30 ADRs** (29 ADR files + ADR-0012 in-flight; 27 accepted + 2 superseded + 1 in-flight); range `0003–0027` → `0003–0031`; ADR-0015 row annotated with its two in-place Patch records (B3 2026-07-05 · B7a 2026-07-06). **§0** status/companion/gates-downstream ADR cites → 29 ADRs at `0003–0031`; status-line migration head 0019 → 0023 + SPEC.1 cross-cite → 1.0.14 (same-line epoch coherence); version → 1.0.17. |
| 1.0.18 | 2026-07-18 | HMH | **Discovery route reconciliation** (paired with SPEC.1 → 1.0.17, new §22 Discovery). §3.3 Pattern R-2: the market-list route `/markets` → `/` (the Discovery front page — SPEC.1 §22) in the R-2 prose, the R-2 summary-table row, and the illustrative page path (`src/app/(public)/markets/page.tsx` → `src/app/(public)/page.tsx`). No mechanism change — R-2's `'use cache'` opt-in, public-only constraint, and `cacheLife` shape are unchanged; only the route the pattern names is corrected to where Discovery is served (SPEC.1 §22). §0 → 1.0.18. | — |
| 1.0.19 | 2026-07-21 | HMH | **ADR-0032 bookmarks riders** (same-commit with migration `0024`; UI-A6 execute — the gated plan→execute slot). **§5** preamble 24 → 25 tables / ten → eleven domains / Bucket C eleven → twelve. **§5.1** new `bookmarks` row (#25, **Bucket C**, NEW `bookmarks` domain, carries `user_id`; both FKs `ON DELETE restrict`, `UNIQUE(user_id, comment_id)`, `bookmarks_comment_id_idx` per the positions/A31 FK-index convention; NO append-only/TRUNCATE trigger — deliberately absent from `TRUNCATE_GUARDS`, protected set unchanged at thirteen). **§5.2** Bucket-C summary 11 → 12 (+ `bookmarks`). **§5.3** domains 10 → 11 (+ `bookmarks`). **§4.2** Server Actions catalogue Fourteen → Sixteen (`addBookmarkAction`, `removeBookmarkAction`; new **F-BM** family, F-BM-1). **§19.3** excluded-entirely three → four (+ `bookmarks`; dataset-relevant unchanged at 21; ship/not-ship unchanged at 16 + 5). **§22** ADR-0032 index row (30 → 31 ADRs; 27 → 28 accepted; "31-row index"; upper bound `0003–0031` → `0003–0032`; §22 heading + §22.5 SSOT counts). **Appendix A** new `bookmarks.ts` (A.1) + `src/server/bookmarks/{add,remove,list,figures}.ts` (A.3) + the `(public)/bookmarks/` surface (A.4) rows. **Appendix B** closing-notes "excluded entirely (4)" note (+ `bookmarks`; no per-column entry — nothing ships, the `bet_receipts` precedent). Migration head `0023` → `0024`. Event-free (Option 1 — EVENT_TYPES unchanged at 24); zero invariant surface (no `events`/`dharma_ledger`/`bets`/`comments`/`resolution` write). **Count reconciliation flagged:** ADR-0032 §6 / the plan named the §5.1 total as 22 → 23 (an AGENTS.md operational-table-excluded count); reconciled to the SPEC.2 §5.1 all-inclusive live count **24 → 25** per the verify-don't-trust mandate (AGENTS.md's "22" left untouched — out of A6 scope). **§0** → 1.0.19. | — |
| 1.0.20 | 2026-07-22 | HMH | **UI-6 S3 — reactive Remove/Ban + live review-feed reader (F-ADMIN-4 partial; paired with SPEC.1 → 1.0.20, the §15 F-ADMIN-4 rider).** **§4.2:** the `moderateComment` row **corrected** from the superseded SCAFFOLD.16 F-γ-thin approve/block/remove-pass-verdict model to the ratified reactive contract `moderateComment({ commentId, action })`, `action: 'remove' \| 'ban'` (ADR-0020/0021 — two independent axes, no combined verb; Remove appends one `content_removed` row + writes NOTHING to `comments`, Ban appends one `user_banned` row + sets `users.banned_at` only-where-NULL, neither touches a position/ledger, neither emits an `events` row). A correction, not a new row — Server-Action count unchanged at **Sixteen**. **§4.2 read-model note added:** the F-ADMIN-4 live review feed is the read-only `loadReviewFeed` (`src/server/admin/moderation/review-feed.ts`) backing the `/admin/moderation` Server Component page (not a Server Action) — every live Track-C comment across all markets MINUS the `content_removed` set (a `NOT EXISTS` anti-join on the same predicate `loadRemovedSet` uses; the 200-cap keyset window applies to live rows), no filter / no ranking (ADR-0021), images minted as short-TTL (60s) admin-gated `signRead` URLs — never a raw R2 key; the three DEBATE.7-deferred F-ADMIN-4 pieces (Track-A rows, LD-3 carve-out ban-review, inline participant affordance) are **not** built (F-ADMIN-4 partially delivered). **No DDL** (`content_removed`/`user_banned` already in `mod_reason` — no §5 change); **no new event type** (§7.1 `EVENT_TYPES` unchanged at **24**); no new ADR (ADR-0020/0021 govern). **§0** → 1.0.20. | — |
| 1.0.21 | 2026-07-30 | HMH | **BOOKMARK-ADD-WIRE (B1) — the bookmark add path wired on the debate view** (same-commit riders with the Slice 1–3 code; ADR-0034 cited, not authored — it landed spec-lane at PR #272). **§4.2** the `addBookmarkAction` invocation-surface cell replaced: the action had **zero production call sites** at A6 and is now reached from debate-view post cards (`ArgProfile` via `PostCard`/`PostFocusHeader`) and reply cards (`ReplyCard` via `ReplyPreview`/`ReplyScroller`); own arguments render no icon (only another author's argument is bookmarkable — ADR-0032 D-3; the own-comment id set is composed viewer-scoped per ADR-0034); signed-out viewers keep the disabled affordance; the other-user **Profile card surface is NOT wired** — pending the W2.7 / canon §3.11 design ruling. The same edit repairs the **broken `(§11)` cite** in that cell: inside SPEC.2 `§11` resolves to the Rate-Limit & Idempotency Contract, whereas the referent is `docs/plans/UI-A6.md` §11 — now an explicit cross-document cite. **Appendix A.3** new `src/server/debate-view/viewer-context.ts` row (a pre-existing UI.A2 omission paid while B1 extends that module's contract): the DTO gains two additive, market-scoped, **ID-only** arrays (`bookmarkedCommentIds`, `ownCommentIds`) read by two SELECTs inside the existing read-only transaction — arrays not `Set`s (the RSC → client boundary), never a masking input. **§22** two index rows (**0033** + **0034**) + count reconciliation: "31-row index" → **33-row**; inventory 31 → **33 ADRs** (32 ADR files + ADR-0012 in-flight); accepted 28 → **30**; range `0003–0032` → `0003–0034`; §22.5 SSOT counts; §22 intro range. **Ruling G1 (this task):** ADR-0033 receives an **index row only** — its substance is folded into neither SPEC.1 nor SPEC.2 and that fold stays a docketed follow-up owned by its own task; the row exists so the §22 arithmetic matches the files on disk. **§0** status-banner reconciliation — the banner read `1.0.19` while §0 metadata read `1.0.20` (drift found at recon R1); both now `1.0.21`, and the banner's ADR cite moves to `0003–0034` (32). **Migration head unchanged at `0024`; EVENT_TYPES unchanged at 24; zero invariant surface** — B1 adds no write path, it only makes an existing, reviewed one reachable. `loadDebateView` carries a **zero-line diff** (ADR-0034's load-bearing guarantee). **SPEC.1: no amendment** — §23 delegates bookmarks entirely to ADR-0032 and F-BM-1's acceptance already exists. |
| 1.0.22 | 2026-07-31 | HMH | **F-DEBATE-4 (B3) — same-commit rider with the polling build.** §4.3's SSE/WS line replaced: the debate view refreshes by re-invoking the `/m/[slug]` server read on a client-side interval, not by polling a Route Handler; the eleven-handler catalogue is unchanged and F-DEBATE-4 adds no twelfth, because a dedicated read endpoint would fork removal-masking (ADR-0021; ADR-0034 D-4 keys masking solely on `loadRemovedSet` inside `loadDebateView`) into a second implementation. POLL_INTERVAL_MS_DEBATE_VIEW recorded as provisionally pinned at 15000 ms per SPEC.1 §16.1 / Appendix B, with the tune still deferred to HARDEN.6 — the prior wording deferred the value outright, which left the flow unbuildable before go-live. The measured per-tick cost is recorded because the refresh re-executes the layout as well as the page: twelve to fourteen sequential round-trips per open tab, two of them session reads and three of them scaling with the market's comment and event counts. Suspension is keyed on composer-**open**, not composer-dirty, and the stop rule on `market.status` alone — the global conclusion freeze reaches no client component and is deliberately not implied. `/m/[slug]` gains an explicit `force-dynamic` segment config, closing a latent trap where the route's dynamism depended on an incidental `headers()` call. §0 status-banner reconciliation: the SPEC.1 cite advances to 1.0.25, and "DEBATE incl. .4 render" is disambiguated as the DEBATE.4 tracker stratum rather than SPEC.1 §9's F-DEBATE-4 flow — two different referents colliding on one string. §0 metadata Date advanced; it was stale at 2026-07-22 against a 1.0.21 row dated 2026-07-30. `loadDebateView` carries a zero-line diff (ADR-0034 D-1). Migration head unchanged at 0024; EVENT_TYPES unchanged at 24; no DDL, no new ADR, no new Route Handler, no read-model or DTO change. |
| 1.0.23 | 2026-08-20 | HMH | **SYNC-4 — the §22 ADR index rebuilt to the files on disk; the `N5` backlog discharged.** `docs/parked.md`'s `N5` row had said since SYNC-1 that this is a **normative §22.1/§22.5 edit**, not a row you can append, and had accumulated four ADRs while being corrected in place three times. **§22.1** gains four index rows — **0035** (guarded staging reset, STAGING-PARITY Slice A), **0036** (Vitest-context operational runners, STAGING-PARITY Slice A), **0037** (onboarding-deck seen-marker cookie, O1-DECK), **0038** (scale target 100k, SYNC-3) — each row's Title, Status and Accepted date taken from that ADR's own header. **Every count in this document was then RE-MEASURED off the amended table by grepping it, never recomputed by arithmetic**, which is the discipline `N5` and `O-2` both name: §22.1 heading `33-row` → **37-row**; §22 intro range `0034` → **0038**; inventory `33 ADRs` → **37**; status split `30 accepted` → **34 accepted** + 2 superseded + 1 in-flight; numbering range → `0001, (0002 skipped), 0003–0038`; §22.5 SSOT cell → `37 ADRs — 36 ADR files + ADR-0012 in-flight`. **§0** the same figure corrected in its three mirrors — the status banner (`ADRs 0003–0034 (32)` → `0003–0038 (36)`), the companion-files line, and the *Gates downstream* row. **The §0 staleness annotation is rewritten and its "folded into NEITHER" clause struck** — after this pass the four ARE folded, and an annotation that outlives its condition is itself the defect the sweep exists to remove; what remains of it is the standing `O-2` instruction to read `ls docs/adr/` rather than trust this line. **Measured, not asserted:** 37 index rows, 36 with a real file, 1 `(file pending)` (ADR-0012), against **36** ADR files on disk — the §22.3 cross-reference invariant that the index match the files now holds again, having been broken since ADR-0035 landed. No ADR substance folded: these are index rows plus count reconciliation, the same shape as the BOOKMARK-ADD-WIRE G1 ruling. **SPEC.1 unchanged.** |
| 1.0.24 | 2026-08-21 | HMH | **LOTS-1 / ADR-0039 registry riders (PHASE-0) — the `lots` table admitted to every inventory it belongs in, same-commit with migrations `0025_lots` + `0026_lots_no_delete`.** Mechanical bookkeeping only: ADR-0039's substance is the ADR's, and the one decision this pass could have quietly made it deliberately refuses to (see §19.3 below). **§5** preamble 25 → **26 tables** / eleven → **twelve domains** / Bucket C twelve → **thirteen**; the protected total stays **thirteen**, and that is the load-bearing non-change. **§5.1** new `lots` row (#26, **Bucket C**): 1:1 with `bets` via `lots_bet_id_uq` + FK, so lot ⊂ bet ⊂ comment is closed by storage rather than convention (INV-1 supplies the second link); three FKs `ON DELETE restrict`, each indexed on the referencing side; seven CHECKs; **no append-only trigger and deliberately no TRUNCATE guard**, because `surviving_shares` falls on every sell and a teardown's `TRUNCATE bets CASCADE` must still reach it. **§5.2** Bucket C 12 → 13 (+ `lots`) plus a paragraph on why the R9 permanence guard `lots_no_delete` (migration 0026) does **not** raise the protected count: it is row-level `BEFORE DELETE` only, named outside the `bucket_%` family on purpose so it enters neither the §6 contract nor the staging reset's guard catalogue — both of which it would misdescribe. `lots` may CHANGE, in one direction, forever, and may never LEAVE; no bucket names that, so it is said in prose rather than smuggled into a classification. **§5.3** domains 11 → 12 (+ `lots`, its own domain rather than a fourth table in `bets.ts` — it carries its own pure core, sentinels and invariant-class rule). **§19.3 — the one that is NOT decided here.** `lots` is added as dataset-relevant (21 → **22**) with **Shipped? UNDECIDED**, because ADR-0039 declines the call in terms: *"Whether lots are released alongside bets is a SPEC.1 G3 question, not this ADR's."* It is counted in **neither** the 16 nor the 5, precisely so an unanswered question cannot be mistaken for a settled NO by arithmetic; the row records that the question exists. **Appendix B.17** gains the matching note and, for the same reason, **no per-column B.n section** — writing one would be indistinguishable from having decided — but does record the eleven column names and the single PSEUDO column (`user_id`, on the `positions` footing) so a future G3 ruling costs a reading rather than a re-derivation. **§22** ADR-0039 index row (37 → **38 ADRs**; 34 → **35 accepted**; "37-row index" → **38-row**; range `0003–0038` → `0003–0039`; §22 intro + §22.5 SSOT cell). **Appendix A** new `src/db/schema/lots.ts` (A.1) + `src/server/lots/{compute,persist,errors,basis}.ts` (A.3) rows; A.1's `bets.ts` cell corrected to name `bet_receipts`, which has been in that file since ADR-0031 and was never listed. **§0** all three mirrors moved together, which is the only way this line stops going stale: the status banner (migration head `0024` → **`0026`**, ADR cite `0003–0038 (36)` → `0003–0039 (37)`), the companion-files line (`36 ADRs at 0003–0038` → `37 at 0003–0039` — the mirror SYNC-4 had to correct too), and *Gates downstream* 36 → 37; the staleness annotation notes it has now gone stale by one **three** times and is re-scoped to a named task rather than a standing fact. **EVENT_TYPES unchanged at 24** — a lot rides the bet's events and mints none of its own. **SPEC.1: deliberately untouched.** The §23 Đa prose rider is founder-authored and stays owed; nothing here writes it. |
| 1.0.25 | 2026-08-22 | HMH | **AMEND-1 — records ADR-0040 (unwire the bookmark module and the Profile Dharma graph).** **§4.2** Server Actions catalogue Sixteen → **Fourteen** (`addBookmarkAction`, `removeBookmarkAction` rows removed). **Appendix A.3** four `src/server/bookmarks/{add,remove,list,figures}.ts` rows removed (files deleted); the surviving `src/server/debate-view/viewer-context.ts` row's `bookmarkedCommentIds`/`ownCommentIds` clause replaced with a one-line note that both arrays were removed at ADR-0040 — the row itself stays, the file and its other three return fields (`position`/`balance`/`spendableToday`) are unchanged. **Appendix A.4** the `src/app/(public)/bookmarks/page.tsx` row removed (route deleted). **Deliberately NOT touched, and flagged so the gap is findable rather than silent:** **Appendix A.1**'s `src/db/schema/bookmarks.ts` row stays — the table is deliberately retained, no DDL; **§5/§5.1/§5.2/§5.3/§19.3**'s table/domain/Bucket-C/dataset-exclusion counts are untouched for the same reason (the schema-registry facts they state remain true); **§22**'s ADR index does not yet carry an ADR-0040 row — this pass is the API-surface + version-bump rider only, the index row is a follow-up in the SYNC-4/ADR-0033-G1 pattern, not bundled here. | ADR-0040 unwires the bookmark module and the Profile Dharma graph product-wide. This row and its companion SPEC.1 §23 row are the same-commit spec amendment ADR-0040 §Supersedes-and-amends promises for SPEC.2 §4.2. |
| 1.0.26 | 2026-08-26 | HMH | **S-4 (ADR-0041) — §4.3 same-commit rider correcting the 1.0.22 row's `/m/[slug]` dynamism and tick-cost claims.** `cacheComponents` (Next 16.3.2) is enabled: `export const dynamic = "force-dynamic"` — which 1.0.22 recorded as the route's dynamism mechanism — is gone (the directive errors the build under Cache Components, redundant by the framework's own new default) and is replaced by `export const instant = false`, the equivalent instant-navigation opt-out; the poll's own guarantee (no `'use cache'` on the page file itself, so a refresh always re-executes it) is unaffected. The measured per-tick cost 1.0.22 recorded — twelve to fourteen sequential round-trips including two session reads — is superseded: a request-scoped session-dedup helper (`src/app/(public)/_lib/session.ts`) folds the layout's and page's session reads into one, and the shared comments/ranking/totals/media/chart block now serves from a per-market `"use cache"` entry keyed on `(market, reserves)` rather than re-executing every tick, taking the cost to **2 round-trips per open tab signed-out, 10 signed-in**. No DDL, no new event type, no invariant surface (nothing here writes `bets`/`comments`/`dharma_ledger`/`resolution_events`). **§22 ADR-0041 index row deferred** — same posture as 1.0.25's ADR-0040 row: this pass is the §4.3 rider only. |
| 1.0.27 | 2026-08-27 | HMH | **S-7 (ADR-0044) — §11 same-commit rider.** Idempotency scoping corrected from **global** (key alone) to **`(user_id, key)`** — the Redis cache, the durable-receipt read, and the 23505-catch outcome are all now scoped to the authenticated user; cross-endpoint (same-user, place vs sell) reuse remains a named open gap (`docs/parked.md` D-8). The paragraph's claim that comment-bearing bets are placed via `placeDirectComment`/`placeReply`/`placeImageComment` Server Actions using a `(user_id, market_id, body_hash, posted_at_minute)` natural key is removed — none of the three functions, `body_hash`, or `posted_at_minute` exist anywhere in `src/` or `drizzle/` (verified live at `d9128e22`); comment-bearing bets are placed through the same `place`/`sell` Route Handlers as any other bet, with the same `Idempotency-Key` contract. **⚠ The same false Server-Action claim also appears at §4.2's "Fourteen Server Actions" intro paragraph, its table row for `placeDirectComment`, and Appendix A's file-map row for `src/server/comments/place.ts` — none of those three sites are touched by this rider; S7-12 is expanded to route all three to the AMEND lane rather than leaving them implied-fixed by this row.** No DDL, no new event type. |
| 1.0.28 | 2026-08-29 | HMH | **DATASET.2 — seven ratified dataset-release edits (founder ruling, 2026-08-28). No ADR: R1 aligns the CSV arm with masking ADR-0025 already rules, and the rest are per-column treatments that live in Appendix B by design.** **(E1)** Appendix **B.6** — `comments.body` and `comments.image_uploads_id` move from `SHIP` to the new `WITHHELD_IF_REMOVED` treatment: a reactively-removed comment's body is withheld, the row preserved. The old `SHIP` note (*"only `pass`-verdict comments exist in this table"*) described the **pre-commit gate** (§10) and not **reactive removal** (ADR-0021), which admits the comment and takes it down by appending a `mod_actions` row with **zero writes to `comments`** — so the body is still in the column and an un-intersected read publishes it. Shipping it would republish removed content permanently under CC-BY-4.0, and — since `mod_actions.reason` + `target_comment_id` also SHIP — would publish a labelled index of which bodies those were, while contradicting the `debates/*.md` arm that inherits masking via `loadDebateView`. `mod_actions` continues to ship the fact of removal. Appendix B's treatment legend grows five → **six**; §19.4.1's `comment.placed` row is corrected in the same pass (it cited *Appendix B.13* for the `comments` table, which is **B.6**). **(E2)** Appendix **B.13** — `events.aggregate_id` for `aggregate_type = 'admin_session'` is **REDACTED** to `[redacted-admin-session]`, replacing *"SHIPs raw — defense-in-depth covered by `BREAK_GLASS.md` rotation + payload STRIP rules per §19.4.1"*. That carve-out cited as its mitigation the very rule that **strips `payload.sessionId` from `admin.signed_in`/`admin.signed_out`** — the same value, in the same row, stripped from one field and published from another — while §19.3 row 21 withholds the whole `admin_sessions` table. One admin, so it identifies no participant; it is nonetheless a session identifier published permanently for zero research value, and an internal contradiction resolves toward the stricter arm. Sentinel not hash, per §19.4's strip-not-hash policy. **(E3)** Appendix **B.11 + B.12** — `metadata.actor_id` now **defers to B.13** instead of restating it: key name **preserved**, value rewritten, uniformly. Three appendix blocks previously gave three treatments for one JSONB key (B.11 bare `SHIP_KEY`; B.12 *"rewritten to `actor_pseudonym`"*; B.13 sentinel-or-PSEUDO). The rename cannot work — `actor_id` is polymorphic (participant UUID or the `'admin-singleton'`/`'system'` sentinel) and **a CSV column cannot change its name per row**, so `actor_pseudonym` would label the sentinel as a pseudonym; in B.12 specifically the key is self-actor, so a rename would deliver one identity twice under two names. B.11/B.12 `metadata.user_id` likewise routed to B.13. **(E4)** **§19.4.1** — two additions. **Depth:** the STRIP_KEY rules now apply at **any nesting depth and through arrays**, and the export pipeline's egress value-scan must walk to the same depth. The one-level walk was correct for every payload shape that exists today, which is what made it worth changing: depth is not a property a future author checks when adding an event type, and the value-based guard cannot rescue it because a nested secret is never harvested into the forbidden set — the miss is silent in both layers at once. The **`m/` vs `u/` R2-key namespace rule is preserved and load-bearing** (market media ships; `u/`-prefixed never does), because a depth rule without it strips a value that must ship. **`commentId`:** added to `image_upload.committed`'s STRIP_KEY targets **unconditionally** — it reconstructs from the `events` side exactly the comment↔upload association B.6 now withholds. Unconditional rather than conditional-on-removal because **a conditional strip leaks the condition** (presence/absence of the key becomes the removal flag), and it costs nothing since `comments.image_uploads_id` already carries the association wherever it is permitted. **(E5)** **§19.4** — the table titled *"the ten PII columns dropped at export"* contained `users.pfp_filename`, whose own treatment cell read *"Released as-is"*: a row saying it ships, inside a table whose title said it does not. `pfp_filename` **is not dropped** — it is `NULL_IF_ERASED` (B.1) and is the identity-pool join key; dropping it would destroy that join. It moves out of the table into an explicit note, the title and lead sentence become **nine**, and the table gains an explicit `#` column because §19.4.1 and the release runbook both cite it **by row number**. **(E6)** Citation drift, editorial. §19.4.1's `user.tos_accepted` row cited *"§19.4 row 3-4"* for `ip`/`user_agent`; those are `name`/`image` — corrected to **rows 5–6**, cited by name. `docs/runbooks/dataset-release.md` step 6 said *"the 8 PII columns"* (never a count §19.4 has had) → **nine**, with the `pfp_filename` caveat; step 7 cited *"§19.4 row 7-8"* for `metadata.ip`/`user_agent` → **rows 8–9**, by name. ⚠ **The runbook's post-E5 numbers are 8–9, not the 9–10 they would have been before E5 removed `pfp_filename` from the table** — E5 and E6 are coupled, and E6 was applied against the file as E5 left it rather than against a number quoted from a brief. Step 7 also gains a `commentId` check and a note that the four key-absence checks are **at any depth**. **(E7)** `docs/parked.md` — the `ADMIN-EVENTS-WRITER` fork is extended to name **`user_events`**, which carries the identical defect and was flagged nowhere: zero writers anywhere in `src/` (every `userEvents` reference is a schema declaration), contracted as shipped at §19.3 row 9, with a **full** Appendix B.12 treatment map including a `PSEUDO` rewrite for a `user_id` no row carries — while the lifecycle facts are actually written to `events`. Same recommended fix (a **projection**, not a writer), same **2026-09-15** date, which now governs both tables; a ruling closing only `admin_events` leaves half the defect past its own deadline. No DDL, no new event type, no invariant surface. ⚠ **Not swept in this pass:** the §0 header blockquote still opens `Status: 1.0.24` against this table's version — two version strings, three patches apart, in one file — and CLAUDE.md §1 quotes the stale one. Flagged for a SYNC sweep rather than fixed here, this pass being fenced to E1–E7. |
| 1.0.29 | 2026-09-01 | HMH | **DATASET.3 — five ratified dataset edits (founder rulings E, S1, S2, S5, S6). No ADR: these are per-column treatments and a per-key selection rule, both of which live in §19.4.1 and Appendix B by design.** **(F1)** **§19.4.1 INVERTS from a deny-list to an allow-list** and is retitled *Per-payload-key **SHIP** rules*. It named, per event type, the keys to REMOVE, with everything else shipping. `events.payload` is `jsonb` — its shape is open, anyone adding an event type or a key extends it — so *"everything else"* is unbounded and **"unlisted" is the state every new key starts in**. The same defect was found three nights running and patched three times by extending a list: §19.5's six FK paths against Appendix B's ~14; `comment.placed` rebuilding by a shorter route the association `image_upload.committed.commentId` had just been ruled to protect; and a nested key under a name on no list at all (`payload.client.remoteAddr`), which is **unstripped, unharvested, unmatched by value and unmatched by key** — four layers quiet on one input, with the build reporting success. §19.3 already had this right one level up (an unclassified table fails the build rather than defaulting either way); this extends that property inward, from tables to keys. The declaration is a **tree**: a leaf ships a scalar, a container is declared by nesting, and **reaching a container under a leaf fails the build**, because *"ship this subtree unread"* is precisely how an unreviewed key rides out inside a reviewed one. An event type that ships nothing declares an empty set **explicitly** — and a MISSING entry now exports a blank payload rather than a leaked one, which is the safe direction and still the wrong artifact (a whole event type's research content silently absent from an append-only corpus), so it fails the build. ⚠ **The `m/` vs `u/` R2-key namespace carve-out is no longer needed by the strip** — `market.created` declares `media[].key` and `image_upload.*` declares none, so the outcome is identical and decided by the declaration rather than by inspecting the value; it remains load-bearing in the secret harvest and the egress assertion, which still reason about key names. **(F2)** Appendix **B.5** `bets.idempotency_key` and **B.11/B.12/B.13** `metadata.idempotency_key` move from `SHIP`/`SHIP_KEY` to **`STRIP`/`STRIP_KEY`** (ruling S2). The old note read *"client-generated opaque string; carries no PII"* — an **assumption wearing the clothes of a constraint**. The stored value is the raw `Idempotency-Key` header, validated only by `^[A-Za-z0-9_-]{1,255}$`: 255 bytes the participant chooses, which §10's gate never sees because it covers comment bodies and images and not headers. Stripped from **both** surfaces in one pass, because stripping a value from a column and shipping it from a metadata blob one table over is the `admin_session` mistake B.13 already made once. The §3.7 metadata set is seven fields; **four now ship**. **(F3)** **§19.4.1 `comment.placed` no longer ships `uploadId`** (ruling S5) — the other half of the recovery path the 2026-08-28 `commentId` ruling closed. That payload carries `commentId` too, so shipping `uploadId` rebuilds the association B.6 withholds **in one row, with no join** — a shorter path than the one that was closed, and closing one end while leaving the other is not a partial mitigation but none. Nothing is lost: `comments.image_uploads_id` carries the link for every comment that is not withheld. ⚠ **Under F1 this is an absence rather than an edit** — the key is simply not declared — which is the inversion's argument in miniature: the safe outcome became the cheap one. **(F4)** **§19.4.1 + Appendix B preamble** state that timestamps export at **full stored precision, six fractional digits**, ISO-8601 UTC (ruling S6). Stated in two places because it is invisible in every layer that would normally catch it: Postgres stores microseconds, a JS `Date` holds milliseconds, so a reader that materialises a `timestamptz` through a driver's default parse discards three digits **before any formatting code sees the value**, and two events 200 µs apart export as the same instant. It cannot be fixed downstream of the read, and it cannot be detected by comparing two builds that both floor — a fixture whose timestamps are round milliseconds is **structurally unable** to test whether it was preserved. **(F5)** `docs/runbooks/dataset-release.md` **step 7 — content correction, not a citation fix.** Its first bullet told the operator that `user.tos_accepted` rows *"show `userId`"*. `userId` is a raw `users.id` and has never shipped; B.13 has always listed it in the general strip set. An operator following that bullet on the morning of 6 November would find no `userId`, conclude the export was broken, and have to decide under time pressure whether to trust the runbook or the artifact. Step 7 also gains checks for the S5, S2 and S6 outcomes, and step 6 is corrected to read §19.4.1 the right way round — a check for *"the listed keys are absent"* is now exactly backwards. **Also recorded, not edited:** ruling **S1** (needle provenance partitions fatality) changes no spec text — it is a property of the export pipeline's guard tier, not of what ships; and the §0 `Status:` blockquote drift flagged at 1.0.28 was **fixed on `main`** in the interim, by removing the number rather than updating it (`O-15`). |
...
Both remaining patterns (W-1, W-3) share SERIALIZABLE isolation and the 3-attempt full-jitter retry shape from ADR-0013 (parameterised by the per-flow callback). They differ in lock-order spine and actor identity. ENGINE.7 (W-1) / ENGINE.9 (W-3) / ENGINE.13 (grant) implement; ENGINE.10 is the cross-flow correctness-at-scale exit gate.
...
Twenty-six tables in v1 across twelve domains. Ten strictly append-only (Bucket A); three append-only with one whitelisted column transition (Bucket B); thirteen mutable with no append-only trigger (Bucket C). Total protected by §6's append-only enforcement contract: thirteen — **unchanged by `lots`**, which is Bucket C and carries a narrower `lots_no_delete` guard that is not an append-only trigger (§5.1 row 26).
...
| **C** — mutable | 13 | No append-only trigger (constraint-driven validation only) | `users`, `markets`, `pools`, `positions`, `sessions`, `accounts`, `verifications`, `admin_sessions`, `watermark_state`, `cron_alarms`, `market_media`, `bookmarks`, `lots` |
...
**Per-domain schema-file split.** Tables are grouped into domain files at `src/db/schema/<domain>.ts` per ADR-0008 §4 with a barrel re-export at `src/db/schema/index.ts`. Twelve domains in v1: `auth`, `markets`, `bets`, `bookmarks`, `comments`, `dharma`, `events`, `identity`, `image-uploads`, `lots`, `audit`, `system`. `lots` is its own domain rather than a second table inside `bets` (LOTS-1 / ADR-0039): it has its own pure core, its own error sentinels, and its own invariant-class rule, and the `bets` file already carries three tables. The `auth` domain spans two ADR ownerships (ADR-0004 for the four Better Auth tables; ADR-0010 for `admin_sessions`); both groups share the same schema file because they share the auth surface conceptually.
...
> **[Substantively absorbed from ADR-0014 (SPEC.15) on 2026-05-07. Track B *consequence* amended by ADR-0021 (2026-06-18): held → block; no held queue; admin moderation is reactive on live content; text-only `sexual/minors` carve-out blocked-then-surfaced. The gate shape, vendor, fail-closed posture, Redis reservation, idempotency-first ordering, CSAM short-circuit, and F-MOD-4 atomicity below are UNCHANGED.]**
...
**Track A image-presence carve-out (SCAFFOLD.16 LD-3) — reactive model (ADR-0021).** Text-only `sexual/minors === true` flags route to Track B; image-attached `sexual/minors === true` flags route to Track A (block + auto-ban + CSAM auto-report). Under the reactive model the text-only path is the **one** Track B item that is not audit-only: the content is **blocked at the gate** (it never publishes), the author is **not** auto-banned (text-classifier false-positive risk for this CSAM-adjacent category is elevated by news/fiction/educational vectors), and the `mod_actions` row (reason `sexual_minors_text_blocked`) **surfaces in the admin's reactive review** for a **ban decision** — block now, human reviews for ban later (ADR-0021 §8; SPEC.1 §14 F-MOD-2). This preserves the LD-3 admin-judgment mitigation (aligned with Bluesky / Roblox / Reddit practice) without a held queue. At the model level, `sexual/minors` is text-only on `omni-moderation-2024-09-26` per OpenAI docs (image input returns score 0 for this category); the carve-out is aligned with classifier capability, not just policy. ADR-0021 resolves the prior SPEC.1 Appendix A (which stale-read "Track A auto-ban" for text-only `sexual/minors`) ↔ SPEC.2 §10 drift in favour of this LD-3 routing. **A2 image-adult-sexual escalation (DEBATE.7 / ADR-0014 §18 patch-record).** Adult `sexual === true` on an **image-attached** submit also routes to Track A — the live CSAM-image backstop while PhotoDNA is parked (omni scores image-borne CSAM as adult `sexual`, not `sexual/minors`, which is text-only on the snapshot); adult-sexual **text** stays Track B (auto-ban-on-text deferred to HARDEN.5). Realised in `precommit.ts` via the omni `sexual` category + `imageR2Key` presence, implementing the fixed App.A image→Track A rows (SPEC.1 Appendix A). Post-experiment hardening recommendations (R-1 predicate strengthening, R-2 verdict-shape expansion, R-3 retry-policy expansion) deferred per `docs/parked.md` "SCAFFOLD.16 §research — R-1/R-2/R-3 hardening".
...
4. **Call `precommitModerate(input)`.** Returns `{ outcome: 'pass' | 'track_a' | 'track_b'; categories: string[]; categoryScores: Record<string, number> }`. *(DEBATE.7 §6 enrichment: `categoryScores` carries the raw OpenAI category scores at decision time; the consequence writer (`recordGateBlock`) persists it into `mod_actions.categories` — "with confidence", SPEC.1 §786.)*
5. **Branch on verdict:**
   ...
   - **`track_a` / `track_b`** — write a `mod_actions` row in a standalone short transaction; return the SPEC.1 §14 F-MOD-* response. The bet/comment transaction **never opens**. *(ADR-0021: the Track B consequence is a content **block** — rejection, author may revise, no auto-ban — not a held-for-review state; the standalone-`mod_actions` + never-open-the-tx mechanics here are unchanged. Reason codes: `track_b_blocked`, or `sexual_minors_text_blocked` for the text-only `sexual/minors` carve-out.)*

**Events-row emit on block (AUDIT-FIX-B5, A13).** The `track_a` / `track_b` standalone `mod_actions` transaction emits exactly one `events` row of `event_type = 'moderation.blocked'` inside that same transaction, on every block branch (`track_a_autoban`, `sexual_minors_text_blocked`, `track_b_blocked`) — satisfying the §3.7 ≥1-event-per-state-mutation rule and the §7.5 F-MOD-* write set (`mod_actions` + `events`). `aggregate_type = 'mod_action'`, `aggregate_id = mod_actions.id`. Payload `{ userId, reason, banned, uploadId }` (`banned = outcome === 'track_a'`; `uploadId` is the `image_uploads` row id or `null` on the text-only branch; the raw `imageR2Key` is deliberately excluded — it embeds the userId). `metadata.flow_id = 'F-MOD-1'` for `track_a`, `'F-MOD-2'` for the two `track_b` branches (per SPEC.1 §14 F-MOD-1/F-MOD-2 — the event is a moderation flow, not the rejected bet flow). `metadata.user_id = actor_id = <submitting user>` per the §3.7 binary actor convention (the automated gate is not an admin actor; `event_type='moderation.blocked'` disambiguates the participant-self-actor encoding as "this participant's submission was blocked"). This is a distinct column from `mod_actions.actor_id`, which remains `'system'` (the executor of the enforcement) — the two answer different questions and do not conflict. `metadata.request_id`/`ip`/`user_agent` = `'unknown'` and `idempotency_key = null` at this seam (S-C deferral; the established `logout.ts`/`tos-accept.ts` placeholder pattern — the writer takes no request-scoped metadata param, and `ip`/`user_agent` are STRIP_KEY at export per §19.4 regardless). The dormant `image_upload.blocked` event type is unaffected (reserved for the DEBATE.2 image-lifecycle emit).
...
**Moderated-object immutability (ADR-0028).** The image bytes moderation classifies are immutable between moderation and render: the participant upload uses a write-once conditional PUT (`If-None-Match: *`, §12.3), so the object cannot be swapped after it passes the gate. This is the precondition that makes the OpenAI verdict binding on the bytes render later serves; without it, a `pass` would not guarantee the served bytes were the moderated bytes (AUDIT.1 A1; SPEC.1 §16.5). A pre-moderation `HeadObject` verify runs outside any transaction and fails closed (§12.2 step 5 / §12.3).
...
A `flagged:true` OpenAI verdict that maps to **no** category (empty/renamed category map — snapshot drift) is treated as malformed and **fails closed** (503 `moderation_unavailable`), never `pass` — the gate admits nothing on an uninterpretable verdict (AUDIT-FIX-A21; realises SPEC.1 §16.5).
...
**Track A degrade mode (HARDEN.5 trigger).** SPEC.1 §14 F-MOD-1 (auto-ban on Track A) and §14 preamble both label the auto-ban as `provisional` pending Aug 15–31 sample-content testing. If HARDEN.5 surfaces unacceptably high false-positive rates, Track A degrades to **flag-only mode**: content blocked, `mod_actions` written, user **not** banned, admin bans manually via SPEC.1 §15 F-ADMIN-4. **ADR-0021 removed the held queue this clause originally assumed flag-only items would land in for manual ban; HARDEN.5 must re-home flag-only Track A items into the reactive live-review surface (F-ADMIN-4), not a held queue.** The degrade decision is owned by HARDEN.5 and ratified via a follow-up ADR or HARDEN.5 close-out memo at that time.
...
**Distinction from §10's moderation reservation.** The 10-second Redis intent-reservation key (per §10 / ADR-0014) on `mod:reserve:{user_id}:{market_id}:{idempotency_key}` is structurally distinct from the idempotency cache key (per this section) on `idem:{user_id}:{key}` (user-scoped since ADR-0044, S-7 — was `idem:{key}` alone through S-7, and the two keys are now both user-scoped for the same reason: no client-controllable path to another user's slot). The reservation guards the in-flight window between cache miss and cache write and holds for 10 seconds; the idempotency cache replays completed responses for 24 hours. Both consume the same `Idempotency-Key` header from the client request but on disjoint Redis key spaces — and the idempotency cache's tuple (`{user_id, key}`) remains a strict subset of the reservation's (`{user_id, market_id, key}`), so the idempotency gate still fires first (ADR-0044's namespace invariant). The reservation never sees the cached response; the cache never sees the reservation state. Both fail closed; both emit §17 alarm 6 on Upstash unreachable (sub-IDs 6a + 6b respectively per §17.2 alarm-6 sub-table).
...
5. **Pre-moderation object verify (ADR-0028):** before moderation runs, `verifyUploadedObject` issues one `HeadObject` on the uploaded key — fail-closed on oversize (`error_image_oversize`), missing object (`error_storage_object_missing`), or R2-unavailable (`error_storage_unavailable`) — and captures the ETag + real byte size for the step-6 pass-branch `image_upload.committed` event; the bet transaction never opens if the verify fails. **Server runs full §11 handler stack including §10 multimodal moderation.** The moderation step calls `precommitModerate()` with a multimodal input array (text + image_url with a 60-second signed R2 read URL minted at §12.4); OpenAI omni-moderation-2024-09-26 per ADR-0014 §10.
6. **Branch on verdict:**
...
Per §5.1's inventory; twenty-two are dataset-relevant. **Four** tables are excluded from the dataset inventory entirely: the two pg_cron operational tables `watermark_state` + `cron_alarms`, the AUDIT-FIX-B3 idempotency backstop `bet_receipts` (ADR-0031 — an operational per-request receipt whose `result` content is fully derivable from `events` + `pools` (`newPrice = getPrices(post-trade reserves)`), so it carries no independent research signal), and the UI-A6 convenience table `bookmarks` (ADR-0032 — private per-viewer saved pointers at other authors' comments; Bucket C, no thesis/research signal). **Of those twenty-two: sixteen ship; five do not (operational / privacy-sensitive); ONE — `lots` — is deliberately UNDECIDED (row 22).**
...
**Shipped: 16 tables; not shipped: 5; undecided: 1 (`lots`).** Shipped = the 9 Bucket-A audit tables (`events`, `dharma_ledger`, `bets`, `comments`, `resolution_events`, `payout_events`, `mod_actions`, `admin_events`, `user_events`) + 2 Bucket-B (`identity_pool`, `image_uploads`) + 4 current-state-context Bucket-C (`markets`, `pools`, `positions`, `market_media`) + `users` (PII-stripped per §19.4). Not shipped = `system_state`, `sessions`, `accounts`, `verifications`, `admin_sessions` (operational / privacy-sensitive). The two pg_cron operational tables (`watermark_state`, `cron_alarms`), the `bet_receipts` idempotency backstop (ADR-0031), and the `bookmarks` convenience table (ADR-0032) are not part of the dataset inventory at all. `lots` (ADR-0039) IS part of the inventory and is the one row whose ship/not-ship answer is open; it is counted in neither the 16 nor the 5 until SPEC.1 G3 rules, precisely so that an unanswered question cannot be mistaken for a settled NO by arithmetic.
...
Per ADR-0016 D6 + 3-E A1: the **nine** PII columns below are **dropped** (removed from the released schema) rather than pseudo-anonymized via hash. Strip-not-hash is the chosen treatment because:
...
**The nine PII columns dropped at export:**

| # | Column | Source table | Treatment |
|---|---|---|---|
| 1 | `email` | `users` | Removed from released schema (column does not appear in dataset) |
| 2 | `google_id` | `users` | Removed |
| 3 | `name` | `users` | Removed (Google display name) |
| 4 | `image` | `users` | Removed (Google avatar URL) |
| 5 | `tos_acceptance_ip` | `users` | Removed |
| 6 | `tos_acceptance_user_agent` | `users` | Removed |
| 7 | `r2_object_key` | `image_uploads` | Removed |
| 8 | `metadata.ip` | All audit tables (`user_events`, `admin_events`, `mod_actions`, `events`) | Removed at the JSONB-key level (subset of the seven-field metadata set per §3.7) |
| 9 | `metadata.user_agent` | All audit tables | Removed at the JSONB-key level |

⚠ **`users.pfp_filename` is NOT in that table, and must not be dropped.** It sat inside it until the 2026-08-28 pass, under a title reading *"the ten PII columns dropped at export"*, while its own treatment cell read *"Released as-is; H2-erased rows release as NULL"* — a row that says it ships, listed in a table whose title says it does not. **It ships** (`NULL_IF_ERASED`, Appendix B.1): it is the identity-pool join key, and dropping it destroys the `users` ↔ `identity_pool` join the dataset needs. Its only PII-adjacent behaviour is that H2 erasure nulls it, which is the `NULL_IF_ERASED` treatment and not a strip. It is stated here, outside the table, so that the table's title is true of every row inside it — a row-count in a heading is a claim a reader checks against the rows, and one that disagreed with its own contents is how the next implementer strips a column that ships.

⚠ **Row numbers in this table are load-bearing** — §19.4.1 and `docs/runbooks/dataset-release.md` both cite it by row number, and both citations were stale before this pass (they were written against the ten-row version, and were wrong about that one too). The `#` column is explicit so a future citation can be checked rather than counted.
...
### §19.4.1 Per-payload-key SHIP rules for `events.payload`

> ⚠ **This section INVERTED on 2026-09-01 (DATASET.3, ruling E). It was titled *"Per-payload-key STRIP rules"* and named, per event type, the keys to REMOVE — everything else shipping. It now names, per event type, the keys that SHIP — everything else dropped, unread.**
>
> **Why.** `events.payload` is a `jsonb` column. Its shape is open: anyone adding an event type, or adding a key to an existing type's payload, extends it. A deny-list therefore ships every key nobody has named, and *"nobody has named it"* is the state every new key starts in.
>
> The same defect was found three nights running and patched three times by extending a list: §19.5's six FK paths against Appendix B's ~14; `comment.placed`'s payload rebuilding, by a shorter route, the association `image_upload.committed.commentId` had just been ruled to protect; and a nested key under a name on no list at all (`payload.client.remoteAddr`) which is **unstripped, unharvested, unmatched by value and unmatched by key** — four layers quiet on one input, with the build reporting success.
>
> **The list was the defect.** §19.3 already had this right one level up: a `pgTable` classified as neither shipped nor not-shipped nor undecided fails the build rather than defaulting either way. This extends that property inward, from tables to keys.

The `events.payload` JSONB column ships **only the keys this table declares**, per event type. Every other key is dropped at export, at any nesting depth and through arrays, without being read. A key's absence from this table is not an oversight to be corrected later — it is the default, and it is the safe one.

Audit trails are exhaustive at the runtime emission layer by design (INV-4 + ADR-0005 sync-target rule); export-time selection handles the privacy boundary. Two separate concerns, two separate layers. Runtime emission MUST NOT pre-strip — full payload fidelity is required for in-database forensics + admin replay.

**Depth.** The declaration is a TREE, not a flat list. A leaf entry ships a SCALAR; an object or array is declared by nesting, and an array applies its child declaration to every element. **Reaching a container under a leaf entry is a contract gap and fails the build** — "ship this subtree unread" must be unsayable, because it is precisely how a key nobody has reviewed rides out inside a container somebody did.

**What the inversion made unnecessary.** The deny-list needed a `m/` vs `u/` R2-key namespace carve-out inside the strip: it banned the key NAME `key` globally and then had to re-admit the one place this table ships it. That rule had to hold in three places at once (the strip, the secret harvest, the egress assertion) and any one of them disagreeing was a guaranteed total build failure. Under a SHIP declaration the question does not arise — `market.created` declares `media[].key` and `image_upload.*` declares no `key` at all, so the outcome is identical and decided by the DECLARATION rather than by inspecting the value. **The namespace rule remains load-bearing in the harvest and the assertion**, which still reason about key names: a `m/` key must not be harvested as a secret, because Appendix B.16 ships that identical string as a `market_media` column and the build would otherwise fail on a value it published itself.

**An event type that ships NOTHING declares an empty set explicitly.** Silence and "ships nothing" must not share a representation, or a reader cannot tell an answered question from an unasked one — and, since the inversion, an unasked one exports a blank payload rather than a leaked one. That is the safe direction and still the wrong artifact: a whole event type's research content silently absent from an append-only corpus that cannot be corrected after publication. A missing entry therefore fails the build.

| event_type | SHIP keys | Rationale |
|---|---|---|
| `user.tos_accepted` | `tosVersionHash`, `privacyVersionHash` | Which version of the terms was accepted, without who accepted it. `userId` is withheld (`aggregate_id` is already PSEUDO per §19.5; a raw payload copy re-identifies by cross-join); `ip` / `userAgent` are PII per §19.4 **rows 5–6** and redundant with the already-stripped `users.tos_acceptance_*` columns |
| `user.oauth_signed_in` | — (nothing) | The payload is `userId` + `googleId` + a `provider` literal that is `"google"` on every row. The first two are identity; the third carries no signal a researcher can use and **shipped under the deny-list only because nobody had named it**, which is the inversion's argument in one key |
| `user.otp_signed_in` | — (nothing) | `userId` + `email`, both identity |
| `user.pseudonym_assigned` | `pseudonym`, `pfpFilename` | The public face, which is the join key the dataset is built around. `userId` withheld |
| `user.signed_out` | — (nothing) | `userId` only |
| `image_upload.sign_requested` | `uploadId`, `contentType`, `byteSize` | Upload shape and size are research signal. `userId` withheld; `key` is an R2 object key and embeds the uploader's `users.id` per SCAFFOLD.15 §Q9 (`u/<userId>/<uploadId>.<ext>`) |
| `image_upload.committed` | `uploadId`, `etag`, `byteSizeActual` | ⚠ **`commentId` does not ship** (2026-08-28 ruling, restated under the inversion). Appendix B.6 withholds a reactively-removed comment's `image_uploads_id`, and this key reconstructs exactly that association from the other side. **Unconditional rather than conditional-on-removal**, because a conditional leaks the condition: presence or absence of the key would itself be the removal flag, on every row, in an archive that already ships `mod_actions`. Under the deny-list this needed an explicit STRIP entry; under the allow-list it needs a line NOT to be written, and the safe outcome became the cheap one |
| `image_upload.blocked` | `uploadId`, `modVerdict`, `reasonCategory` | The moderation outcome is research signal; `userId` withheld |
| `image_upload.orphaned` | `uploadId` | The row id. `key` withheld per the namespace rule |
| `moderation.blocked` | `reason`, `banned`, `uploadId` | Block reason code, track-A auto-ban boolean, and the `image_uploads` join key (`null` on the text-only branch). Actor identity ships via `metadata.user_id` (PSEUDO per §19.5) (AUDIT-FIX-B5 emit site) |
| `admin.signed_in` | — (nothing) | `sessionId` is the admin cookie value and `ip` is the admin's; both withheld as defense-in-depth beyond the `BREAK_GLASS.md` pre-freeze rotation. Nothing else is on the payload — and the empty set says that was decided, not overlooked |
| `admin.signed_out` | — (nothing) | Cookie value only |
| `dharma.credited` | `amount`, `creditedForDate` | The Daily Credit accrual, which is the issuance record. `userId` withheld — `aggregate_id` already carries it as a pseudonym (ENGINE.12 emit site) |
| `dharma.granted` | `amount` | The one-time genesis issuance (ENGINE.13 emit site) |
| `bet.placed` | `betId`, `marketId`, `side`, `stake`, `shares`, `price`, `commentId`, `parentCommentId` | The K_eff(t) derivation core per §19.6. Only the actor's identity is withheld (ENGINE.8 emit site) |
| `bet.sold` | `betId`, `marketId`, `side`, `sharesSold`, `proceeds`, `price` | The sell leg of the same core (ENGINE.8 emit site) |
| `comment.placed` | `commentId`, `betId`, `marketId`, `side`, `parentCommentId`, `bodyLength` | ⚠ **`uploadId` NO LONGER SHIPS** (2026-09-01, ruling S5). It is the other half of the recovery path `image_upload.committed.commentId` was ruled to close: this payload carries `commentId` too, so shipping `uploadId` rebuilds the association Appendix B.6 withholds **in one row, with no join** — a shorter path than the one that ruling closed. Closing one end and leaving the other is not a partial mitigation; it is none. Nothing a researcher can use is lost: `comments.image_uploads_id` (B.6) already carries the link for every comment that is not withheld. The comment `body` + `side_at_post_time` are not payload keys; they ship via the `comments` table per **Appendix B.6**, and `body` is `WITHHELD_IF_REMOVED` rather than a bare SHIP (ENGINE.8 emit site) |
| `market.created` | `marketId`, `resolutionDeadline`, `media[].key`, `media[].displayOrder`, `media[].isDefault`, `mediaVideoUrl` | ⚠ **The one nested declaration, and it is load-bearing.** `media` is an array of objects and `schemas.ts` declares it `.min(1)`, so every market carries one. Its `key` lives in the `m/<marketId>/` namespace — operator-curated, no user id embedded, and Appendix B.16 ships the identical string as a `market_media` column. Declaring the three child keys ships them **without** shipping a fourth key somebody adds to that object later (ENGINE.14 emit site; media extended at MEDIA.1 OD-2) |
| `market.opened` | `marketId`, `seedAmount` | The seed is public CPMM state — it IS the reserves (ENGINE.14 emit site) |
| `market.closed` | `marketId` | (ENGINE.14 emit site) |
| `market.resolving` | `marketId` | Outcome and evidence live on `resolution_events`, never duplicated here (R-9.1) (ENGINE.9 emit site) |
| `market.resolved` | `marketId`, `winningSide`, `resolutionNote`, `poolUnwindAmount` | Settlement core for K_eff derivation (R-9.5e) (ENGINE.9 emit site) |
| `market.corrected` | `marketId`, `correctsEventId`, `correctedWinningSide`, `resolutionNote` | The corrections chain is thesis-core audit (ENGINE.9 emit site) |
| `market.voided` | `marketId`, `voidReason`, `poolUnwindAmount` | (ENGINE.9 emit site) |

**Timestamp precision.** `created_at` and every other `timestamp with time zone` column export at **full stored precision — six fractional digits**, in ISO-8601 UTC form (`2026-11-06T12:00:00.123456Z`). ⚠ This is stated because getting it wrong is invisible: Postgres stores microseconds and a JavaScript `Date` holds milliseconds, so a reader that materialises timestamps through the driver's default parse silently discards three digits, and two events 200 µs apart export as the same instant — in a corpus whose subject is the order things happened in. The precision must be preserved **in the query** (a text cast), because by the time a value reaches any formatting code it has already been floored (2026-09-01, ruling S6).

**Adding a new event_type or modifying a payload shape** is a same-commit amendment to this table plus the schema declaration at `src/server/events/schemas.ts` plus the per-site emit call. ⚠ **Adding a KEY to an existing payload is equally an amendment**, and this is the case the deny-list could not see: a new key needs a decision here or it silently does not ship. The export pipeline derives its per-payload-key selection from this table, and binds it to `eventPayloadSchemas` so that a schema key which is neither declared here nor recorded as deliberately dropped fails CI.
...
§22 owns the *consolidated index of architectural decision records* for the experiment-phase build — the ADRs at `docs/adr/0001-…md` and `docs/adr/0003-…md` through `docs/adr/0039-…md`, their accepted / superseded / in-flight status, the task → ADR-NNNN mapping that gates each ADR, and the cross-reference invariant that every ADR reference in SPEC.2 resolves to an existing ADR file. This §22 sits at the *index layer* — the ADRs themselves are immutable substance per ADR convention; this section catalogues their status and exposes the gating map without restating their decisions.

The inventory is **38 ADRs** (was 14 at v0.3-draft; ADR-0017/0018/0019 folded at SYNC.7; ADR-0020 + ADR-0021 folded at the 2026-06-18 moderation amendments — ADR-0020 added as superseded; ADR-0022 (prod-migration-strategy + drift-guard) + ADR-0023 (participant-shell topology) + ADR-0024 (deploy pipeline + migration sequencing) folded at the 2026-06-25 deploy-pipeline catch-up, ADR-0024 partially superseding ADR-0022's `/api/health` drift-comparison method; ADR-0025 (debate `.md` export) folded 2026-06-29; ADR-0026 (market media) folded 2026-06-30; ADR-0027 (admin market-media direct upload — no moderation) folded 2026-06-30, superseding ADR-0026 §D4; ADR-0028 (moderated-image byte-identity binding) folded at AUDIT-FIX-A1 2026-07-03; ADR-0029 (dharma-ledger total-order `seq`) + ADR-0030 (TRUNCATE rejection on append-only tables) folded at AUDIT-FIX-B2 2026-07-04; ADR-0031 (durable bet receipts + terminal error mapping) folded at AUDIT-FIX-B3 2026-07-05; ADR-0032 (bookmarks) folded at UI-A6 2026-07-21; ADR-0034 (viewer-scoped debate reads live outside the export-bound view model) folded at BOOKMARK-ADD-WIRE 2026-07-30; **ADR-0033 (OTP delivery boot guard + optimistic-request semantics) admitted to the index at BOOKMARK-ADD-WIRE 2026-07-30 as an INDEX ROW ONLY** — ruling G1: its substance is folded into neither SPEC.1 nor SPEC.2, and that fold remains a docketed follow-up owned by its own task; the row exists so this inventory matches the ADR files on disk; **ADR-0001 (license choice) admitted to the index at BC.2 — it was authored all along, this section's prior claim that it had "no ADR file" was itself incorrect**; **ADR-0035 (guarded staging reset) + ADR-0036 (Vitest-context operational runners) + ADR-0037 (onboarding-deck seen-marker cookie) + ADR-0038 (scale target 100k) folded at SYNC-4 2026-08-20** — the `N5` backlog these four had accumulated in, discharged as the normative §22.1/§22.5 rebuild that row always said it required, rather than as four running one-row corrections; **ADR-0039 (per-argument lot accounting) folded at PHASE-0 2026-08-21**, same-commit with migrations 0025/0026 rather than accumulated). One earlier-numbered slot (ADR-0002) remains genuinely unauthored — its number was never used; whatever brand/repo-structure topic it was once earmarked for at FOUND.7/FOUND.8 lives directly in TRADEMARK.md + the repo structure, not in the ADR registry. The numbering runs 0001, (0002 skipped), 0003–0039. The ADR file numbering is the canonical inventory.

Of the 38 ADRs, **35 are accepted** (ADR-0001, ADR-0003 through ADR-0008, ADR-0010 through ADR-0011, ADR-0013 through ADR-0019, ADR-0021 through ADR-0039), **2 are superseded** (ADR-0009 by ADR-0017; ADR-0020's held queue by ADR-0021), and **1 is in flight** (ADR-0012). ADR-0022 remains **accepted** — its prod-apply path stands; only its `/api/health` drift-comparison method and health-env-var line are superseded by ADR-0024 (a scoped partial supersession that does not flip 0022's status). Per the §22.2 in-flight carve-out, SPEC.2 v1.0 locks with ADR-0012 in flight; design.md finalization triggers a same-commit SPEC.1 + SPEC.2 minor-version bump (v1.0 → v1.1) without re-opening PRECURSOR.4. SCAFFOLD.* tasks that do not consume design.md proceed in parallel during ADR-0012's in-flight window.

### §22.1 The 38-row index
...
|---|---|---|---|---|---|
| **0001** | FOUND.3 | `0001-license-choice.md` | Use AGPL-3.0-or-later for the experiment-phase repo | accepted | 2026-04-29 |
...
| **0015** | SPEC.16 | `0015-rate-limit-idempotency.md` | Rate-limit + idempotency (Upstash Redis sliding windows; SETNX-with-pending-sentinel; Stripe-style key envelope; asymmetric fail-open/closed posture) — carries two in-place **Patch records**: AUDIT-FIX-B3 (2026-07-05: ownership-checked never-throws release + completion-write alarm half, pairing ADR-0031) + AUDIT-FIX-B7a (2026-07-06: A14 transport bounding reconciled) | accepted | 2026-05-07 |
...
| **0020** | Admin-dashboard ideation | `0020-decoupled-content-removal.md` | Decoupled content removal & three-option held queue — **superseded by ADR-0021** (held queue removed; the content-removal-vs-user-ban decoupling principle is retained by 0021) | superseded | 2026-06-18 |
| **0021** | DEBATE.7 | `0021-reactive-moderation-no-held-queue.md` | Reactive moderation (no held queue) — **supersedes ADR-0020**; gate returns block/pass, admin reviews live content reactively (Remove/Ban), no moderation action touches a position, text-only `sexual/minors` carve-out blocked-then-surfaced. Amends ADR-0014 Track B consequence only | accepted | 2026-06-18 |
| **0022** | infra (staging-prov. prep) | `0022-prod-migration-strategy-and-drift-guard.md` | Production migration application strategy + schema-drift guard (per-migration-tx `db:migrate:prod`; env-fragment guard; status-only `/api/health` drift field; timestamp+count `db:check-drift`) — **`/api/health` drift method + health-env-var line superseded by ADR-0024**; prod-apply path + `check-drift` timestamp+count stand | accepted | 2026-06-20 |
...
| **0026** | Market media (spec lane) | `0026-market-media.md` | Market Media — admin-set asset model: new `market_media` table (Bucket C, **no `user_id`**); third R2 bucket arm `market-media` (`m/<marketId>/` namespace, isolated credentials); reference-model pick-from-pool via `comments.market_media_id` + a not-both-set CHECK; `markets.media_video_url` outbound YouTube link (new tab, not embedded/self-hosted); admin-context upload moderation, pick path pre-vetted by construction. Amends SPEC.1 §15 F-ADMIN-1 + §8 F-COMMENT-3 + a new §9 display subsection; data model in §5; admin upload route + migration **build** deferred to §4; admin-upload moderation superseded by ADR-0027 | accepted | 2026-06-30 |
| **0027** | MEDIA.1 (spec lane) | `0027-admin-market-media-direct-upload.md` | Admin market-media direct upload — no moderation; supersedes ADR-0026 §D4. Market-media is operator-curated trusted content written directly; the participant UGC moderation pipeline is unchanged. | accepted | 2026-06-30 |
| **0028** | AUDIT-FIX-A1 (+A10) | `0028-moderated-image-byte-identity-binding.md` | Moderated-Image Byte-Identity Binding — participant image uploads are write-once (`If-None-Match: *` conditional PUT) so moderated bytes ≡ rendered bytes by construction; a pre-tx fail-closed `HeadObject` verify wires the A10 real-byte size cap (`error_image_oversize`) + records the ETag forensic fingerprint into `image_upload.committed`. Amends ADR-0014; admin path excluded (ADR-0027). Closes AUDIT.1 A1 (CSAM swap window) + A10. | accepted | 2026-07-03 |
| **0029** | AUDIT-FIX-B2 | `0029-dharma-ledger-total-order-contract.md` | Dharma-ledger total-order contract — `dharma_ledger.seq BIGINT GENERATED ALWAYS AS IDENTITY` + unique `(user_id, seq)` gives the ledger the total order `created_at`/`id` cannot (tx-frozen `now()`; sub-ms-random `uuidv7()`); `readLatestBalance` orders by `seq DESC`. Refines ADR-0016's cross-row-ordering guidance *for `dharma_ledger` only*. Closes AUDIT.1 A2 | accepted | 2026-07-04 |
| **0030** | AUDIT-FIX-B2 | `0030-truncate-rejection-append-only.md` | TRUNCATE rejection on append-only tables — statement-level `BEFORE TRUNCATE` guards (new `enforce_bucket_a_no_truncate()` + Bucket-B analog; 25 triggers, migration 0021) close the row-trigger hole (TRUNCATE fires no row-level triggers); extends the §6 contract (ADR-0005/ADR-0008) with a forward obligation on every future protected table. Owner-privilege residual (app role owns the tables — guards are droppable by an owner-level attacker) recorded honestly + parked → the OQ-2 role split. Closes AUDIT.1 A20 (accident / blast-radius class) | accepted | 2026-07-04 |
| **0031** | AUDIT-FIX-B3 | `0031-durable-bet-receipts-and-terminal-error-mapping.md` | Durable bet-request receipts + terminal error-mapping contract — Bucket-A `bet_receipts` (UNIQUE `idempotency_key`; the LAST write inside the W-1 tx) backstops the Redis idempotency window durably (I-IDEM-ONCE-001): endpoint durable pre-check + 23505 replay catch, so a committed bet/sell is never re-executed and never masked as a 500; terminal errors map to cacheable 4xx (`insufficient_shares` oversell pre-check; `error_position_conflict` 503). The release-semantics half (ownership-checked never-throws release + completion-write alarm) scopes ADR-0015 via its in-place Patch record, same commit. Closes AUDIT.1 A3 + A4 + A9 | accepted | 2026-07-05 |
| **0032** | UI-A6 | `0032-bookmarks.md` | Bookmarks — private per-viewer saved pointers at other authors' comments (convenience state, no soulbound/thesis surface): a Bucket-C `bookmarks` table (migration 0024; `UNIQUE(user_id, comment_id)`; both FKs `ON DELETE restrict`), two idempotent Server Actions (add/remove; app-layer others-only guard, D-3), and a fully-batched cross-author read rendering each bookmarked author's Đa/Đb + marker in the Profile surface's forced-visitor mode (FI-2 same-source cross-surface identity, §4.5). Event-free (Option 1 — EVENT_TYPES unchanged at 24); zero invariant surface. The cross-surface add-icon wiring is the named follow-on BOOKMARK-ADD-WIRE (pre-`TESTING.0` gate). | accepted | 2026-07-20 |
| **0033** | AUTH-OTP-DELIVERY | `0033-otp-delivery-boot-guard-and-optimistic-request-semantics.md` | OTP delivery — sandbox-from boot guard, optimistic-request error semantics, and the structural send-suppression constraint. **Index row only** (BOOKMARK-ADD-WIRE ruling G1): this ADR's substance is folded into neither SPEC.1 nor SPEC.2, and that fold remains a docketed follow-up owned by its own task. The row exists so the §22 inventory arithmetic matches the ADR files on disk. | accepted | 2026-07-22 |
| **0034** | BOOKMARK-ADD-WIRE | `0034-viewer-scoped-debate-reads.md` | Viewer-scoped debate reads live outside the export-bound view model — per-viewer state (the bookmarked / own comment-id sets) is composed in `loadViewerMarketContext` BESIDE `loadDebateView`, never on `DebateViewModel`, because ADR-0025 binds the public debate `.md` export to that type and ADR-0032 D-8 excludes `bookmarks` from the public dataset entirely; putting viewer state on the export-bound model would place private state in a public export's input type. Overrides the `docs/plans/UI-A6.md` §11 forward note. Keeps `loadDebateView` viewer-independent, so removal-masking stays structurally identical for every viewer. | accepted | 2026-07-30 |
| **0035** | STAGING-PARITY Slice A | `0035-guarded-staging-reset.md` | Guarded Staging Reset via Owner-Privilege Trigger Disablement — the staging reset disables the Bucket-A/B append-only triggers under owner privilege for the duration of a guarded truncate, then restores them; five-guard contract (intent · target · environment · live connection · post-run verification) with the G-5 intent token added by the Addendum. Gates STAGING-PARITY Slices B–D, POLISH.1–.8 and SP-2. Extends the §6 append-only contract with an explicit, audited exception rather than a hole | accepted | 2026-08-05 |
| **0036** | STAGING-PARITY Slice A | `0036-vitest-context-operational-runners.md` | Vitest-Context Runners for Operational Staging Tasks — `tests/staging/` holds operational artifacts, **not tests**: they borrow the Vitest harness for module resolution only, point at the LIVE staging database, and are excluded from the default `vitest run` by config so no bare run can reach one. Primitive 3: anything that writes a row or moves Dharma is never mocked; only the HTTP/cookie shell may be. Gates Slices B–D | accepted | 2026-08-05 |
| **0037** | O1-DECK | `0037-onboarding-deck-seen-marker-cookie.md` | Onboarding-Deck Seen-Marker: Browser Cookie, Not a User Column — the first-login deck's seen-marker is a browser cookie rather than a `users` column, so the marker carries no soulbound or thesis surface and adds no write path to an append-only table. Frames SPEC.1 §21.9 (the deck) against §13 (session model); consumes ADR-0004's participant session | accepted | 2026-08-18 |
| **0038** | SYNC-3 | `0038-scale-target-100k.md` | Scale target: 100,000 signups and 2,000,000 page loads — lifts ADR-0006's `$300`/`$500` per-month cost ceiling for the live window (it returns at archive); sizing is decided from measurement, never estimate; authorises Supabase compute through Large, transaction-mode pooler, Upstash Pro, Resend Scale + dedicated IP, quota headroom and 14-day PITR without further ADR. A read replica, any single-region change and any moderation-pipeline change still require a decision record. **Scopes ADR-0006 §Cost and §Sizing**; region, failure-mode profile and single-service topology untouched | accepted | 2026-08-19 |
| **0039** | LOTS-1 | `0039-per-argument-lot-accounting.md` | Per-argument lot accounting (lot = bet = argument; sell is per-lot) — a holding stops being one aggregate number with nothing beneath it. ONE `lots` row per `bets` row (R1), so a participant's Đa decomposes into the arguments that produced it and a partial sell has a rendered quantity that can close the gap; before this, argument cards showed the frozen Bucket-A `bets.stake` while Đa was multiplied by a surviving fraction rendered nowhere, so after any partial sell the badges were **guaranteed** never to reconcile. Mints the invariant-class rule **R2** — `Σ lots.surviving_shares == positions.quantity` per `(user, market, side)`, tested by `I-LOT-SUM-001` — while keeping `positions` the aggregate authority: attribution never vetoes it, so a sell against absent or insufficient lots proceeds and allocates what it can rather than trapping Dharma behind a disagreement between two of our own tables. **R8**: no back-fill, as a property of migration 0025 rather than a convention around it. **R9**: a lot never grows and Sold is permanent — DIRECTIONAL rather than append-only, and split across two layers rather than held by one: the seven CHECKs bound RANGE only, so monotonicity is application-enforced (`sellFromLot` is the sole writer) while permanence is the row-level `BEFORE DELETE` reject in migration 0026 — either way not a bucket reclassification (§5.1 row 26, §5.2). *(Corrected at MERGE-1 — this row had credited the CHECKs with the whole of R9.)* Position-level sells allocate **pro-rata**, the one mechanic R1–R10 does not state, chosen because it changes nothing observable: `Σ(basis × f) = (Σ basis) × f`, whereas FIFO or LIFO would assert an ordering claim the participant never made on the control that names no argument. Widens F-BET-3's reachable error set by one (404 `lot_not_found`) | accepted | 2026-08-21 |
...
| Concern | Source-of-truth file |
|---|---|
| Per-ADR substance | `docs/adr/<NNNN>-<slug>.md` (38 ADRs — 37 ADR files + ADR-0012 in-flight; 35 accepted + 2 superseded + 1 in-flight; ADR-0001 admitted at BC.2; ADR-0034 folded at BOOKMARK-ADD-WIRE; ADR-0035/0036/0037/0038 folded at SYNC-4; ADR-0039 folded at PHASE-0; ADR-0033 index-row only — its substance is folded into neither spec, a docketed follow-up) |
...
| Per-task status (not_started / in_progress / blocked / done) | `tracker_v15.html` (current) |
| Per-task ordering within phase | tracker HTML (`tracker_v15.html`) |
...
ADRs consumed by §23: ADR-0002 remains out of inventory (per §22.1 — no ADR file; substance lives in TRADEMARK.md + repo structure). ADR-0001 is in-inventory (§22.1) but, like the operational ADRs 0020 onward, sits outside §23's phase-distributed gating relationships — it's a foundational repo decision, not a SPEC.x/SYNC phase gate. ADR-0003 through ADR-0019 in their phase-distributed gating relationships per §23.1 (ADR-0017/0018 consumed by the DEBATE row; 0017/0018/0019 minted under SYNC).
...
| `src/db/schema/bets.ts` | `bets`, `positions`, `bet_receipts` schemas | §5.5 |
| `src/db/schema/lots.ts` | `lots` schema (LOTS-1 — Bucket C; 1:1 with `bets` via the UNIQUE `lots_bet_id_uq` + FK; FKs → `users`/`markets`/`bets`, each indexed, `ON DELETE restrict`; the seven R9 monotonicity/positivity CHECKs; ADR-0039). Its own domain rather than a fourth table in `bets.ts` — it carries its own pure core, error sentinels and invariant-class rule (§5.3) | §5.1, §5.3, §5.5 |
| `src/db/schema/bookmarks.ts` | `bookmarks` schema (UI-A6 — Bucket C; `UNIQUE(user_id, comment_id)`; FKs → `users`/`comments`, both indexed, `ON DELETE restrict`; ADR-0032) | §5.1, §5.5 |
| `src/db/schema/comments.ts` | `comments` schema (reply-as-bet: `bet_id` deliberately nullable — 1:1 with the comment-bearing bet via `bets.comment_id NOT NULL` instead, INV-1; `parent_comment_id` post/reply discriminator, `side_at_post_time`; no `stake_at_post_time`, no `friendly_fire_events`) | §5.1, §5.5 |
...
| `src/server/lots/compute.ts` | The lot pure core (ADR-0039 D-1/D-3/D-4): `mintLot` · `sellFromLot` · `allocateProRata` · `sumLots`. No IO, no clock, no randomness; every quantity a canonical `NUMERIC(38,18)` string. Two arithmetics deliberately — share allocation in exact BIGINT units of 1e-18 (a partition problem: Σ of the parts must equal the whole to the last place or R2 is false), basis reduction in `CpmmDecimal` (a genuine division) | §5.1 |
| `src/server/lots/persist.ts` | The only `lots` write path: `mintLotForBet` (one lot per bet, no `onConflictDoUpdate` — a duplicate mint is a caller bug and its 23505 must abort the W-1 transaction) + `planLotSale` / `applyLotSale` (plan then write, so a per-lot oversell is a refusal rather than a rollback that happens to end correctly). Takes a `DbTransaction`, never the top-level handle, so passing the wrong one is a compile error | §5.1, §14.4 |
| `src/server/lots/errors.ts` | `LotOversellError` / `LotInputError` — module-local sentinels (`extends Error`, not `BetProductError`), mapped explicitly onto the §4.4 wire envelope by `toWireError`; without those maps they reach the client as an UNCACHED 500 and a deterministic failure is retried forever | §4.4, §15.4 |
| `src/server/lots/basis.ts` | Đa read helper — `Σ lots.surviving_basis` per holding (ADR-0039 D-4) | §5.1 |
...
| `src/server/debate-view/viewer-context.ts` | `loadViewerMarketContext` + the `ViewerMarketContext` DTO — the viewer-session read composed BESIDE `loadDebateView` in the `/m/[slug]` RSC, never inside it (ADR-0034). One transaction, SELECTs only, READ-ONLY BY LAW: no ledger append, no accrual write, no cursor write, no `events` row. Returns `position` (`computeSell(quantity).proceeds` basis, FI-2) · `balance` · `spendableToday` (the pure Daily-Credit preview, never a call into `accrueDailyCredit`). The two additive, market-scoped, ID-ONLY bookmark arrays this DTO carried at BOOKMARK-ADD-WIRE were removed at ADR-0040. | §4.2, ADR-0034 |
...
| `src/server/identity-pool/consume.ts` | Pseudonym pool consumer invoked from the Better Auth `user.create.before` hook (`SELECT ... FOR UPDATE SKIP LOCKED` in its own transaction; `user.pseudonym_assigned` emits post-commit per §7.5.1) | §3.5, §3.7 |
| `src/server/moderation/precommit.ts` | `precommitModerate()` orchestration (OpenAI omni-moderation + Redis intent reservation; ADR-0027 — the admin market-media upload path is **not** moderated (operator-curated content) and does not call this file) | §10 |
...
| `src/lib/ranking.ts` | Ranking model (pure TS, no IO) — per ADR-0017 + `RANKING.md` | §5.4 |
...
| `docs/specs/RANKING.md` | Ranking model companion spec (per ADR-0017, supersedes ADR-0009) | §5.4, §7.4 |
...
| `docs/adr/0001-...md`, `0003-...md` through `0027-...md` | 26 ADR files (23 accepted + 2 superseded + 1 in flight per §22.1) | §22.5 |
...
Per-column treatment for the 15 tables shipped in the 2026-11-06 public dataset release per §19.3. Each table's columns are classified into one of six treatments:
...
- **`WITHHELD_IF_REMOVED`** — column ships verbatim except for rows whose content an admin reactively removed (a `mod_actions` row with `reason = 'content_removed'` per ADR-0021); those rows release the cell EMPTY, with the row itself intact. Distinct from `NULL_IF_ERASED`: erasure is the *user's* request against their own PII and fires at the `users` row, whereas removal is a *moderator's* action against one piece of content and is recorded in a different table entirely. Both preserve the row so the join graph and the audit trail survive; they differ in who acted and on what.

**Timestamp precision — every `timestamp with time zone` column, on every table below.** Timestamps export at **full stored precision: six fractional digits**, in ISO-8601 UTC form — `2026-11-06T12:00:00.123456Z` (2026-09-01, DATASET.3 ruling S6).

⚠ **Stated here, per-column, because getting it wrong is invisible in every layer that would normally catch it.** Postgres stores microseconds; a JavaScript `Date` holds milliseconds. A reader that materialises a `timestamptz` through a driver's default parse therefore discards three digits *before any formatting code sees the value*, and two events 200 µs apart export as the same instant — in a corpus whose subject is the order things happened in. It cannot be fixed downstream of the read, and it cannot be detected by comparing two builds that both floor: the precision has to be preserved **in the query** (a text cast), and a fixture whose timestamps are round milliseconds is structurally unable to test whether it was.
...
| `idempotency_key` | text \| null | **STRIP** | ⚠ **Changed from `SHIP` on 2026-09-01 (DATASET.3, ruling S2).** The old note read *"client-generated opaque string; carries no PII"* — an **assumption wearing the clothes of a constraint**. What is stored is the raw `Idempotency-Key` request header, validated only by `^[A-Za-z0-9_-]{1,255}$` (§11 / ADR-0015): **255 bytes the participant chooses**, that moderation never sees, because the §10 gate covers comment bodies and images and not headers. A participant can place N bets each carrying a doxx, a slur, or another person's Google `sub` — all regex-valid — into `bets.csv` and every `events.csv` metadata blob, under CC-BY-4.0, permanently. Its research value is close to nil: it is an opaque replay token, not a fact about a bet. Weighed against unmoderated participant-chosen text in an artifact that cannot be un-published, the trade is not close. ⚠ Stripped from **both** surfaces in the same pass — see the `metadata.idempotency_key` rows in B.11/B.12/B.13 — because stripping it from the column and shipping it from the metadata blob one table over is the `admin_session` mistake this appendix already made once (B.13, corrected at 1.0.28) |
| `created_at` | timestamptz | SHIP | |
...
| `body` | text | WITHHELD_IF_REMOVED | Comment text content. ⚠ **Ships verbatim EXCEPT for reactively-removed comments, whose body is withheld** (2026-08-28 ruling). This row previously read `SHIP` with the note *"post-moderation; only `pass`-verdict comments exist in this table per §10"* — true of the **pre-commit gate** (§10) and not of **reactive removal** (ADR-0021), which admits the comment and takes it down afterwards by appending a `mod_actions` row with `reason = 'content_removed'` and making **zero writes to `comments`** (the table is Bucket A). So a removed body is still in this column, and a read that does not intersect the removed set publishes it. Shipping it would republish, permanently and under CC-BY-4.0, the exact content moderation removed — and because `mod_actions.reason` + `target_comment_id` also SHIP (B.10), the archive would carry a *labelled index* of which bodies those were. It would also put the two arms of one tarball in contradiction: `debates/*.md` inherits masking through `loadDebateView` per ADR-0025, so the same body would be withheld in one file and published in another. The removal remains visible as a fact — `mod_actions` still ships `reason` and `target_comment_id`, which is the honest half — what is not visible is what the comment said. CLAUDE.md §5.14 **SC-1** is the standing check that fires here: masking is a property of every code path that reads this column, not of rows |
| `image_uploads_id` | uuid \| null | WITHHELD_IF_REMOVED | FK to `image_uploads.id` for F-COMMENT-3; NULL for text-only comments. Withheld on removal alongside `body` for the same reason and by the same predicate: a removed comment's attached image is part of the removed content, and leaving the FK lets a reader join straight to the `image_uploads` row — the same leak wearing a join. ⚠ The reciprocal recovery path through `events` is closed by the §19.4.1 `image_upload.committed` rule, which strips `payload.commentId` **unconditionally**; see the note there for why unconditionally rather than on-removal |
...
| `target_market_id` | uuid \| null | SHIP | FK to `markets.id` — the market a gate-block submit targeted (a gate-block has no comment/bet row to JOIN); powers F-ADMIN-5 market search + the dashboard market filter (DEBATE.7). NULL when a comment/bet target is present |
| `reason` | mod_reason | SHIP | The action reason (DEBATE.7 / ADR-0021 §78 ∪ §84): `track_a_autoban` / `track_b_blocked` / `sexual_minors_text_blocked` (gate auto-actions) / `content_removed` / `user_banned` (reactive admin — dashboard stratum, forward-compat). NOT NULL |
...
| `blocked_text` | text \| null | STRIP | The rejected comment body for a gate-block (no comment row exists); retained for the carve-out's reactive ban-review (SPEC.1 §786) — admin-only, removed from the released dataset per §19.4 (DEBATE.7) |
| `image_r2_key` | text \| null | STRIP | Operational; per §19.4 — column removed from released schema |
| `actor_id` | text | SHIP | `'system'` for all gate auto-actions (Track A autoban, Track B block, the `sexual_minors_text_blocked` carve-out); `'admin-singleton'` for reactive Remove/Ban (dashboard stratum) |
| `created_at` | timestamptz | SHIP | |
...
| `metadata.user_id` | uuid \| null | SHIP_KEY → PSEUDO (when not NULL) | **Per B.13 — this block does not restate the rule.** NULL for an admin actor, which is every row this table is contracted to hold; PSEUDO otherwise. ⚠ This row read a bare `SHIP_KEY` until the 2026-08-28 pass, which would have published a raw `users.id` on any row an admin-actor assumption did not cover |
| `metadata.actor_id` | text | **Per B.13** | ⚠ **Deferred to B.13 rather than restated (2026-08-28 ruling).** The key name is **preserved** and only its value is rewritten, uniformly across every table carrying this key. This row previously said `SHIP_KEY` with the note `'admin-singleton'`, B.12 said *"rewritten to `actor_pseudonym`"*, and B.13 said sentinel-or-PSEUDO — **three appendix blocks, three treatments, one JSONB key.** The rename is the one that cannot work: `actor_id` is polymorphic, holding either a participant UUID or the `'admin-singleton'` / `'system'` sentinel, and **a CSV column cannot change its name per row** — so `actor_pseudonym` would have to be the header for the sentinel rows too, labelling the sentinel as a pseudonym. A per-table key name would also make the three audit tables structurally incomparable for no research gain |
| `metadata.idempotency_key` | text \| null | **STRIP_KEY** | ⚠ **Changed from `SHIP_KEY` on 2026-09-01 (DATASET.3, ruling S2)** — the same value and the same reasoning as `bets.idempotency_key` (B.5). The raw `Idempotency-Key` header, participant-chosen, unmoderated, 255 bytes. The §3.7 metadata set is seven fields; **four now ship** (`request_id`, `flow_id`, `user_id`, `actor_id`) |
...
| `metadata.user_id` | uuid | SHIP_KEY → PSEUDO | **Per B.13 — this block does not restate the rule.** Self-actor; the KEY renames to `user_pseudonym` and the value rewrites. (A rename is correct *here* and wrong for `actor_id` one row down, because `user_id` is never a sentinel — it is a participant id or NULL, so one column name is true of every row) |
| `metadata.actor_id` | uuid | **Per B.13** | ⚠ **Deferred to B.13 rather than restated (2026-08-28 ruling).** This row previously said *"rewritten to `actor_pseudonym`"* — a **rename** — while B.11 said bare `SHIP_KEY` and B.13 said sentinel-or-PSEUDO. The key name is **preserved** and only the value is rewritten; see B.11's note for why a rename cannot work on a polymorphic column in a CSV. The extra reason it is wrong specifically here: `user_events.metadata.actor_id` is **self-actor and echoes `user_id`**, so under the old rule the same identity would arrive twice in one metadata object under two different key names (`user_pseudonym` and `actor_pseudonym`) |
| `metadata.idempotency_key` | text \| null | **STRIP_KEY** | ⚠ **Changed from `SHIP_KEY` on 2026-09-01 (DATASET.3, ruling S2)** — the same value and the same reasoning as `bets.idempotency_key` (B.5). The raw `Idempotency-Key` header, participant-chosen, unmoderated, 255 bytes. The §3.7 metadata set is seven fields; **four now ship** (`request_id`, `flow_id`, `user_id`, `actor_id`) |
| `metadata.ip` | text | STRIP_KEY | PII per §19.4 |
| `metadata.user_agent` | text | STRIP_KEY | PII per §19.4 |
| `created_at` | timestamptz | SHIP | |
...
| `aggregate_id` | uuid | SHIP_OR_PSEUDO | Per-aggregate-type: `user` and `dharma_account` aggregate_ids rewrite to pseudonym; other aggregate types preserve raw UUID (a `market` / `bet` / `comment` / `image_upload` / `mod_action` aggregate_id is that table's PK and not a user at all — a blanket rule in either direction is wrong, and pseudonymize-all would throw on every one of them). ⚠ **`admin_session` aggregate_id is REDACTED** to the fixed sentinel `[redacted-admin-session]` (2026-08-28 ruling). **This row previously said it SHIPs raw** — *"defense-in-depth covered by `BREAK_GLASS.md` rotation + payload STRIP rules per §19.4.1"* — which does not survive being read next to the thing it cites: §19.4.1 **strips `payload.sessionId` from `admin.signed_in` and `admin.signed_out`**, so the same value in the same row was stripped from one field and published from another, and the STRIP rule was named as the mitigation for shipping it. §19.3 row 21 also withholds the whole `admin_sessions` table as privacy-sensitive, which publishing the id here partially undoes. There is one admin, so this identifies no participant — but it is a session identifier published permanently for zero research value against §19.6 or SPEC.1 G3, and an internal contradiction resolves toward the stricter arm. A fixed sentinel rather than a hash, per §19.4's strip-not-hash policy; a per-session opaque token would be pseudonymization of an entity that has no pseudonym, admin having no `users` row. The cost is the ability to correlate a `signed_in` with its `signed_out`, which the withheld `admin_sessions` table already made unavailable by design. `BREAK_GLASS.md` rotation stays — it is now defense-in-depth for a value that does not ship, rather than the sole control on one that does |
