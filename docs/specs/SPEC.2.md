...
**⚠ The rules below apply at ANY NESTING DEPTH, and through arrays** (2026-08-28 ruling). A `STRIP_KEY` target names a key, not a top-level key: if `userId` appears at `payload.userId`, at `payload.actor.userId`, or at `payload.items[3].userId`, every one of them is stripped. The same depth applies to the export pipeline's egress *scan* — the guard that asserts no known secret value survives must walk to the same depth the strip does, or it is checking a shallower object than the one being written.

This was a one-level walk until this pass, and the one-level walk was **correct for every payload shape that exists today** — which is exactly what made it worth changing. Depth is not a property a future author thinks to check when adding an event type; they think about *which keys* are sensitive, and the rules table asks them only that. The value-based egress guard cannot rescue the gap either, because a nested secret is never *harvested* into the forbidden-value set, so the scan does not know to look for it — the miss is silent in both layers at once. `market.created.payload.media[]` is the live shape that proves the depth is real (each element is an object with its own `key`), and it survives the strip only because of the namespace rule below, not because of its depth.

**The `m/` vs `u/` namespace rule for R2 object keys is preserved and is load-bearing.** A `key` under the `m/` prefix is market media — admin-set, no user id embedded, and it **SHIPS** (§19.3 row 16 / Appendix B.16). A `key` under the `u/` prefix embeds the uploader's `users.id` per SCAFFOLD.15 §Q9 and **NEVER** ships. A depth rule applied without the namespace rule strips a value that must ship and fails the build on the first real read.

| event_type | STRIP_KEY targets | Rationale |
|---|---|---|
| `user.tos_accepted` | `payload.ip`, `payload.user_agent` | PII per §19.4 **rows 5–6** (`tos_acceptance_ip` / `tos_acceptance_user_agent` — this cited *"row 3-4"*, which are `name` and `image`, until the 2026-08-28 pass); redundant with `users.tos_acceptance_ip` + `users.tos_acceptance_user_agent` (already STRIP) |
...
| `image_upload.committed` | `payload.userId`, `payload.key`, `payload.commentId` | `userId` / `key` per the row above. ⚠ **`commentId` is stripped UNCONDITIONALLY** (2026-08-28 ruling) — not conditionally on whether the comment was removed. Appendix B.6 withholds a reactively-removed comment's `image_uploads_id`, and this payload key reconstructs exactly that association from the other side, so leaving it would rebuild the link B.6 withholds. Unconditional beats conditional-on-removal for a reason that is easy to miss: **a conditional strip leaks the condition.** If the key were present for kept comments and absent for removed ones, its presence or absence would itself be the removal flag, on every row, in a table that already ships `mod_actions`. It also costs almost nothing, because the key is redundant where it is permitted — `comments.image_uploads_id` (B.6) already carries the association for every comment that is not withheld, so a researcher joining comments to uploads uses that column and never needed this one |
...
| `comment.placed` | `payload.userId` | Same rationale; payload research keys (`side`, `bodyLength`, market/bet/comment ids, `uploadId`) SHIP — the comment `body` + `side_at_post_time` are not payload keys; they ship via the `comments` table per **Appendix B.6** (this cited B.13 — the `events` block — until the 2026-08-28 pass; `comments` is B.6), commentary being the dataset's thesis-core signal. ⚠ `body` is `WITHHELD_IF_REMOVED`, not a bare SHIP — see B.6 (ENGINE.8 emit site) |
