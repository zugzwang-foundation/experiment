...
| **Gates downstream** | 25 ADRs (`ADR-0003` through `ADR-0027`; see §22.1 for per-ADR task attribution) + all `SCAFFOLD.*`, `ENGINE.*`, `DEBATE.*`, `DESIGN.*`, `EXPORT.*`, `MEDIA.*`, `TESTING.*`, `HARDEN.*` tracker tasks |
...
| Concern | Source-of-truth file |
|---|---|
| Per-ADR substance | `docs/adr/<NNNN>-<slug>.md` (26 ADRs — 25 ADR files + ADR-0012 in-flight; 23 accepted + 2 superseded + 1 in-flight; ADR-0001 admitted at BC.2) |
