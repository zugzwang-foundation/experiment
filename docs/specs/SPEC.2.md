...
| `addBookmarkAction(commentId)` | F-BM | `src/server/bookmarks/add.ts` | Bookmark icon on debate-view post cards (`ArgProfile`, via `PostCard` and `PostFocusHeader`) and reply cards (`ReplyCard`, via `ReplyPreview` and `ReplyScroller`), wired at **BOOKMARK-ADD-WIRE**. Own arguments render no icon — only another author's argument is bookmarkable (ADR-0032 D-3, the app-layer others-only guard); the viewer's own-comment id set is composed in `loadViewerMarketContext` per ADR-0034. Signed-out viewers see the disabled affordance. The other-user Profile card surface is pending the W2.7 / canon §3.11 design ruling and is **not wired**. See `docs/plans/UI-A6.md` §11 for the deferral rationale. | F-BM-1 |
| `removeBookmarkAction(commentId)` | F-BM | `src/server/bookmarks/remove.ts` | Active bookmark icon on `/bookmarks` (the in-page un-bookmark; UI-A6 Slice 4) | F-BM-1 |
...
**Server-Sent Events / WebSocket: explicitly absent.** The debate view refreshes by re-invoking its own `/m/[slug]` server read on a client-side interval (SPEC.1 §9 F-DEBATE-4), not by polling a Route Handler — the catalogue above is closed and F-DEBATE-4 adds no entry to it, because a dedicated read endpoint would fork removal-masking into a second implementation (ADR-0021, ADR-0034 D-4). `POLL_INTERVAL_MS_DEBATE_VIEW` carries a provisional pin of 15000 ms (SPEC.1 §16.1, Appendix B) so the flow is buildable before go-live; the tune remains deferred to HARDEN.6. The refresh re-executes the route's layout as well as its page, so a tick costs twelve to fourteen sequential database round-trips per open tab, including two session reads; polling suspends while the document is hidden or a bet composer is open, and stops permanently once the market leaves `Open`. `/m/[slug]` carries an explicit `export const dynamic = "force-dynamic"` from this build forward: the route was previously dynamic only as a side effect of calling `headers()` for the session, and a poll against an accidentally-static route would serve a frozen payload indefinitely. SSE / WS deferred to testnet phase per ADR-0006.
...
| `src/server/bookmarks/add.ts` | `addBookmarkAction` (F-BM-1) — auth-gated, others-only (D-3), idempotent `ON CONFLICT DO NOTHING` INSERT (ADR-0032) | §4.2 |
| `src/server/bookmarks/remove.ts` | `removeBookmarkAction` (F-BM-1) — auth-gated idempotent DELETE; the /bookmarks in-page un-bookmark | §4.2 |
| `src/server/bookmarks/list.ts` | `loadBookmarks` — the cross-author read (13 batched IN-list queries) + `BookmarkItem` DTO; forced-visitor, recency order; FI-2 same-source (§4.5) | §4.2 |
| `src/server/bookmarks/figures.ts` | The §4.3 Đa/Đb figure helper (FI-2 same-source walk, mirrors `positions.ts::walkMarket`) | §4.2 |
| `src/server/debate-view/viewer-context.ts` | `loadViewerMarketContext` + the `ViewerMarketContext` DTO — the viewer-session read composed BESIDE `loadDebateView` in the `/m/[slug]` RSC, never inside it (ADR-0034). One transaction, SELECTs only, READ-ONLY BY LAW: no ledger append, no accrual write, no cursor write, no `events` row. Returns `position` (`computeSell(quantity).proceeds` basis, FI-2) · `balance` · `spendableToday` (the pure Daily-Credit preview, never a call into `accrueDailyCredit`) · **`bookmarkedCommentIds`** + **`ownCommentIds`** — two additive, market-scoped, ID-ONLY arrays added at BOOKMARK-ADD-WIRE that drive the bookmark icon's saved state and own-argument suppression. Arrays, not `Set`s (a `Set` does not survive the RSC → client boundary). Neither array reads a content, side or author column, and neither is EVER a masking input — masking stays keyed solely on `loadRemovedSet` inside `loadDebateView`. | §4.2, ADR-0034 |
...
| `src/app/(public)/bookmarks/page.tsx` (+ `loading.tsx`, `error.tsx`) | The /bookmarks surface (UI-A6 Slice 4) — auth-gated forced-visitor list; mounts `removeBookmarkAction` via the client `UnbookmarkButton` in `src/components/bookmarks/` (`BookmarkCard`, `states`) | §4.2 |
