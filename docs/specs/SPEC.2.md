...
| `addBookmarkAction(commentId)` | F-BM | `src/server/bookmarks/add.ts` | Bookmark icon on debate-view post cards (`ArgProfile`, via `PostCard` and `PostFocusHeader`) and reply cards (`ReplyCard`, via `ReplyPreview` and `ReplyScroller`), wired at **BOOKMARK-ADD-WIRE**. Own arguments render no icon (viewer-scoped suppression per ADR-0034 D-4); signed-out viewers see the disabled affordance. The other-user Profile card surface is pending the W2.7 / canon §3.11 design ruling and is **not wired**. See `docs/plans/UI-A6.md` §11 for the deferral rationale. | F-BM-1 |
