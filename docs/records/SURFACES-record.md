...
| Mobile-responsive read surfaces | **BUILT** — Discovery #486 · profile #509 · auth #497/#510 · header #511 · `/m/[slug]` phone tier MOBILE-2 (**PR #517 MERGED at `9e12fb0d`, 2026-09-13**) — ⚠ **AND THE LANE DID NOT STOP THERE.** #517 was merged with a MERGE commit (two parents), not a squash, and work continued on the same branch: rounds **2j · 2k · 2l · 2m · 2n** (ADR-0051 A5–A10) are on `feat/mobile-2-market-detail` and were on **no PR at all** until MOBILE-2m opened one. A reader who takes "#517 MERGED" to mean "the lane is landed" is four rounds behind. ✅ **The lane CLOSED at MOBILE-2z, 2026-09-15** — ADR-0051 A1–**A12**, sixteen runs, `docs/records/MOBILE-2-close-out.md` is the record, and what remains unmerged is exactly **PR #536**: 25 files, none under `src/server/**`, `drizzle/**`, `src/db/**` or auth. ⚠ Everything before round nine already reached `main` through `staging` (#532, #535), so #536 is the last three rounds and the close-out, not the whole lane · the profile's phone view joined that lane at MOBILE-2e and became TILES at MOBILE-2h | — | — | **0045** · **0048** · **0049** · **0051** | `tests/unit/design/{discovery,profile,phone}-*` · `tests/unit/design/phone-round-five` · `tests/unit/shell/global-header-mobile-reflow` · `tests/unit/debate/phone/*` · `tests/unit/profile/render/phone-sell-{sheet,host}` | ⚠ This row read **NOT BUILT** with "no PR" until 2026-09-12 while five PRs had merged. ⚠ It also cited ADR **0050** — the phone tier was renumbered to **0051** at the MOBILE-2c merge (parked `2c-1`) and §3 below has said 0051 since, so this row contradicted its own document. `/m/[slug]` was the last surface and is no longer unmerged — #517 landed on `main` and `staging` together at `9e12fb0d`, which is also where four `instrumentation-register` tests began failing (see the MOBILE-2h run report; they arrive with `fix/auth-turnstile-tos`, which reached `staging` by a direct merge and has no CI run at all) |
...
| `shell/` | **14** | `GlobalHeader`, `HeaderNav`, `BrandCluster`, `Wordmark`, `IdentityCluster`, `DharmaCluster`, `PageContainer`, `VisitorCounter`, `GitHubStars`, `RulesControl`, `RadioSlot`, `CountdownDigits`, `countdown-format`, + **`header-control`** (the 34×34 icon register lifted out of `HeaderNav` at MOBILE-2's round ten so two controls wear one string rather than two literals that agree today). ⛔ **`GitHubIconControl` was the fifteenth and is DELETED at MOBILE-3a** — ADR-0051 **A13 D-1** withdraws the phone's GitHub control, and A12 D-1's *one off-site control* with it, because the 360px row has to seat the freeze countdown and a repository link is the one thing on it a phone reader can do without. The file is removed rather than left unmounted: an unmounted control with a green guard is a control nobody can tell is gone. ⚠ `header-control` now has ONE consumer, which is not a reason to inline it back — `HeaderNav`'s Home and Back are both still in it. ⚠ `ls src/components/shell/ | wc -l` is the claim; the figure read **13** until the MOBILE-2 close-out and **15** until this round. The two rows above are measured with it: `debate/` is **40**, `profile/` is **18** |
...
| **MOBILE-2z** | *(#536, the close-out)* | — | **2026-09-15** | the lane closes (ADR-0051 A12) — **no product code changes.** The X control is withdrawn by founder ruling, eleven amendments gain fifteen in-place supersession markers, and the lane is measured once more at its final tip against `origin/main` `a7bf4d2d`: `/u/` and `/m/` at 1440 and 640, both auth states, **0 added · 0 removed · 0 paint**, and **20 geometry rows that the same build against ITSELF also moves — the same twenty, the same ten price-chart families whose x is a function of `now`. The wall equals the floor.** Suite 536 files / 5,624 tests green, `tsc` 0, biome unchanged from `main` at 20 warnings and 13 infos. ⚠ **Two harness defects were found INSIDE this run and both returned plausible numbers rather than errors** — the signed-in arm was signed out (identical node counts across the two arms was the tell nobody was reading, `2z-1`), and the fingerprint keys a leaf by its own text, which a ticking countdown churns into two unrelated elements under one key. ⚠ **The GitHub control's href answers HTTP 404 to an anonymous request** — the repository is still private, which is a fact about the repository rather than about the control |
| **MOBILE-3a** | *(this PR)* | — | **2026-09-15** | the countdown comes back to the phone header (ADR-0051 **A13**). Below 640 the row is a flat flex line — `home · rules · logo · countdown · identity` — and the three zones stop being zones: both wrappers go `display:contents`, so every gap in the row comes from ONE declaration instead of three that agree while somebody maintains them. The countdown is `CountdownDigits` mounted alone at a phone cell scale, on the tick `BrandCluster` already owned: **one interval in the whole header**, measured, and a tick re-renders the digits and nothing else (siblings **0** renders across five minute boundaries). GitHub leaves the tier and its component is deleted; the logo rejoins the flow, withdrawing A9 D-3 below 640. ⚠ **The fit ladder ran all five rungs and stopped 2.40px short at 360 signed out, 7.13px signed in** — absorbed by the brand mark, which renders 45.60px and 40.87px against 48 with zero reported overflow. ⚠ An earlier probe in the same run reported 0.88/5.60; it had pinned the countdown's descendants too late and measured an already-shrunken row. The figures above close arithmetically against the component widths and were reproduced on the preview build. Nothing is clipped, nothing scrolls, every control renders, and at 375+ the mark is 48 in both arms. ⚠ Desktop proven unmoved against a same-build floor of **0**: `/` at 1440 and 640, both auth states, **0 geometry and 0 paint differences** over 56/58 painted nodes — with a positive control at 360 showing **20** differing rows, so the zeroes are a measurement rather than a blind method |
