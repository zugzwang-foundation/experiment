...
| Friendly-fire toggle — a declared stance on a same-side reply-bet (`comments.friendly_fire`, set on INSERT, never updated; `b` reads endorse vs contest) | SHIPPED (FF-1; server PR-A, UI PR-B) | `src/db/schema/comments.ts` + migration `0031_comments_friendly_fire`; write path `src/app/api/bets/place/route.ts` (two frontstops) + `src/server/bets/place.ts` (in-tx guard, same INSERT as `side_at_post_time`); aggregates `src/server/debate-view/ranking-substrate.ts` (+ the two mirrored sites); `src/lib/ranking.ts` (`derive()`); `src/server/comments/foreclosure.ts` (`friendlyFireEligible`) | SPEC.1 §8 `F-COMMENT-2` · §9 · SPEC.2 §5.1 row 4 · §5.4 · `RANKING.md` §2 / §6.1 | 0058 (D-51) | `tests/db/comments-friendly-fire.spec.ts` · `tests/server/comments/friendly-fire.test.ts` · `tests/server/debate-view/reply-substrate.integration.test.ts` · `tests/unit/ranking/{contested,friendly-fire-purity.property,substrate-site-parity}.test.ts` · `tests/unit/debate-export/serialize.test.ts` | exporter change parked (`docs/parked.md` FF-DATASET) |
