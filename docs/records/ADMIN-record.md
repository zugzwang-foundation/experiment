...
| Market list + overview | SHIPPED | `src/server/admin/markets/overview.ts`, `src/app/(admin)/admin/markets/page.tsx` | SPEC.1 §15.1 | — | `tests/server/admin/markets.test.ts` | — |
| Per-market admin page | SHIPPED | `src/app/(admin)/admin/markets/[marketId]/page.tsx` | SPEC.1 §15.1 | — | `tests/server/admin/terminal-actions.test.ts` | — |
...
| **Moderation review feed** | SHIPPED | `src/server/admin/moderation/review-feed.ts`, `_components/ReviewFeed.tsx` | SPEC.1 §15 · `F-ADMIN-4` | 0021 | `tests/server/admin/moderation/review-feed{,-completeness.integration,-side-chip.component,.component}.test*` | `docs/parked.md` UI-6 D1, D3 |
...
| **Resolution trio** — settle / correct / void | SHIPPED | `src/server/resolution/{settle,correct,void,trigger,basis}.ts`, W-3 `transaction.ts` | SPEC.1 §11 · `F-RESOLVE-1..3` | 0039 | `tests/server/resolution/` (6 files), `tests/invariants/I-RESOLVE-ONCE-001…`, `tests/integration/resolution-conservation.integration.test.ts` | — |
