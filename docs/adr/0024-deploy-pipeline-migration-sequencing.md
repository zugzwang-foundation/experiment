...
1. **Two standing Supabase projects, same committed migrations. [minted — refines ADR-0006]** Staging (`rwfdoqzsghqhhdapxafg`) and production (`zbvprdcyxhlguxbostdj`), both `ap-south-1`, both running the single mixed-origin Drizzle migration set (ADR-0008). **No Supabase branching.** ADR-0006 ratified the vendor topology and single region but not the staging/prod project split; this ADR names it. *(Confirmed at commit 2026-06-25: ADR-0006 §2 ratifies a single Postgres instance, not a two-project staging/prod split — so this item mints. ADR-0006's vendor topology is unchanged.)*
...
8. **Pooler + `prepare:false`. [minted]** All envs connect via the session pooler on `:5432`; the runtime client (`src/db/index.ts`) gets `prepare: false` as a defensive measure (safe on 5432; forward-safe if a transaction pooler is ever introduced). **(Scoped by Patch P3: read as "all envs **except staging**" — staging connects via the transaction pooler on `:6543` per ADR-0038 decision 3 + ADR-0038 P1. `prd` is unchanged and stays on `:5432`. `prepare: false` is unchanged and correct on both paths — the "forward-safe" clause is the case that arrived.)** **(Scoped again by Patch P4: `prd` may also run the transaction pooler on `:6543` when it declares `DB_POOLER_MODE=transaction` and carries `DATABASE_URL_TXN`; migrations stay on `:5432`.)**
...
- **`prepare:false` is correct only while the pooler stays on session mode `:5432`.** *Acceptable because:* a pooler-mode change is itself a deliberate infra change that would revisit this; the flag is harmless on 5432. **(Corrected by Patch P3: this inverted the dependency. `prepare: false` is *defensive* on `:5432` and **mandatory** on `:6543` — it was never at risk from a mode change; it was the precondition for one. The revisit this line anticipated is S-1, and it required no edit to the flag.)**
...
D2 plan: `docs/plans/D2.md` (Ratified decisions). Operator-provisioned GHA secrets — `VERCEL_API_TOKEN`, `DOPPLER_AUDIT_TOKEN` (env-audit), `DOPPLER_TOKEN_STG` (staging-migrate) — plus non-secret vars `VERCEL_PROJECT_ID`, `VERCEL_TEAM_ID`, `DOPPLER_PROJECT`. Branch protection adds required status check context `ci` (required-reviews stay 0). *(The env-audit's `DOPPLER_AUDIT_TOKEN` is superseded by Patch P2 below.)*

---

## Patch P2 — env-audit scope reduction (sync-health deferred; 2026-06-26)

In-place **Patch record** (CLAUDE.md §5.12), not a supersession: **no §Decision Outcome primitive changes** — item 9's CI required-check + journal-integrity + secret-drift gates all stand; the env-audit simply does **less**. This reconciles Patch P1's env-audit with an operator-infrastructure constraint found after #167.

1. **Sync-health detection deferred (the (c) sync-not-`In-Sync` + (d) duplicate-sync checks).** Listing Doppler↔Vercel syncs via the Doppler API needs account-level visibility through a **Service Account**, which is **Doppler Team/Enterprise-only**; the operator's plan does not include it, so those two checks are undeliverable as specified. The env-audit is therefore scoped to **(a) orphan Vercel keys + (b) missing required keys** — the two checks that ride the stable Vercel-env + Doppler-secret-name shapes and need only **config-scoped read** access. `scripts/ci-env-parity.ts` drops the `syncs` input, the `unhealthySyncs`/`duplicateSyncs` outputs, and the Doppler syncs API call. Sync-health re-enters scope only if the operator adopts a Doppler Team plan.

2. **Doppler reads re-sourced to two config-scoped tokens.** Patch P1's single cross-config `DOPPLER_AUDIT_TOKEN` (which implied a Service Account) is replaced by **two config-scoped read-only tokens**: `DOPPLER_TOKEN_STG` (stg, reused from staging-migrate) + a **new** `DOPPLER_AUDIT_TOKEN_PRD` (prd). `env-audit.yml` swaps the secret; `VERCEL_API_TOKEN` + the three vars (`VERCEL_PROJECT_ID`, `VERCEL_TEAM_ID`, `DOPPLER_PROJECT`) are unchanged. No secret-value logging discipline is unchanged.

D2 plan: `docs/plans/D2.md` → Patch P2. PR `fix/d2-env-audit-descope`. Operator: create a read-only **prd** config Doppler Service Token → secret `DOPPLER_AUDIT_TOKEN_PRD`; delete the now-unused `DOPPLER_AUDIT_TOKEN`.

---

## Patch P3 — Decision outcome #8 scoped to exclude the transaction-pooler environment (S-1, 2026-08-21)

In-place **Patch record** (CLAUDE.md §5.12), not a supersession: **no primitive in §Decision Outcome is withdrawn.** Item 8's `prepare: false` half is unchanged and stays correct. What changes is the reach of its first half.

Item 8 reads *"All envs connect via the session pooler on `:5432`."* On 2026-06-25 that was a complete description of the estate. It is now the **default, not the universal**: ADR-0038 decision 3 authorises **transaction mode (`:6543`) with `prepare: false`** without further ADR, and **ADR-0038 P1** records how the runtime reaches it (a new `DATABASE_URL_TXN` secret read by `src/db/index.ts`, rather than a repoint of `DATABASE_URL`).

*(The `prd` half of this paragraph is superseded by Patch P4 below.)* **Item 8 is therefore read as:** all environments connect via the session pooler on `:5432` **except staging**, which connects via the transaction pooler on `:6543` per ADR-0038 decision 3 + ADR-0038 P1. **`prd` is unchanged and stays on `:5432`** — all production work is after 10 Sep by founder ruling, and the existing session-pooler decision remains applicable to every other environment.

⚠ **`prepare: false` is correct on both paths, and its justification improves rather than changes.** The §Consequences → Negative line — *"`prepare:false` is correct only while the pooler stays on session mode `:5432`"* — inverted the dependency. The flag is **defensive** on `:5432` and **mandatory** on `:6543`; it was never at risk from a mode change, it was the **precondition** for one. That is exactly what item 8's own parenthetical anticipated (*"forward-safe if a transaction pooler is ever introduced"*). **The anticipated change is the one that happened, and the flag needed no edit.** Both sites are corrected in place, this commit — an amendments block records a change; it does not deliver one (CLAUDE.md §8, O-5).

⚠ **What this patch does not touch.** Decision 5 (migrate-before-serve), decision 6 (per-hash `/api/health` drift), decision 2 (staging deploys from the `staging` branch) and the D1 errata are all unchanged and all still bind S-1's execute. Two of them bind it hard, and are named here so they are not rediscovered at the flip:

- **The D1 errata's sync topology is what makes the blast radius unscopable.** Doppler `stg` syncs to **two** Vercel environments — Staging (custom env) and Preview — and the sync has **no per-secret include/exclude filter**. A repoint of `DATABASE_URL` in `stg` cannot be scoped to staging-but-not-previews. That is a property of the mechanism, not a policy choice, and it is the first reason ADR-0038 P1 chose a new secret over a repoint.
- **A Doppler change does not reach a running deployment until a redeploy.** `docs/runbooks/staging-provisioning.md:122` — *"treat auto-redeploy as not firing."* This holds symmetrically for the revert, so **the rollback contains a redeploy**. Recorded in a decision record because a provisioning runbook is not what anyone opens mid-incident.
...
## Patch P4 — production allowed on the transaction pooler (2026-09-17)

In-place **Patch record** (CLAUDE.md §5.12), not a supersession. Requested by the operator in session on 2026-09-17, after `DATABASE_URL_TXN` (`:6543`) and `DB_POOLER_MODE=transaction` were added to Doppler `prd`. Patch P3 kept `prd` on `:5432`, and `src/db/index.ts` enforced that by throwing at module load when `DB_POOLER_MODE=transaction` met `ZUGZWANG_ENV=prod`. **That refusal is removed.**

**Item 8 is now read as:** every environment defaults to the session pooler on `:5432`; any environment, **`prd` included**, runs the transaction pooler on `:6543` when it declares `DB_POOLER_MODE=transaction` and carries `DATABASE_URL_TXN`. The preconditions are ADR-0038 decision 3 + P1.3 and hold on prod unchanged: `prepare: false`; no session-level `SET`, `LISTEN`/`NOTIFY`, advisory locks or cursors; one pinned backend per transaction on the money path.

What stays:
- **Migrations stay on `:5432`.** `DATABASE_URL_PROD` and `scripts/migrate-prod.ts` are untouched (item 5).
- **A missing secret is still loud.** Prod with the flag and no `DATABASE_URL_TXN` throws, names the variable, and constructs no client; there is no fallback to `DATABASE_URL`.
- **The flag lives in Doppler `prd`**, which syncs to Vercel Production with no per-secret filter (D1 errata). A Doppler change reaches a running deployment only on redeploy, so **rollback (delete `DB_POOLER_MODE`) also needs a redeploy.**

⚠ **Order at execute.** Every build made before this change still carries the refusal, and `ZUGZWANG_ENV` is inlined at build time, so an older build deployed or promoted while the flag is present fails at load. The first prod deploy that sees the flag must contain this patch. Then confirm with `scripts/verify-pooler-mode.ts --expect=transaction` under `--config prd`, plus the per-hash `/api/health` gate.

⚠ **Not decided here.** `max: 4` and Pool Size 15 are unchanged (ADR-0038 decision 2, P1.2).

Code: `src/db/index.ts` (guard removed), `tests/unit/db/pooler-mode.test.ts` (row 4 inverted). Companion record: **ADR-0038 Patch record P2**, same change.
