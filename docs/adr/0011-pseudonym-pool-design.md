...
| **Frame document** | SPEC.2 §1.4 #5 (delegation: pseudonym pool word lists + asset pipeline owned here, not duplicated in SPEC.2), §5 (Data Model — `identity_pool` named in inventory), §22 (ADR Index) |
| **Supersedes** | — |
| **Superseded-by** | — |
...
|---|---|
| Pipeline architecture, word-list curation rules, seed-derivation, runbook, pool-extension procedure | ~~`experiment/docs/specs/PSEUDONYM.md`~~ — ⚠ **promise deleted, D-28 row 16.** The file was never written and will not be. Architecture, curation rules and the extension procedure are **in this ADR**; the word lists are `src/server/identity-pool/vocabulary.ts` (see the PFP-1 patch record, §1) |
...
- **Visual identity is the differentiator.** Coherent name-IS-the-image PFPs across the namespace make the debate view scannable in a way text-only walls (Manifold, Polymarket) cannot match. ~~The Devcon Mumbai conclusion-event presentation surface gains meaningfully from the visual quality.~~ (⚠ **conference struck, D-21 / D-26** — the experiment ends at the `2026-11-05 23:59 UTC` write-freeze (`system_state.frozen_at`) and this repository describes nothing after it. The driver is unaffected: scannability of the debate view stands on its own.)
...
- ~~Devcon Mumbai conclusion-event presentation surface degrades — debate-view screenshots lose their primary visual hook.~~ (⚠ **conference struck, D-21 / D-26** — the experiment ends at the `2026-11-05 23:59 UTC` write-freeze (`system_state.frozen_at`) and this repository describes nothing after it.)
...
- ~~Devcon Mumbai conclusion-event presentation surface degrades meaningfully.~~ (⚠ **conference struck, D-21 / D-26** — the experiment ends at the `2026-11-05 23:59 UTC` write-freeze (`system_state.frozen_at`) and this repository describes nothing after it.)
...
## Patch record — 2026-09-05 · the `PSEUDONYM.md` promise is deleted (D-28 row 16)

**D-28 row 16 deletes the promise of a `PSEUDONYM.md` companion spec.** It was never written, and the two sites that made it a *commitment* rather than a citation are struck above: the single-source-of-truth file map, which named it as the home of five concerns, and the negative consequence whose stated mitigation was a section of it. ⚠ **The remaining `PSEUDONYM.md` references in the body are left exactly as they are, deliberately.** They sit inside genesis reasoning — *"curation rules per §1.1"*, *"numbers are never generated by Flux per §7"* — and the rules they cite are written out in this ADR beside the citation, so nothing is lost by the file's absence and the argument reads as it was made. Striking a promise is not the same as editing the thinking that once relied on it. **This is the same shape the PFP-1 patch record already recorded for `colours.txt`, `animals.txt` and `composite_numbers.py`** — artefacts this ADR named that were superseded or never built — and it is now the fourth. ⚠ **`SPEC.2` Appendix A carries the same promise and is NOT corrected here**, being outside SYNC-11's lifted wall; D-28 row 16 names it, and it belongs to the `SPEC.2` pass.
...
animals = 871 pairs"* — and marks the 50,000-row manifest **still owed**: *"None of the
three is started."*
...
## Patch record — 2026-09-14 · the number is derived from the pair, not from the pass index (PFP-3)

A divergence being closed, not a new decision. `:83` already specifies what the number should be: derived from `hash(colour + ":" + animal + ":" + version_tag + ...)`, with three named properties — reproducible, "visually varied (numbers spread across the range, not clustered)", and "pool-extension collision-free". What shipped in `src/server/identity-pool/rotation.ts` was `number: pass`, the index of the full sweep over the 871 pairs. It satisfied the first property and neither of the other two.

**What that looked like.** `consumeIdentityPoolTuple` hands rows out FIFO in seed order, so the first 871 participants all carried `000` and the next 871 all carried `001`. SPEC.1 sizes the experiment at 1K–10K participants, which reaches one or two passes, so in practice the number was a constant. A pool seeded for launch read `RedAlpaca000`, `OrangeArmadillo000`, `GoldBadger000` down the page.

⚠ **This is Option 7, which this ADR rejected at `:257`.** That option — sequential numbers per pair — was turned down because it is "visually monotonous" and "clusters at the low end of 000–999", with the verdict that "the deterministic-PRNG approach gives both". The built code arrived at the rejected option by a different route, and nothing in the test suite could see it: the thirteen assertions in `tests/unit/identity-pool/rotation.test.ts` all pass unchanged against the new derivation, because not one of them asserted anything about what the number *is*. **The regression was invisible to the suite that covered the file.**

**The mechanism.** Per `(colour, animal)` pair, `sha256(colour:animal:version_tag)` yields two coefficients, and the number on a given pass is `(step × pass + offset) mod 1000`. `offset` spreads the first pass, which is the only pass most of this experiment reaches. `step` is drawn from the 400 residues coprime to 1000, and coprimality is what makes the map a bijection on ℤ/1000 — so a pair walks all 1000 numbers before repeating one, and extending the pool cannot collide with what is already seeded. That is `:83`'s third property, held by arithmetic rather than by a lookup against existing rows.

⚠ **The step is drawn from a table built by the definition of coprimality, not computed.** An earlier draft derived it arithmetically and carried a comment asserting it was never a multiple of 5. It was, and the result was six colliding `(colour, animal, number)` triples in 8,710 — which the unique index `identity_pool_tuple_idx` rejects, turning a cosmetic change into a failed seed. A comment cannot hold that property; a filter over the range can.

**Measured, not asserted.** Over the full 871,000-tuple namespace: 871,000 distinct triples, 0 duplicates, 0 numbers outside 000–999, and every one of the 871 pairs yielding exactly 1000 distinct numbers. Over the first pass — the 871 rows a launch actually hands out — 592 distinct numbers against a uniform-draw expectation of 582, spread across the decades 90/75/78/83/96/86/97/98/85/83, with zero adjacent rows sharing a number and lag-1 autocorrelation −0.014. The first four signups read `RedAlpaca757`, `OrangeArmadillo643`, `GoldBadger328`, `OliveBeaver868`.

**Driver 2 requires the derivation be re-derivable from this repository alone,** and "hash-derived" does not say which bytes become which coefficient. `NUMBER_VERSION_TAG`, `OFFSET_BYTE` and `STEP_BYTE` are therefore named constants in `rotation.ts` rather than inline literals. Moving any of the three re-numbers every unassigned pair, which is what the version tag exists to announce.

⚠ **The pool is pre-seeded DATA, so this reaches rows seeded after it lands and no others.** `identity_pool` is Bucket B: `drizzle/migrations/0003_append_only_triggers.sql` permits only `assigned_at` to transition and rejects every DELETE, so already-seeded rows keep their numbers and cannot be removed. Staging holds 1,070 rows with 220 assigned, all `000`; the sanctioned path is the guarded reset (ADR-0035). ⚠ **`docs/polish/staging-coverage.json` pins ten `…000` pseudonyms and gate 4 of `tests/staging/gates.staging.test.ts` fails RED when it drifts** — it is regenerated by the gates themselves on the re-seed, and no edit here can substitute for that measurement.

⚠ **This does not reach production seeding.** `rotation.ts` feeds `scripts/seed-identity-pool-dev.ts` and `scripts/seed-staging.ts` only. `scripts/seed-identity-pool.ts` ingests an external CSV manifest, so unless that manifest is generated from `generatePoolTuples`, production numbers remain whatever the operator's offline pipeline emits. Recorded as an obligation, not discharged here.
