...
SPEC.1 §14 mandates AI-driven pre-commit moderation across Tracks A (auto-ban categories: CSAM, sexual/minors, NSFW), B (admin-review categories: violence, hate, harassment, threats, self-harm, weapons), and C (passes). The moderation runs on every comment: F-BET-1 entry (atomic with bet, governed by INV-1 and F-MOD-4), F-COMMENT-1 (direct), F-COMMENT-2 (reply), and F-COMMENT-3 (image-attached). Per SPEC.1 §14 Appendix A, the canonical category-to-track mapping is fixed; threshold values per category are deferred to HARDEN.5 sample-content testing, at the number-tuning pass.
