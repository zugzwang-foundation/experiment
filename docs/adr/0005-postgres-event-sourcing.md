...
### P1 — INV-3 side-freeze is the `comments` Bucket-A guarantee; the capture was DEBATE.2 (DEBATE.3, 2026-06-17)

In-place Patch record per CLAUDE.md §5.12 (consumer-surface scoping, **not** supersession). **The load-bearing decision is unchanged** — `comments` remains a Bucket A strictly-append-only table per the Decision's nine-table append-only set and the "Flow & invariant constraints absorbed" INV-3 row; no classification, trigger, or schema change. DEBATE.3's recon established that INV-3's database-layer enforcement IS the whole-row Bucket-A trigger on `comments`: `bucket_a_no_update` / `bucket_a_no_delete` (`drizzle/migrations/0003_append_only_triggers.sql`, via `enforce_bucket_a_no_update()`) `RAISE` `P0001 "UPDATE not permitted"` on any UPDATE to a comment row, so `side_at_post_time` immutability follows from the whole-row guarantee rather than a dedicated column trigger — the mechanism SPEC.2 §6.5 names ("INV-3 … via append-only `comments`"). A bespoke column-scoped trigger on `side_at_post_time` is therefore **rejected**: there is no legitimate post-insert UPDATE path to `comments` (the image link `image_uploads_id` is set at INSERT, `src/server/bets/place.ts:135`; moderation is pre-commit per ADR-0014 — on an A/B verdict the bet+comment tx never opens, so a flagged comment is never inserted and there is no post-insert moderation-state mutation), so such a trigger would only *relax* the table from full append-only, a regression against §6. DEBATE.3 introduces **no trigger and no migration** — migration head stays `0015`. The post-time-side **capture** (writing `side_at_post_time` = the author's/replier's own held side at INSERT inside the W-1 bet transaction, `src/server/bets/place.ts:133`) was built in **DEBATE.2** (#136); the named invariant `I-SIDE-BIND-001` was minted at **ENGINE.8** (`docs/logs/ENGINE-phase-record.md` §5). Consequently the "Flow & invariant constraints absorbed" INV-3 row — which read *"ADR-0013 / DEBATE.3 own the post-time-side capture itself"* — is corrected in place: the **capture** is DEBATE.2; the **immutability** is this ADR's Bucket-A guarantee (mechanism at `0003`, invariant at ENGINE.8); ADR-0013 governs the W-1 transaction shape the capture rides, not the capture itself. DEBATE.3's residual is a single named `I-SIDE-BIND-001` column-targeted storage assertion (`UPDATE comments SET side_at_post_time` → `P0001`) in the canonical invariant gate, plus this record.
