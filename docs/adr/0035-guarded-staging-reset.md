...
### A.3 · Primitive 2 was measured, not only reasoned about

*Recorded at STAGING-PARITY Slice A.* Primitive 2 asserts that the batch
is one implicit transaction and that an abort therefore rolls the DISABLE
back. That mechanism was subsequently measured against Postgres 17 rather
than left as an argument. `SET LOCAL lock_timeout` issued alone reads
`"0"` and emits `WARNING 25P01`; issued inside the batch it reads `"15s"`;
it reverts to `"0"` on the next round-trip. A `DISABLE TRIGGER` followed
by an in-batch failure leaves `tgenabled = 'O'`. The simple query protocol
was established from the driver source and from the server-side refusal of
multiple commands on the extended path. The evidence is
`docs/logs/STAGING-PARITY-A-mutation-audit.md` STEP 1; the assertions that
pin it live in `tests/integration/staging-reset-mechanism.integration.test.ts`.
