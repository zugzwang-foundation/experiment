...
| Tuning `lock_timeout_ms` | ⛔ **THE INVARIANT IS `(open_markets − 1) × lock_timeout_ms < 1000`.** The sweep is ONE transaction, so a pool row locked at the first market stays locked until the last finishes, and a bet waiting on it has a **1,000 ms `statement_timeout` whose `57014` is NOT retryable** — a bet that loses that race does not retry, it fails. Migration `0028` adds a CHECK ceiling of **250 ms**, but that is a TYPO GUARD and not a proof: a CHECK cannot see the market count. ⚠ **Measured 2026-09-07: staging carries TEN `Open` markets, not the eight this ADR reasons from** — so the safe ceiling there is 111 ms and the shipped 100 ms holds with almost no room. **Re-derive this before adding markets, not after** |
