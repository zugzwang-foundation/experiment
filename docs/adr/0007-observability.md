...
## Patch record — 2026-09-14 · React error boundaries report (the client arm was never wired)

**The decision is unchanged. Its consumer surface is scoped**, per CLAUDE.md §5.12: Sentry remains the error vendor, the fail-open posture of §1 is reaffirmed rather than amended, and `sendDefaultPii: false` is untouched.

**What was missing.** §1 above says "both the client SDK and the server SDK wired", and both `Sentry.init` calls have shipped since SCAFFOLD.5. Initialisation is not coverage. `instrumentation.ts` re-exports `onRequestError`, so every SERVER throw reports; **nothing a React error boundary caught reported at all.** A boundary is by definition what stops an error propagating, so it never reaches `window.onerror` and the browser SDK's default global handlers never fire. The product shipped four boundaries — `global-error.tsx`, `(auth)/error.tsx`, `m/[slug]/error.tsx`, `u/[pseudonym]/error.tsx` — and every client-side and hydration failure caught by one rendered its apology and went nowhere. On a solo-operated 7-week live window that is the smoke-alarm function of driver 1 silently absent on the arm nobody is watching.

**What landed.** `src/lib/boundary-capture.ts`, a client-side fail-open mirror of `server/observability/safe-capture.ts` (which cannot be reused — it is `server-only`), called from all four boundaries in a `useEffect` keyed on `[error]`. Events carry `kind: react_error_boundary`, the boundary name, and `digest` when present.

**The cost, stated because it is a real one.** The three route boundaries held a structural no-leak guarantee: the `error` prop was deliberately never destructured, so no binding existed and NO BINDING IMPLIED NO READ — of any kind, on any node, in any portal, for any event. A `@security-auditor` pass at POLISH.3 R7 minted that property and a source assertion proving it. **Reporting requires a binding; there is no version of this that keeps both**, because no other layer can see a boundary-caught error. The guarantee is now procedural: `error` may reach `captureBoundaryError` and nothing else. The guard was INVERTED rather than deleted — it pins the single permitted destination and fails on a second — and the read set is pinned to exactly `["digest"]`, so lifting `message` or `stack` into a tag goes RED. Both were mutation-tested. `digest` alone is deliberate: it is the join key to the `onRequestError` event and carries no message text, while `message` / `stack` / `cause` are the fields that hold the DB error, the Better Auth internal and the participant's own input. They still reach Sentry inside the error object, which is the point; what is forbidden is promoting one to an indexed, retained tag facet.

**Not in scope, and each a separate ruling:** `Sentry.setUser` identification and session replay (§5 above keeps replay disabled; both cross the SPEC.1 §16.3 privacy floor), a `tunnelRoute`, `beforeSend` scrubbing, env-scoped `tracesSampleRate`, and converting the bare `captureException` call sites AUDIT-FIX-B1 ruling #8 deliberately left alone.
