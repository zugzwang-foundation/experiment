...
- **No velocity guard.** RECON-2 §10.6 measured it anti-correlated with the
  trigger.
- **`BET_MAX_STAKE` is NOT changed here.** See **OD-5** — the ADR pins it at 250
  and the kickoff does not scope it. That is a founder call, not a plan call.
...
|---|---|---|
| **C1** | T1 + its cpmm.md §7.1/§13 rider | One function body + the spec sentence it changes (`O-9`) |
| **C2** | T2 + T3 | The payload schema and the table it references type-check together |
...
**OD-1.** This edits a file `liq-1/phase-1` landed. It is a correction — the
current body is measurably wrong on a class the product can reach after one
`coefficient` INSERT — but it is a Phase-1 surface and the founder should say
so.
...
**(d) `run_liquidity_injection()` — a `PROCEDURE`, not a function.** The eleven
steps of ADR §D, per market, with **one transaction per market**:

```
  advisory: pg_try_advisory_lock(<key>)  -- overlap guard; RETURN if not acquired
  read policy once  (newest effective_from <= now(), version DESC)
  exit if not p.enabled
  exit if system_state.frozen_at IS NOT NULL
  target := max(p.floor, p.coefficient * (SELECT count(*) FROM users))   -- ONCE per tick
  FOR each market in status 'Open':
      SET LOCAL lock_timeout = p.lock_timeout_ms
      ...
      COMMIT
  INSERT liquidity_heartbeat
  pg_advisory_unlock
```

Four decisions inside that, each with its reason:

- **A `PROCEDURE` called with `CALL`, not `SELECT f()`.** The `0007`/`0011`
  precedent is `SELECT f()` because those functions only *alarm* — one statement,
  one transaction, nothing held. This one takes a **pool row lock that the bet
  path also takes**. A single transaction across eight markets would hold up to
  eight pool locks for the duration of the whole sweep; a procedure that COMMITs
  per market holds exactly one, for one UPDATE plus two INSERTs. pg_cron 1.6.4
  (measured on both environments) accepts `CALL`. ⚠ `SET LOCAL` resets at each
  COMMIT — **re-issue it every iteration**, or the second market runs with the
  session default (no timeout) and a contended pool blocks the sweep
  indefinitely. That is the failure this bullet exists to prevent.
...
|---|---|---|
| `liquidity_silence` | no `liquidity_heartbeat` row in 15 minutes | edge-triggered via a `watermark_state` row, `0007`'s `IS DISTINCT FROM` pattern — fires once per episode, not once per tick |
...
```sql
CREATE OR REPLACE PROCEDURE run_liquidity_injection()
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
	-- OVERLAP GUARD. A session-level advisory lock, not a transaction-level one:
	-- this procedure COMMITs per market, and pg_advisory_xact_lock would be
	-- released by the first COMMIT, letting a second run in mid-sweep.
	IF NOT pg_try_advisory_lock(hashtext('zugzwang.liquidity_injector')) THEN
		RETURN;
	END IF;
	...
	IF NOT FOUND OR NOT p.enabled THEN
		PERFORM pg_advisory_unlock(hashtext('zugzwang.liquidity_injector'));
		RETURN;
	END IF;

	-- Conclusion freeze (§20.2). The injector is a WRITE surface, so unlike the
	-- resolution flows it has no exemption: post-freeze it is a no-op.
	IF EXISTS (SELECT 1 FROM system_state WHERE frozen_at IS NOT NULL) THEN
		PERFORM pg_advisory_unlock(hashtext('zugzwang.liquidity_injector'));
		RETURN;
	END IF;

	-- ONE count per tick, not one per market: the target is uniform across every
	-- Open market (ADR §C), and eight counts could straddle a signup and hand
	-- two markets different targets in one sweep.
	...
	LOOP
		...
		-- ENDGAME: leave the closing window alone.
		CONTINUE WHEN now() + make_interval(hours => p.endgame_hours)
		              >= m.resolution_deadline;

		BEGIN
			-- SET LOCAL is reset by every COMMIT below, so it is re-issued each
			-- iteration. Without this line the second market onward runs with no
			-- timeout and one contended pool blocks the whole sweep.
			...
			IF NOT FOUND THEN COMMIT; CONTINUE; END IF;

			v_tank  := pool_row.yes + pool_row.no;
			v_price := pool_row.no / v_tank;         -- p_yes = n / (y + n)

			IF v_tank >= p.trigger_ratio * v_target
			   OR v_price > p.guard_high OR v_price < p.guard_low THEN
				COMMIT; CONTINUE;
			END IF;

			-- a = L * (target/tank - 1)   (SIM-2 §0a, derived from
			-- tank' = tank + a*(L+S)/L and solved for tank' = target)
			v_a := GREATEST(pool_row.yes, pool_row.no) * (v_target / v_tank - 1);
			IF v_a <= 0 THEN COMMIT; CONTINUE; END IF;

			SELECT * INTO r FROM zz_add_liquidity(pool_row.yes, pool_row.no, v_a);
			...
			-- created_at is DERIVED from the event id's first 48 bits, exactly
			-- as insert.ts's uuidv7ToCreatedAt does; two tests assert that
			-- property, and `events` is RANGE-partitioned on this column.
			...
			COMMIT;
		EXCEPTION
			-- 55P03 lock_not_available / 40001 / 40P01: this market is busy or
			-- lost a race. Skip it. The trigger is still true and the next tick
			-- is 60 s away, so a skipped market self-heals; there is nothing to
	...
	VALUES (v_run_id, p.version, v_considered, v_injected);
...
⚠ **`CONTINUE WHEN` and `EXCEPTION`-blocks-with-`COMMIT` are the two constructs
to verify against PostgreSQL 17.6 before trusting this shape.** A `BEGIN … END`
block containing `COMMIT` is legal inside a procedure but the interaction with an
`EXCEPTION` handler is version-sensitive; measure it against the local Postgres
in the first execute session and record what it does. **If it will not compile,
the fallback is a per-market `SELECT inject_one_market(id)` function called from
a loop that COMMITs between calls** — same semantics, one more routine.
...
### B · `docs/specs/SPEC.1.md` — version **1.0.49 → 1.0.50**

Patch, per the precedent in its own change log: 1.0.48 replaced §9's entire X
domain and took a patch bump; §10.6 is a product policy, not one of INV-1..4.
**OD-3** if the founder reads *"bump major on invariant changes"* more broadly.
...
| **B-6** | **§11 Non-goals** `:1511` — *"**Mid-market liquidity adjustments.** Pool seed fixed at creation (per `B6`)."* | **predicates 1 + 2** | **Strike the bullet and replace in place**: *"**User-provided liquidity and limit orders.** No participant may add or remove pool depth, and there is no order book (per `B6`, and out of scope by ruling). The system's own signup-pegged injector is not a user surface — §10.6."* |
| **B-7** | **Appendix B** `:1983` — `POOL_SEED_PER_MARKET_DEFAULT = TBD` | **predicate 4** | Replace with: `# POOL_SEED_PER_MARKET_DEFAULT — RETIRED by ADR-0047 §B (never existed in src/)` plus `MARKET_OPENING_PRICE_YES = 0.10  # ADR-0047, D-14, all eight` and `MARKET_OPENING_TANK = 100000  # ADR-0047 §B` . **`BET_MAX_STAKE = TBD` stays TBD here — see OD-5.** ⚠ **The kickoff says *"the four constants pinned by ADR-0047"* and this plan cannot make that four.** ADR-0047's own table says `BET_MAX_STAKE` is *"the ONE Appendix B constant this ADR pins"*, and the injector parameters are explicitly policy-table values. Measured content of Appendix B touched by ADR-0047: **two** (`POOL_SEED_PER_MARKET_DEFAULT` retired, `BET_MAX_STAKE` pinned — the latter withheld under OD-5), plus **two minted** (`MARKET_OPENING_PRICE_YES`, `MARKET_OPENING_TANK`). If "four" means those four rows, this row delivers it; if it means something else, **OD-6**. |
| **B-8** | **§17** spec-rule table | — | Add `cpmm::liquidity-injection-preserves-price` and `cpmm::injection-recorded-as-event` — both invariant-class, both `—` in the INV column (the `I-DAILY-ONCE-001` / `markets::open-implies-market-opened-event` precedent: a wrong INV tag sends a reader to the wrong trigger). |
| **B-9** | **§0 + change log** | — | Version → 1.0.50, last-updated → the merge date; one change-log row naming B-1…B-8, **and naming B-3/B-4 as a Phase-1 debt this pass found rather than a Phase-2 change** — that distinction is what stops the next reader concluding Phase 1 shipped wrong code. |
...
| **R-7** | **Two injector runs overlap.** pg_cron will not start a second run of the same job while one is running, but a hand-run (`liquidity_silence` fires → the Runbook says call it by hand) can race the scheduled one. | `tests/db/liquidity-injector.spec.ts::overlapping-runs-one-works-one-exits` — two concurrent `CALL`s; exactly one injects, the other returns immediately. ADR §Acceptance "Overlap". | ⚠ **`pg_try_advisory_lock`, not `pg_advisory_xact_lock`.** The procedure COMMITs per market, and a transaction-scoped lock is released by the first COMMIT — letting a second run in mid-sweep, which is the exact case this row exists to prevent. |
...
| **R-9** | **`SET LOCAL lock_timeout` silently stops applying after the first COMMIT**, so market 2 onward blocks indefinitely on a contended pool and the sweep never finishes. | `tests/db/liquidity-injector.spec.ts::lock-timeout-applies-to-every-market` — hold a pool lock in a second session, assert the sweep skips that market and **still processes the ones after it**. | The failure is invisible: the sweep just takes longer, the heartbeat is late rather than absent, and nothing errors. A test that only checks market 1 passes. |
...
## §11 Open decisions — for the founder, before execute

| # | decision | recommendation |
|---|---|---|
| **OD-1** | **T1 edits `src/server/cpmm/calculate.ts`, a Phase-1 landed file.** The current `addLiquidity` is measurably one ulp low at `S == L` with reserves ≥ 1e8 (0% below 1e7; 6.2–6.9% above; 0/50,000 at the D-14 skew). | **Fix it.** Not reachable by today's markets, but `p = 0.5` is inside the guard band and 1e8 reserves are one `coefficient` INSERT away — and ADR §A's *"two implementations, one definition"* is not true until it is fixed. |
| **OD-2** | **ADR §Acceptance's row *"`k` monotone with named door — `k′ > k` only on inject"* is FALSE as written.** `cpmm.md` §12 E2 is a **buy** that moves `k` from `10000` to `10000.000000000000000010`; `floor18` dust raises `k` on buys and sells too (INV-C2). | Assert the ADR's own **Consequences** wording instead: *"`k` changes only through a named door"* — a test that walks buy/sell/inject and asserts `k′ ≥ k` after each, plus that no OTHER code path writes `pools` (a source scan over `tx.update(pools)` / `UPDATE pools`, matching class TOKENS, not bare words). |
| **OD-3** | **SPEC.1 version bump: patch or major?** §0 says *"bump major on invariant changes"*; §10.6 is a product policy, not INV-1..4. | **1.0.50 (patch)**, on SPEC.1's own precedent — 1.0.48 replaced §9's whole X domain on a patch bump. |
| **OD-4** | **ADR §G says the seeded policy row is `enabled = true`. This plan seeds it `false`.** Under ADR-0024 the migration applies to prod *before* the code is promoted, so `true` means the injector writes `pool.liquidity_added` rows into a database whose running code cannot read them — under-reporting `settleMarket`'s residual by the injected discard on a terminal, append-only row. | **Seed `false`; arm with one INSERT after the promote.** The cost is one operator command; the ADR's own §G says tuning is one INSERT and never a deploy, so this uses the mechanism rather than working around it. **R-8.** |
| **OD-5** | **`BET_MAX_STAKE`.** ADR-0047's constants table pins it at **250 Đ**; `limits.ts` ships **`"10000"`**; the kickoff's SCOPE does not name it. A 40× reduction on the participant bet path is not a liquidity change. | **Withheld from this plan.** It is a one-line change with a large behavioural surface, it belongs to the number-tuning pass the ADR says it is borrowing from, and SIM-2's runs used 100 Đ bets — so nothing here measured it. Founder ruling: land it in Phase 2, or as its own task. |
| **OD-6** | **The kickoff says *"Appendix B (the four constants pinned by ADR-0047)"*; the ADR says `BET_MAX_STAKE` is *"the ONE Appendix B constant this ADR pins"*.** | §6 row **B-7** delivers four Appendix-B rows (one retired, one pinned-but-withheld under OD-5, two minted). If "four" meant something else, name it. |
| **OD-7** | **`liquidity_policy` ships in the public dataset** (§6 C-4), per ADR §G *"The history ships in the dataset."* | **Confirm.** It carries no PII and no `user_id`, and without it no reader can reproduce an injection — the target rule is not derivable from the events alone. |
...
- Whether `CONTINUE WHEN` and a `COMMIT` inside a `BEGIN … EXCEPTION` block
  compile together in a PL/pgSQL **procedure** on 17.6 — the procedure body was
  designed but not compiled. `CREATE PROCEDURE` against the local Postgres reads
  it, and §4d names the fallback.
- Whether pg_cron 1.6.4 accepts `CALL` in a `cron.schedule` command on
  **Supabase's** build specifically. Both live jobs use `SELECT`.
  `SELECT cron.schedule('probe','* * * * *',$$CALL …$$)` on staging reads it —
  do it in the first execute session, not at soak.
