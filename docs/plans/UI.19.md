...
1. **Chart data rides `loadDebateView` on a TOP-LEVEL `model.priceChart` (web Gate-C #5 — reversed).** The RSC (`m/[slug]/page.tsx`) already calls `loadDebateView`; the chart model (`{ series, nodes } | null`) sits at **`model.priceChart`**, a **sibling of `model.market`** (whose `pricing` feeds `PriceBar`) and `model.posts` — **not** on `DebateMarketHeader`. A ~256-point series must not ride a type several components read, and sibling placement makes the one-payload guarantee legible (exactly what `::series-on-same-payload-as-pricing` asserts). `DebateView` destructures it and passes it as a prop to `MarketHeader` → `MarketPriceChart` (client, owns expand state). Null when the read failed (error state below). No client fetch, no separate route.
   ...
   **Correction (found at execute, slice 1).** The claim "Additive field; all consumers still typecheck" is FALSE. A required top-level field obligates every literal constructor of `DebateViewModel`; two exist, both outside the slice-1 file list — `tests/unit/debate-export/_fixtures/mumbai-metro.input.ts` and `tests/unit/debate-export/serialize.test.ts`. Founder ruling 2026-07-23: keep `priceChart` **required** and add `priceChart: null` to both fixtures (the `.md` export is text-only per ADR-0025, so `null` is the honest value). Optional was rejected — `undefined` would blur with the `null` that means "series read failed." The claim was inherited from the pre-reversal (#5) shape and not re-checked when the reversal landed.
