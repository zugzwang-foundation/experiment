...
1. **Chart data rides `loadDebateView` → the serialized `DebateMarketHeader`.** The RSC (`m/[slug]/page.tsx`) already calls `loadDebateView`; the chart model (`{ series, nodes }`) attaches to `market.priceChart` and flows to `MarketHeader` → `MarketPriceChart` (a client component owning the expand state). No client fetch, no separate route.
...
3. **F-DEBATE-4 refresh — satisfied STRUCTURALLY (web Gate-C ruling).** The series is on the one market view-model payload (`priceChart.series` on `DebateMarketHeader`), so it refreshes on the SAME read that carries `market.pricing`; there is **no separate client-side fetch for the series, ever.** When F-DEBATE-4 interval polling lands, the series rides that read for free — nothing extra to build. Interval polling is **not wired on `/m/[slug]` today** (docketed finding, self-critique #4). A slice-1 test asserts `priceChart.series` and `pricing` arrive on the same `loadDebateView` payload.
...
Files touched:
- `src/server/config/limits.ts` — add `export const MARKET_SERIES_MAX_POINTS = 256` (§16.1 / Appendix B pinned design value; JSDoc cites 1.0.22). *(Type/config — the type-check is its gate, §5.6-exempt.)*
...
- `src/server/debate-view/load-debate-view.ts` — add `priceChart: { series: PricePoint[] }` to `DebateMarketHeader`; assemble via `loadMarketPriceSeries(db, marketId, pricingAndUnitToWin?.pricing.yes ?? null)` **after** the pricing read (additive field; all consumers still typecheck).
...
- `src/components/debate/MarketHeader.tsx` — mount the host **above `<PriceBar>`**; remove **only** the `Price history — …` line from `DeferredPlaceholders` (keep `Resolver cards` + `Market media`).

States: **loading** — none (RSC-delivered, like `PriceBar`). **empty** — none by spec; defensive `series.length === 0` (unreachable) → render null, header still shows `PriceBar`. **error** — none new; shares `loadDebateView`'s fail-fast read posture (§5.2).

Tests (test-writer first):
- `tests/server/debate-view/price-series.integration.test.ts` — `::market-lifetime-domain` (X = opened→last event), `::single-point-renders-flat-line` (unbet → flat at opening), `::downsample-cap-respected` (>256 events → ≤256, first+last kept), `::series-frozen-on-non-open` (INV-4 series half), **`::series-on-same-payload-as-pricing`** (series + `pricing` on the one `loadDebateView` return — web Gate-C #4: one payload, no separate fetch), **`::terminal-equals-pricing-spot`** (`series[last].yes === market.pricing.yes` — web Gate-C #6).
- `tests/unit/debate/render/price-chart.test.tsx` — `::collapsed-renders-no-axis` (collapsed shows no axis/nodes), `::accessible-summary-present` (sr-only opening/current/endpoints), `::flat-line-when-single-point`.
...
- `src/server/debate-view/load-debate-view.ts` — extend `priceChart` to `{ series, nodes }`; call `deriveMarketPriceChart` passing the **already-loaded** `postSubstrate` + `removedSet` + `pricing.yes` (reuse the `replayReserveSeries` result across series + node-y).
...
**Web Gate-C-mandated tests (beyond the eight §17 ids):** `::series-on-same-payload-as-pricing` (#4, slice 1), `::terminal-equals-pricing-spot` (#6, slice 1), `::within-bucket-order-is-toporder` (added to (c), slice 2).
...
5. **`DebateMarketHeader` growth.** Attaching `priceChart` to the header type is the least-plumbing path, but it grows a type several components read. Alternative: a top-level `model.priceChart`. Chosen header-attach for the natural `MarketHeader` prop flow; flag if web prefers the top-level split.
...
8. **Stale `DISCOVERY_SERIES_MAX_POINTS` JSDoc.** 1.0.22 recorded it in Appendix B as a "pinned design value," but the code JSDoc still says "NOT a spec constant." A one-line JSDoc reconciliation is *in scope-adjacent* but not required by F-DEBATE-5; I have left it out to stay surgical (§5.3) and note it here as known drift for a future SYNC — not this task.
