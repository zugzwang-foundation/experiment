...
## §19 · Gate C read-1 and read-2 rulings — founder, 2026-08-13

**APPEND-ONLY. §1–§18 are unedited.** **EIGHT** rulings, each recorded with its ground so a later reader finds a definition rather than a citation: **PF-1…PF-7 from Gate C read 1, and PF-8 from read 2** — attributed here so the provenance is on the page rather than inferable from the table. ⚠ **This heading and this count are DATED MEASUREMENTS of the table below, not fixed text**: a ninth row moves both, and moving them is not a breach of append-only. Append-only protects §1–§18 from revision; a section's own heading and intro are metadata ABOUT the appendix, not content OF it. *(The `PF-n` series was cited in three PR-1 commit bodies before it was defined anywhere; that gap is what this section closes — a citation without a definition is the L-space collision `CLAUDE.md` §8 was minted to stop.)*

| ID | Ruling | Ground |
|---|---|---|
| **PF-1** | **§7's item-4 row is STALE under §18 R-a, and it is NOT a halt.** C3 drives from R-a; ⛔ condition 4 does **not** fire. Do not perform the edit §7 describes. | §7 predicts `page-container.test.ts`'s three `9`s "🔴 MOVE". Under R-a nothing there moves: `SITES` stays 9, `GREENFIELD` gains 1, the tree goes to 10, and set-equality balances without touching them. §7 is **descriptive**; §9 C3 and §6 item 4 carry the instruction, and R-a states its own supersession. Verified at PR head: all three numeric `9`s, the prose "nine" and the `adds` set-equality intact. |
| **PF-2** | **The `DebateColumn.tsx` docblock clause naming the removed `Đ BET` trigger is struck at C1, NARROW.** The `"No active position"` stub clause STAYS. | Item 6 deletes the Button, not the stub. §6's "Orphans this change creates" enumerated only the `Button` import, so the docblock orphan had no ruling. ⚠ It proved to be **one of six** instances of the same class — the plan's orphan enumeration is systematically blind to *sibling prose references*, catching a deleted symbol's own docblock but not the neighbouring paragraph describing what the component renders. The other five were swept at `3e012a9`. |
| **PF-3** | **C4's guard is authored with TARGETED QUERIES ONLY.** That discharges §18 C-4 **by construction**, and C5 additionally **measures** it. | A `container.innerHTML` pin, a snapshot, or a container-wide `textContent` assertion in `market-header.test.tsx` would sweep in the dev placeholder box, which C5 removes from the same component one commit later — turning a two-line copy guard into a tripwire for an unrelated deletion. Construction is not measurement, so C5 runs the file and records the actual output. ⚠ **Scoped to COPY guards — see PF-7.** |
| **PF-4** | **§18 R-b's citation of `PageContainer.tsx:19-23` is imprecise. RECORD, do not fix.** | Those lines name D2b and "POLISH .2/.3/.5/.6" but do **not** name `/m/[slug]`; `:36` does — *"the debate surface `/m/[slug]` (site 9) — the only px-6"*. R-b's other ground is exact (`:40` scopes `notice` to the branded 404). **Substance holds; only the line reference is off.** Instance of V-8 — a line citation into a living document. |
| **PF-5** | **§18 R-a and §9 C3 both predict a RED that CANNOT OCCUR.** The observed failure is a set-membership assertion. Recorded at `9468c30` by execution. | Both predict `callSite` throwing `no <PageContainer> found in …`. `callSite` is **not reachable from either set-equality direction** — neither calls it — and at `9468c30` the `it.each(GREENFIELD)` row that does call it did not yet exist. Even reached, it reads the file first, so a nonexistent path throws `ENOENT` before the regex. The shipped guard compares against the set of real tree call sites and yields `… is declared but renders no <PageContainer> on disk`. **Do not hunt the predicted string in any log.** |
| **PF-6** | **`docs/plans/POLISH-3.md` §5 G-4's "paste as `DETAIL_BASELINE`" is CORRECT as a default and is deliberately NOT edited here** (§19 is append-only). What it must never do again is supply the **wording** of the receipt. | Capture-and-paste makes byte-identity **structural** rather than measured, so the prescription is right. But execution edited the literal **in place**, then described that work in the plan's prescribed words — and one pre-authored sentence propagated to the commit body, the shipped file, the run report, the `commits.txt` deliverable and the PR description. **A receipt records the mechanism USED, never the mechanism prescribed.** Resolving `:230` at source belongs to PR 2's plan-mode chat, where editing the plan is in scope. |
| **PF-7** | **Guard form is keyed to guard CLASS, not to surface.** COPY / PRESENCE guards take **targeted queries**. ABSENCE / LEAK guards take the **container-wide** form. | A targeted negative proves only that the secret is absent from the nodes queried, which says nothing about a second render path — **V-3 pointed at a DOM**. `CLAUDE.md` §5.14 SC-1 mandates the container-wide form (*"assert the BODY's absence, not the row's"*), and `auth-error-boundary.test.tsx` — the precedent D4 names — uses `container.innerHTML` for exactly this. ⚠ **PF-3 was carried onto a leak guard without checking its ground transferred; it did not** — nothing removes content from `error.tsx`, and `not.toContain` is insensitive to unrelated content movement regardless. **PF-3 stands unchanged for `market-header.test.tsx`.** ⛔ The real fragility is a snapshot or byte-pin, not query breadth. |
| **PF-8** | **An absence assertion's scope must be the WHOLE RENDERED OUTPUT, and `container` is not that.** `baseElement` is. ⚠ **PF-7 named the right principle with the wrong instrument** — its rule stands, its named scope is corrected here. | React portals render OUTSIDE the parent DOM node, so a `container.innerHTML` string-absence assertion never sees a secret rendered through `createPortal`. **Measured at R7:** a probe rendering `{error.message}` through a portal passed the primary leak guard **GREEN**. ⚠ **This repo's `Dialog` portals BY DEFAULT** (`src/components/ui/dialog.tsx` wraps `DialogContent` in `DialogPortal`), so a "Show details" modal built from components already on disk lands outside `container` **by construction** — the likeliest future leak was the invisible one. **Naming a scope is not verifying one.** ⚠ **And the structural assertion is the PRIMARY guard; the DOM sweep is corroboration.** Every residual — portals, event vocabulary, nodes revealed by a later click, deferred reads — exists because a sweep is PROCEDURAL, and no enumeration of surfaces and events is ever closed. `error` is never bound, and **no binding implies no read** — everywhere, for every event, forever. Asserted from source at R7; it catches a bare `error,` in the destructure **with no read at all**, which no DOM sweep can reach. Routed: `PORTAL-SCOPED-ABSENCE`. |

**Two IDs were requested and are deliberately NOT used.** `O-7` already exists at `docs/logs/STAGING-PARITY-A.md` as a staging re-seed gap while `CLAUDE.md` §8's committed O-space runs **O-1…O-4**, so a bare `O-7` would resolve to the wrong definition. `V-12` would skip V-9, V-10 and V-11 — V-space's ceiling is **V-8**, and V-8's own text rules that a V-number is minted in `POLISH-0_data-manifest.md` §5 *"or it is not minted"*. **The next free V-number is V-9**, and the mint is web's at close-out, in that file, which is not on this PR's allow-list. The lesson itself — *a comment added in a diff is an unverified claim, and reviewers read added comments as findings-free* — is recorded here descriptively, with no number attached.
