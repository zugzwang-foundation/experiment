...
| **P-6** | §8's `AUTH-TURNSTILE-WIRE` row and §4's D03/D19 notes carry coordinates measured at the recon SHA `903b2a1`. **D03's own restructure moved them inside this PR.** The plan is not rewritten; `docs/parked.md`'s row carries head coordinates and is the navigational source of record. ⚠ **The general rule, and it is the one this surface keeps re-learning: a plan authored before execution cites a tree that execution changes.** Every coordinate a plan hands forward to a LIVE docket must be re-measured at PR head before that docket row lands |
