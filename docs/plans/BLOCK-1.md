...
| `docs/data/staging-markets-snapshot.*` | Refreshed in S1 (currently stale — six descriptions have grown 3.8–5.1× since it was captured). |
...
| 2 | RF-2's table shows `RESOLUTION == RESOLVER` (both "CoinMarketCap" / both "Oktoberfest") for 2 of 8 rows, vs. `RESOLUTION == "X"` for the other 6. Could be a typo (RESOLUTION should read "X" uniformly) or intentional. | Intentional, verbatim. RESOLUTION reads "X" only for the 5 markets resolved by watching an X account's activity; for the 3 markets whose evidentiary basis is a named institution's own data (a festival's own report, a price index, a code host's own count), RESOLUTION names that institution too, mirroring RESOLVER. | Silently "correcting" all 8 RESOLUTION values to "X". | RF-2's own instruction is "verbatim, do not paraphrase" — explicit pre-emption of exactly this temptation. The pattern is coherent and repeats twice (oktoberfest, bitcoin), which argues against a one-off typo. U-3's href rule ("opens Zugzwang's own X post") is a claim about the **link target**, not the **display text** — the two are independent, and RESOLUTION's href is null on all eight today regardless. |
