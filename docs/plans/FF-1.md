...
| A-2 | "anchor occurs exactly once, byte-for-byte" — bare substring or line-bounded? | **bare substring** (the file's own semantics: B3.7's note says the site is "the line containing it", and 15 of the 36 anchors are line FRAGMENTS that match zero times line-bounded) | line-bounded | line-bounded would have CARRIED fifteen blocks to rescue one; under substring, exactly one block is CARRIED — B5.2, whose `## Patch record` occurs twice in ADR-0017 (`:15` and the `## Patch record — 2026-09-05 …` heading at `:471`). The rule as written is applied; B5.1's `Amended-by` row still points ADR-0017 readers at ADR-0058 |
...
| A-13 | Required vs optional for the NEW fields on the two type layers | SUBSTRATE types (`PostSubstrate.endorseCount/contestCount`, `ReplySubstrate.friendlyFire`) REQUIRED; DTO types (`DebateReply.friendlyFire`, `ReplyAggregate.friendlyFireDharma`) OPTIONAL, always set by `loadDebateView` and pinned by its integration test | required everywhere | the substrate has three constructors and a handful of fixture builders, so a compile error there is cheap and catches the one failure that matters (a site re-instating the side formula); the DTO is constructed as a literal in ~25 render/export fixture files, and forcing every one to carry `friendlyFire: false` is churn with no guard value — a fixture that omits the flag renders no tag, which is the correct reading of "unset" |
| A-14 | `PlaceParams.friendlyFire` required or optional | OPTIONAL (`?? false` inside `place()`), the same shape as the existing `image` field | required | tsc enumerated 32 direct `place()` callers (fixture, scale and staging harnesses placing ordinary bets) for which "omitted" can only mean `false`; the one live caller (the route) always states it; the in-tx guard is what protects the value, not the type |
| A-12 | "beneath the Support pill" — the Support/Counter TriggerPills live in `ReplySplitBar` (the post-focus header), not in the composer, and the phone tier's relation control is a tab outside the sheet | the switch renders INSIDE `BetComposer`, directly beneath its header row (the row that names `Support <author>'s argument`), on both tiers | beneath the `ReplySplitBar` Support pill with state lifted into the host | design-language B4.4 says "in the reply composer" and RF-8 says "Submit carries `friendlyFire`" — the composer owns submit; the phone sheet has no pill to sit beneath; one implementation serves both tiers (A-10). The morning screenshots show the placement for ratification. |
