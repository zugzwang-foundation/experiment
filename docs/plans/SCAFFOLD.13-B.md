...
- **No external Deploy Hook fires** (https://vercel.com/docs/deploy-hooks): if any cron job, scheduled GitHub Actions workflow, monitoring tool, or external integration is configured to call a Vercel Deploy Hook URL, operator MUST disable or window-block these for the B5c→B6a-smoke duration. Pre-flight check before B5c: review Vercel project Settings → Git → Deploy Hooks for any configured hooks, AND review `.github/workflows/` for any cron-scheduled job that triggers Vercel deploys via webhook.
- **No collaborator pushes** to deploy-wired branches: if any non-operator GitHub collaborator has push access, operator must coordinate the B5c→B6a-smoke window. (Current Zugzwang `zugzwang-foundation/experiment` repo is solo-operator; this is theoretical hardening for future strata.)
- **No `vercel deploy` / `vercel --prod` CLI invocations** from operator's local shell during the window.
...
**Conflict-time behavior observed (capture in §14 SURPRISE if anything surprises):** ~~Doppler may overwrite, skip, or error on Vercel's existing same-name vars. Because values match (B4 sequencing + Amendment 1 copy-paste), the end state is correct regardless.~~ **STRUCK per Amendment 16 / Round 5 cascade-emergent fix:** B5c clean-slate (preceding step) eliminates Vercel-side collisions before B6a fires; Doppler writes fresh vars with no possible collision. Per B0 yield report (2026-05-18), Vercel rejects rather than converges on name-collision; the prior value-match framing was invalid. Note any operator-observed B6a sync behavior in §14 for future plans regardless.
...
**Accepted residual (per Amendment 18 / WI4 + Round 5 security-auditor MEDIUM finding):** an intrinsic Vercel-platform window exists between B6a sync-enable and step-4 literal-value assertion. Doppler auto-redeploys via webhook (per §0.4) before step 4 can fire; if the AMBIGUOUS Sensitive-flag posture resolves as non-Sensitive, the 12 vars are `vercel env pull`-readable during the ~1–5 min auto-redeploy + smoke window. Window-elimination would require a throwaway-Vercel-project pre-flight probe (auditor's F6c-equivalent in Round 3), which Amendment 10 rejected on cost-benefit. Window is bounded (single auto-redeploy cycle), detected (step 4 surfaces it), and recoverable (Q9 (a) pre-committed). Accepted as the cost of the lighter B0 / WI4 pre-flight model.
...
- **If B6a-smoke steps 1–3 PASS but step 4 FAILS (any key shows non-Encrypted / "visible on click"):** **HALT B6b. Execute pre-committed Option (a) abandon per §9 Q9 (web-Claude PRE-COMMITTED 2026-05-18 — no in-execute adjudication required):** disconnect B6a integration via Path A (Doppler-side sync delete, **checkbox CHECKED — explicitly tick "Delete all secrets in Vercel"; do NOT rely on UI default**) → Vercel state cleared of the 12 (non-Sensitive) Doppler-synced vars → re-populate Vercel directly from B5b backup, manually adding each of 12 vars via Vercel dashboard with **Sensitive flag ON at create-time** → resume direct-Vercel-Sensitive posture (Phase B aborted for this attempt). Recovery <30 min per Q9 (a). Document outcome in §14 SURPRISE. **Rationale for CHECKED (deviation from Amendment 17 "lowest-disruption UNCHECKED" framing):** Amendment 17 UNCHECKED applies to GENERAL disconnects where Vercel vars are correctly-flagged (Sensitive). Q9 (a) recovery fires precisely BECAUSE Vercel vars are NON-Sensitive (failure state); retaining them via UNCHECKED would preserve the failure. CHECKED clears the failure state, enabling clean re-population as Sensitive. Per Vercel community thread 38766, *"you cannot change the type of an integration environment variable to sensitive"* — so direct in-place fix is unavailable; clean-slate-and-readd is the only path.
...
| **B6a-smoke step 4 fails — non-Sensitive flag observed on any of 12 keys (NEW per Amendment 18 / WI4 AMBIGUOUS escalation; Q9 PRE-COMMITTED Option (a))** | Doppler-synced vars appear as non-Sensitive / "visible on click" in Vercel (contradicts Doppler default-Sensitive claim) | **HALT B6b. Execute pre-committed Q9 Option (a) abandon** (no in-execute adjudication — web Claude verdict 2026-05-18): disconnect B6a integration via Path A (Doppler-side sync delete, **checkbox CHECKED — explicitly tick "Delete all secrets in Vercel"; do NOT rely on UI default**) → Vercel state cleared of the 12 non-Sensitive Doppler-synced vars → re-populate Vercel directly from B5b backup via manual dashboard adds with **Sensitive flag ON at create-time** → resume direct-Vercel-Sensitive posture (Phase B aborted). Rationale for CHECKED deviation from Amendment 17 UNCHECKED framing: see §4 B6a-smoke step-4 fail branch. Document substrate-downgrade avoidance in §14 SURPRISE. | <30 min per Q9 (a) pre-committed verdict |
...
9 numbered items at re-emit round 5 close: 4 RESOLVED in round 1 (Q4 / Q5 / Q7) + round 2 (Q8) + 2 RESOLVED in round 3 (Q2 / Q3) + 1 PRE-COMMITTED RESOLUTION in round 5 (Q9 — web-Claude verdict 2026-05-18) = 7 RESOLVED total + 1 unresolved-mitigated (Q1; mitigation REPLACED in round 5 / Amendment 16) + 1 unresolved-low-risk (Q6). **Q9 surfaced from round-5 cascade analysis driven by WI4 mini pre-verification (Amendment 18); pre-committed to Option (a) abandon per web-Claude 2026-05-18 with B6a-smoke step 4 as the in-execute trigger.**
...
- **Option (a) — Abandon Doppler-canonical (PRE-COMMITTED VERDICT):** Disconnect B6a integration via Path A (Doppler-side sync delete, **checkbox CHECKED — explicitly tick "Delete all secrets in Vercel"; do NOT rely on UI default since Doppler does not document the default**) → Vercel state cleared of the 12 non-Sensitive Doppler-synced vars → re-populate Vercel directly from B5b backup via manual dashboard adds, **each var created with Sensitive flag ON at create-time** → resume direct-Vercel-Sensitive posture. Phase B aborted for this attempt; defer Doppler integration to a later stratum once Vercel's per-integration Sensitive support lands (no ETA per Vercel staff). **Cost:** ~30 min revert (1 disconnect + 12 manual var-creates); loses Doppler-canonical automation benefit (Doppler-side canonical storage preserved; only the Doppler→Vercel sync workflow regresses, recoverable at testnet phase once Vercel's per-integration Sensitive support GAs). **Rationale for CHECKED (deviation from Amendment 17 "lowest-disruption UNCHECKED" framing):** Amendment 17 UNCHECKED applies to general disconnects where Vercel vars are correctly-flagged Sensitive. Q9 (a) fires precisely because Vercel vars are NON-Sensitive (failure state); retaining them via UNCHECKED would preserve the failure. Per Vercel community thread 38766, *"you cannot change the type of an integration environment variable to sensitive"* — direct in-place flag-fix is unavailable; clean-slate-and-readd is the only path. CHECKED clears the failure state and unblocks Sensitive re-population.
...
6. **Reviewer-call format discipline carry-forward (NEW per Amendment 13 / round 3; recurrence-watch ESCALATED post-Round-5):** reviewer outputs must lead with the findings table, no preamble, no thinking-fragment text. SCAFFOLD.13-B's first security-auditor invocation (round-2 plan-review surface) opened with one line of preamble ("I now have the full picture. Composing the report."). Round-3 re-invocation tightened the prompt instruction. **Round-5 security-auditor invocation (post-Amendments 16–19) recurred with ~4 preamble lines** ("Confirmed: §9 intro at line 752 is stale relative to Round 5..." — opening with a stale-finding observation before the findings table). This is the second occurrence post-Amendment-13's escalation gate. **Round-5 action:** logged in §14 (recurrence-watch); escalation candidate is `.claude/agents/security-auditor.md` hardening (add explicit "no preamble; lead with findings table" rule to briefing) OR `docs/maintenance.md` amendment (codify reviewer-output discipline). Operator + web Claude adjudicate the escalation in a follow-up after Round 5 PR merges. Not blocking Round 5 close.
...
- **Reviewer-call format discipline meta-finding (NEW per Amendment 13 / round 3 recurrence-watch; ESCALATED to pattern signal post-Round-5):** first security-auditor invocation (round-2 plan-review surface) opened with one line of preamble ("I now have the full picture. Composing the report.") — violates §6 + 13-A process observation 2 ("output format: report only, no preamble"). Round-3 re-invocation included tightened prompt prescription per §6 update. **Round-5 invocation (post-Amendments 16–19; 2026-05-18) recurred:** opened with ~4 preamble lines ("Confirmed: §9 intro at line 752 is stale relative to Round 5..." — leading with a stale-finding observation before the findings table). **Two occurrences post-Amendment-13 tightening = pattern signal worth structural fix.** Escalation candidates per Amendment 13's gate: (a) `.claude/agents/security-auditor.md` hardening (add explicit "no preamble; lead with findings table" rule to briefing); (b) `docs/maintenance.md` amendment (codify reviewer-output discipline). **Operator + web Claude adjudicate the escalation in a follow-up after Round 5 PR merges.** Execute-phase close-out invocation will be the third data point. Logged in §12 + §14.
...
**Round 3 history (preserved for audit trail):** plan file landed canonically via PR #41 (commit 15551b2, merged 2026-05-17). Round 3 sign-off scratch resided at `.claude/scratch-scaffold-13-b-round3.md` (gitignored) for web-Claude re-review pass before PR #41 promotion. Execute-phase session has not yet fired (B1 onward pending Round 5 PR + auditor PASS + web-Claude ratification).

**Round 5 status (2026-05-18):** plan amendments 16–19 committed to branch `feat/scaffold-13-b-amend15` (commit 0d41d03; this commit). Round 5 sign-off scratch resides at `.claude/scratch-scaffold-13-b-amendment15.md` (gitignored). Security-auditor re-fire fixes (Round 5 loop) committed atop. After auditor PASS clean + web-Claude ratification, PR opens against `main`; post-merge, fresh execute-phase session opens at A1.
...
11. **§0.4 Condition-1 footnote placement** — chose post-table italic blockquote rather than table-cell continuation (markdown blockquotes don't render inside table cells); the new Sensitive-flag row's third cell carries a "see post-table footnote" pointer to the blockquote. Footnote is anchored AFTER both the "Symmetric pre-verification rule" paragraph AND the "B0 pre-flight closed the two undocumented items" paragraph (i.e., the footnote is the final block under §0.4 before §0.5). The pointer in the row text resolves to the only post-table italic blockquote, so anchor position is correct.
...
13. **Security-auditor loop — HIGH absorbed:** auditor caught Q9 (a) recovery procedure logic gap (Path A checkbox UNCHECKED retains the leaked non-Sensitive vars; subsequent repopulate hits collision wall B5c was designed to prevent). **Fixed Round 5 / commit-loop-2:** Q9 (a) procedure rewritten in §4 B6a-smoke step-4 fail branch, §7 B6a-smoke step-4 fail row, AND §9 Q9 Option (a) verdict text — checkbox CHECKED (explicit tick; do NOT rely on UI default) → Vercel clears 12 non-Sensitive vars → manual dashboard repopulate with Sensitive flag ON at create-time. Rationale (deviation from Amendment 17 UNCHECKED framing) documented inline: UNCHECKED applies to general disconnects with correctly-flagged vars; Q9 (a) recovery operates on the failure state where vars are NON-Sensitive, so UNCHECKED would preserve the failure.
14. **Security-auditor loop — MEDIUM absorbed (§4 B6a stale prose):** "Conflict-time behavior observed... values match (B4 sequencing + Amendment 1 copy-paste), end state is correct regardless" — surviving pre-Round-5 prose contradicting Amendment 16. **Fixed Round 5 / commit-loop-2:** struck and replaced with B0-yield-aligned framing pointing to B5c clean-slate. Round 5 cascade-emergent check item 1 self-claim about coverage (originally false) is now correct post-fix.
15. **Security-auditor loop — MEDIUM absorbed (leakage window):** intrinsic Vercel-platform window between B6a sync-enable and step-4 detection (auto-redeploy bakes vars into runtime before step 4 can fire). **Fixed Round 5 / commit-loop-2:** documented as accepted residual in §4 B6a-smoke section. Window is bounded (single auto-redeploy cycle), detected (step 4), recoverable (Q9 (a) pre-committed). Window-elimination would require throwaway-Vercel-project probe rejected in Amendment 10 on cost-benefit.
16. **Security-auditor loop — MEDIUM absorbed (window-discipline enumeration):** §4 B5c HARD CONSTRAINT did not enumerate Vercel Deploy Hooks, scheduled GitHub Actions, collaborator pushes, or `vercel deploy` CLI as forbidden during the B5c→B6a-smoke window. **Fixed Round 5 / commit-loop-2:** bullet list expanded with all four classes; pre-flight check directive added (review Vercel Settings → Git → Deploy Hooks; review `.github/workflows/` cron jobs).
17. **Security-auditor loop — LOWs absorbed (§9 intro stale + amendment-log §8.2 #9 → #10 + §11 #20 cross-ref expansion + §15 sign-off rewrite + cascade-check item 11 self-correction):** five documentation-drift LOW findings fixed in commit-loop-2; each captured in the appropriate section.
18. **Security-auditor loop — format-discipline meta-finding (Amendment 13 / §14 recurrence-watch):** auditor opened with 4 lines of preamble before findings table ("Confirmed: §9 intro at line 752 is stale relative to Round 5..."), violating Amendment 13's "lead with findings table; no preamble" prescription. **This is the second recurrence post-Amendment-13:** Round 2 plan-review surface had 1 preamble line; Round 5 post-Amendments invocation has ~4 preamble lines. Pattern strengthening — escalate to `.claude/agents/security-auditor.md` hardening OR `docs/maintenance.md` amendment per Amendment 13's escalation gate. Logged in §12 + §14 carry-forward; not blocking Round 5 close.

**Verdict (post-loop):** no new cascade-emergent open questions surfaced. **Round 5 closes coherently after security-auditor loop absorption.** Items 1–12 are implementation details of Amendments 16–19 (originally scratched); items 13–18 are the post-auditor-loop absorption (HIGH + 2 MEDIUMs + LOWs + meta).
