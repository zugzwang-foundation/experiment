...
| 2 | `src/components/ui/relative-time.tsx` | **new** | S2. The render leaf. Reads `Date.now()` at render, renders one plain text node. ⚠ **It carries NO `"use client"` directive** — see A7, which supersedes this row's original wording ("the one `"use client"` leaf"). |
| 3 | `src/components/debate/ArgProfile.tsx` | edit | S2. Adds a required `createdAt` prop; mounts the leaf as the last child of the meta row — **positionally after the row's last pre-existing `Sep`, and carrying no `Sep` of its own** (A9). ⚠ **DISAMBIGUATED, because the original wording had a true reading and a false one.** It read *"after the existing `Sep` divider"*, which is accurate about the POSITION — the last `Sep` is the one inside the `Replies · N` fragment, and the leaf follows that fragment — but reads naturally as *"a `Sep` precedes the age"*, which is the thing **A9 rules out**. The false reading is not hypothetical: **S2 (`0711739`) did ship `<Sep /><RelativeTime …/>`, and S3 (`9d884b7`) removed that `Sep`.** So the sentence was written true of the code of its moment and became misleading one commit later. Rewritten rather than deleted, so the ambiguity stays on the record. **Covers post card, focused post, reply card and both pop-ups at once.** |
...
| 8 | `src/components/discovery/HeroPanels.tsx` | edit | S3. Hero post identity row, after the Đ figure — **and with NO separator before it**; the age separates on the row's own `gap-1.5`. ⛔ **AND THE CLAUSE THAT USED TO SIT HERE — *"with a `HeadSeparator`"* — IS STRUCK.** It is the pre-execution position, reversed at **A9** in this same document, and the shipped hunk adds no `HeadSeparator` element at all: the only `HeadSeparator` string it introduces is inside the comment `⛔ NO HeadSeparator BEFORE IT` that explains the absence. A row stating the superseded position above its own correction is exactly what O-5 forbids — struck here rather than annotated beneath. |
...
are renumbered to the report's sequence; the gaps (`A2`, `A3`, `A12`–`A15`) are
decisions taken DURING the run and recorded only there.

⚠⚠ **AND THE FIRST ATTEMPT AT THIS FIX RECREATED THE COLLISION IT CLOSED.** It
renumbered eight rows correctly and then minted a NINTH id — `A16` — for the
`"use client"` decision, which the run report already numbers **`A7`**. It
simultaneously listed `A7` as a "gap … recorded only there", which was false:
`A7` was not missing, it was the very row being given a second name four lines
below. One decision, two ids, in two documents — the exact defect this note
claims to have ended, on the most-cited decision in the whole cascade
(`@code-reviewer` NEW-1). The report's id wins because it was there first and
because every citation in the cascade already used it.
...
| A8 | Removed posts/replies | **No timestamp on any removed variant** | Render it (the `.md` export does) | On market detail the removed branches render **no identity row at all** (`PostCard.tsx:76-108`, `ReplyCard.tsx:43-50`, `PostFocusHeader` removed arm) — there is no row to append to, and inventing one would be a new feature. On Profile, `RemovedHead` (`:392`) is a deliberately different cluster. Uniform rule: **present cards carry it, removed stubs do not.** ⛔ **AND THE CLAUSE THAT USED TO SIT HERE — *"Also the security-conservative direction"* — IS STRUCK.** It is the superseded position, and leaving it beside its own correction is exactly what O-5 forbids: a reader who stops at the first sentence takes the meaning the amendment exists to remove. ⚠ **AND IT IS A PRODUCT DECISION, NOT A SECURITY CONTROL** — `@security-auditor` was explicit that it be labelled so. `createdAt` is a declared structural field that SURVIVES masking (`load-debate-view.ts:52-54`, ADR-0020/0021 thread integrity), and the exact instant is already public at FINER precision to an unauthenticated visitor through the `.md` export (`debate-export/serialize.ts:310`, `:336` emit `Time: YYYY-MM-DD HH:MM UTC` on the removed branch). The `ordinal` on both variants already brackets a removed node between its neighbours. So an age on a removed card would be SAFE; it is simply not what shipped. Recorded this way so a future ruling that reverses it is not mistaken for a security regression. ⚠⚠ **ONE PRECONDITION ATTACHES TO REVERSING THIS, and it is not optional.** Reversal is safe **once `src/server/debate-view/load-debate-view.ts:334` renders the substrate instant instead of `new Date(0)`**. Before that, a reversed A8 prints roughly `20693d ago` on the `!comment` race path — a fabricated fact, and a visible tell that the node is a substrate stub rather than a moderated removal, which is a disclosure the UI does not have today. ⛔ So reversing A8 first would CREATE the regression this relabelling exists to pre-empt. Full trace in `claude-progress.md` — which is **gitignored**, which is precisely why the precondition is restated here, in a committed file (`@security-auditor` F-11). |
| A6 | Hydration | Compute `Date.now()` at render inside the leaf + `suppressHydrationWarning` on the text element (load-bearing on **every** surface — see A7) | A one-shot `useEffect` re-read at mount; or serialize a server `nowMs` into the payload | All three routes are dynamic (`force-dynamic` ×2; the profile page reads `headers()` and its own docblock says "UNCACHED / dynamic v1"), so the SSR→hydration gap is seconds, not cache-age. A serialized server-now would go stale on any future caching; a mount effect is a second render per card for a sub-second correction. `suppressHydrationWarning` is React's documented escape hatch for exactly this and costs nothing. **No timer, no interval, no rAF, no subscription** — R6. |
...
| B3 | client bundle — **emitted client JS bytes and chunk count**, from two full production builds; plus `package.json` | ⛔ **THE ORIGINAL LAYER HERE WAS WRONG AND IS STRUCK.** It said "`\"use client\"` leaf count … the file that carries the directive is what decides whether a subtree is client". It does not: a file with NO directive still ships to the browser whenever a client module imports it, which is exactly what happens on all three surfaces here (A7). That layer measures the shape of the SOURCE; the reader pays for the shape of the DOWNLOAD. Emitted bytes, from two clean builds of the same tree at two commits, is the only honest layer. **MEASURED: 1,467,471 B / 38 chunks at `1fc741b` → 1,469,009 B / 38 chunks on this branch = +1,538 B (+0.105 %), no new chunk.** Two clean builds of the same tree at two commits, each preceded by `rm -rf .next`, identical placeholder env. `package.json` + lockfile: empty diff. ⚠ **A layer defined and not run is the row that would have caught the C-1 error before the cascade** (`@code-reviewer` NEW-2) — so the number is recorded here and not only in the run report. |
