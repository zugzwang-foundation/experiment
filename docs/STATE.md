...
| **F-7** | **`BETTER_AUTH_TRUSTED_ORIGINS` is absent from Doppler `prd`.** `src/server/auth/index.ts:330-333` reads it and falls back to `[]`; `docs/plans/SCAFFOLD.8-staging-plan.md:173` ratifies `prd = https://zugzwangworld.com`. No decision to drop it is recorded anywhere | same | |
