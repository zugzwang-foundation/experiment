...
| **F-18** | ✅ **DISCHARGED at SYNC-6 · CLEANUP.** The script now completes — `OK: staged 60 content files`, exit 0. The gate asserted `EXPECTED_ADR=29` against a tree holding 43, so it had been dead since the thirtieth ADR merged and would have died again at the forty-fourth. **Raising the number was not the fix**: `SOURCES.md` — which the script's own next gate compares the staged set against — names 29 ADR basenames, so a bumped count would only have moved the failure one gate down. ⇒ **The ADR set is now READ FROM `SOURCES.md` rather than globbed from `docs/adr/`, and `EXPECTED_TOTAL` is derived from its row count**; no hardcoded number remains, and the count gate can no longer disagree with the manifest gate because they read the same file. Tree-side drift (43 in the tree, 29 in the kit — **14 ADRs the reviewer kit does not carry**) is now printed as a `note:` and is **deliberately not fatal**: whether a new ADR belongs in a reviewer's knowledge base is editorial, and a staging script is the wrong place to decide it. ⚠ **Two controls, because a gate that cannot fail is not a gate** — (a) `origin/main`'s copy of the script against the same tree still exits 1 at the stale gate; (b) a manifest row naming a nonexistent ADR still exits 1 on the new fatal path, verified on a throwaway branch that was deleted | `bash scripts/stage-reviewer-project.sh --dest <tmp> --sha origin/main` | SYNC-6 · CLEANUP |
