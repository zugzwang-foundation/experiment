...
| **PD-7a-04** | No error boundary on `(auth)` — an uncaught throw escalated to the whole-document `global-error.tsx` | R | tier-2 · W2.11 kit P1–P7 · `POLISH-0.md:450` §7.4 | `6635201` | routed | fixed | no state file had ever been added to the group | **SHIPPED** — D20, ruled at R-C. `loading.tsx` deliberately NOT added (POLISH.2's `page.tsx:28-30` precedent) and the omission is pinned by a test |
...
| **PD-7a-18** | ToS and Privacy bodies are placeholder Lorem ipsum, rendered in full on the acceptance screen | — | **tier-1** · `SPEC.1.md:776` requires the full text in-page | `public/legal/tos.txt:1` | **data-blocked** | open | content, not build | Pre-recorded at `POLISH-0.md:174`. ⚠ **`LEGAL.1` is a GO-LIVE GATE** — acceptance evidence for Lorem ipsum is not a dispute-resolution record, and it carries the relocated AGPL §13 source offer |
