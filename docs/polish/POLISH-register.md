...
| **PD-0-16** | The chart's **expanded-overlay** variant has no tier-2 or tier-4 baseline; values-log branded three renders, four exist | S | — (no baseline of any tier) | .3 | — | open | **R13 · OPEN — the only open ruling. See POLISH-0 §0.** `SPEC.CHART` is cited as a TIER-1 source in `POLISH-0.md` §3 · POLISH.3 · Tier 1 and **does not exist in `docs/specs/`** — verified 2026-08-10. The expanded-overlay is therefore built UI with **no baseline at any tier** → class **S** → **SPEC-FIRST halt**. Two admissible outcomes, both founder-owned: write the spec, or record the overlay as permanently unbaselined and accept the divergence (P12). See `SPEC.CHART` in `docs/parked.md`. |
