...
| **.7a Auth** | ❌ | ❌ | **none** | single gated pass | ▶ **RUN FIRST** |
...
| **PD-2-32** | ⚠ **A PRODUCTION defect, not staging-only.** A minted R2 URL that later 404s has **no degradation path**. Three sites — `MarketCard:53`, `HeroPanels:64`, and the hero POST image, which handles null but has no `onError`. **One shared `MarketThumb` owning null · error · loaded** — never N patched `<img>`s, which is how the three-implementation problem happened | **PRIMITIVES-2** |
| **PD-2-33** | Market-thumb `alt` duplicates the adjacent title, and is what makes the failure *overflow*. ⚠ **Lands WITH PD-2-32** — fixing it alone hides the symptom | **PRIMITIVES-2** |
...
| **SideBadge presets** | ⚠ **UNOWNED — no register row exists.** d5's `.md` (10px) and Profile's `.sm` (8.5px) have no preset to receive: `badges.tsx:70` is `size?: "hero"`, a one-member union. The routing sentence in the DISCOVERY-COMPLETE plan sends Profile's to `.6`, which is wrong — that mockup is `.5`'s | **`.3` and `.5`.** Mint the rows at kickoff |
...
| Row | When | Note |
|---|---|---|
| **PRIMITIVES-2** | ▶ **next, after this task** | Scope in §5. ⚠ Carries `.7a`'s and every later surface's shared presets |
