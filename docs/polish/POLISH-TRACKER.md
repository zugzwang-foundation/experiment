...
| **CC-2** | ⚠ **Both halves corrected at CHART-2 (2026-08-27).** The reusable form is **`1 + 11N`**, not `1 + 12N` — CHART-1 moved the pin down by one N and the machine pin moved with it (`tests/server/discovery/round-trip-budget.test.ts`, `discovery::round-trip-count-is-1-plus-11N`). And `AGENTS.md` §3 **does not carry** the superseded string this row sends a sweep to fix: its only round-trip sentence is PERF-1's latency figure. The real stale site was `docs/parked.md:818,822`, corrected at CHART-2 | doc sweep |
