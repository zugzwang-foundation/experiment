...
**11 SCHEDULED · 8 RULED · zero OPEN · one (R2) routed out of POLISH.** A `SCHEDULED` ruling is not a blocker — it is a disposition whose owner and timing are named. ⚠ **No ruling in this index is OPEN as of 2026-08-12.** R13, the last one, closed at `design-canon.md` §10 `C-CHART-1`; **nothing in this index stops work.** ⚠ **The figures above were recounted from the table on 2026-08-12 and did not previously match it** — the prior sentence read *"Nine SCHEDULED · eight RULED"* against a table carrying different counts, **wrong in opposite directions and not reconcilable by any reading of R2.** A tally is a measurement, and this one had drifted from the thing it measures. **`POLISH-TRACKER.md` §2 carried a second, differently-drifted copy; it now points here rather than restating.**
