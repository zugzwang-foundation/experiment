...
**Twenty** rulings are cited across this document and `POLISH-register.md`. Each has exactly one of three states. **No ruling is "pending" any more; a ruling that has not been taken is `OPEN` and says what it blocks.** *(Nineteen until POLISH.4 PR A commit A5 filed **`R20`** — the first `OPEN` row this index has ever carried, and the row that makes the rule above do work rather than describe an empty case.)*
...
| **R11** | Sell button hidden vs disabled when ineligible | **SCHEDULED** | Disposition set at POLISH.5 kickoff. `PD-0-12`. Same P12 constraint as R10. The server-side type split (a visitor's DTO carries no `sellEligible` field at all) is the stronger property · ⚠ **THE SCHEDULED EVENT HAS PASSED AND NO DISPOSITION IS ON `main`** — recorded at POLISH.4 PR A commit A5 (`PD-4-09`), routed, **not `.4`'s to rule.** `.5`'s three PRs (**#331 · #333 · #340**) have all merged and nothing here or in `POLISH-register.md` records a disposition for `R11`/`PD-0-12`. `.4`'s own Pre-recorded cell carries the row (*"the decision is server-side; the render is `.5`'s"*), so **`.4` must not treat it as discharged and must not inherit it silently.** ⚠ **Root cause worth keeping: a disposition scheduled against a *kickoff* rather than against a *commit* has no artifact that goes red when it is skipped.** Destination: `docs/parked.md` → `POLISH-4 PD-4-09` |
...
| **R20** | ⛔ **`OQ-1` — the Đa staked-basis ruling** | **OPEN** | **FILED AT POLISH.4 PR A, commit A5 (`PD-4-08` / `OD-4`, founder-ruled 2026-08-16 — the founder mints the number, `.4` files the row).** ⚠ **It was a LIVE hold all along and this index asserted it did not exist.** The hold is quoted **in source** at three `.4` components: `SellModule.tsx` (the module docblock) · `SlotHeader.tsx` (the `yourPositionLabel` readout) · `PositionStrip.tsx` (the component docblock). Tier 3 states the law — `docs/plans/UI-A3.md`: *"**HELD — founder ruling pending.** Law stands verbatim: strip = Đb-only; sell module = You-receive-only; **rendering Đa or P/L before the SPEC.1 line lands is a defect.** The ruling, whenever given, is its own web-authored SPEC.1 line and is NOT carried by A3."* **WHAT IT BLOCKS, named as this section requires:** (1) `PD-4-07` — the sell module's **P/L** readout, which `DESIGN_W2_10` panel E names as an add and which tier 3 forbids until the SPEC.1 line lands; (2) the **`Đa → Đb` grammar** at all three components above, which stay **Đb-only**; (3) POLISH.4's **`H-OQ1`** halt, which fires on any row rendering `Đa` or a P/L figure. ⛔ **The ruling is its own web-authored SPEC.1 line — CC never authors it.** ⚠ **This is the inverse of `R13`'s lesson**: there, a *missing source* was allowed to imply a missing baseline; here, a *present index* was allowed to imply an absent hold. Both are **reading a document's silence as a measurement** |

**5 SCHEDULED · 14 RULED · ⛔ ONE OPEN (`R20`) · one (R2) routed out of POLISH.** A `SCHEDULED` ruling is not a blocker — it is a disposition whose owner and timing are named. ⛔ **`R20` IS A BLOCKER, and it is the first this index has carried.** ⚠ **THIS SENTENCE PREVIOUSLY READ "zero OPEN … Nothing in this index stops work" AND THAT WAS FALSE OF `.4`'S SURFACE** — `OQ-1` was a live, founder-pending hold cited in three `.4` components while this index asserted no ruling was open. Corrected **INTO this sentence** at POLISH.4 PR A commit A5 (`O-5` — an appendix reverses nothing a reader reaches first), in the same commit as the `R20` row it describes. ⚠ **RE-MEASURED off the amended table, not incremented** — 20 rows read, states counted: 14 RULED, 5 SCHEDULED, 1 OPEN. The old sentence records why: *"A tally is a measurement; computing it is how it drifts"* — it had already drifted twice, and arithmetic would have been the third. ⚠ **The superseded claim, kept so the change is legible:** ~~"No ruling in this index is OPEN, and none has been since 2026-08-12, when R13 closed at `design-canon.md` §10 `C-CHART-1`. Nothing in this index stops work."~~ — true of its own nineteen rows, false of `.4`'s surface. **Six moved SCHEDULED → RULED at POLISH.3's kickoff** — R1 · R4 · R5 · R6 · R10 · R14 — founder-ratified 2026-08-12 and recorded in full at `docs/plans/POLISH-3.md` §3. ⚠ **The figures above were RE-MEASURED off the amended table, not derived by subtracting six from the previous pair.** This sentence has drifted twice: it once read *"Nine SCHEDULED · eight RULED"* against a table carrying neither, wrong in opposite directions and not reconcilable by any reading of R2. **A tally is a measurement; computing it is how it drifts.** `POLISH-TRACKER.md` §2 carries no second copy — it points here.
...
| **Components** | **ALL NINETEEN files under `src/components/debate/composer/`** — `AuthGateSlot.tsx` · `BetComposer.tsx` · `ErrorStrip.tsx` · `ImageAttach.tsx` · `PositionStrip.tsx` · `ReplySplitBar.tsx` · `SellModule.tsx` · `SlotHeader.tsx` · **`debate/composer/copy.ts`** · `envelope.ts` · `gating.ts` · `idempotency.ts` · `image-attach.ts` · `payload.ts` · `quote-reader.ts` · `requests.ts` · `sell-convert.ts` · `split-bar.ts` · `state-map.ts`. ⚠ **CORRECTED at POLISH.4 PR A (`PD-4-03`, commit A3), measured with `git ls-files`.** This cell named **11** and **systematically omitted the EIGHT `.ts` behaviour modules** — it enumerated the **render** surface and dropped the **behaviour** surface. ⛔ **Not academic: three of the eight hold live `.4` work.** `state-map.ts` and `idempotency.ts` carry **5 of the 8 `rate_limited` sites** (`PD-4-06`), and `split-bar.ts` holds `computeSplitBar`/`displaySplitTotal`, imported read-only by `.3` — so **an allow-list built from the old cell would have excluded the files holding the work.** ⚠ **`copy.ts` is PATH-QUALIFIED here because it resolves to TWO tracked paths** — `debate/composer/copy.ts` and `profile/copy.ts` — and the bare token has already caused one mis-routing on the record (`docs/logs/POLISH-56-STEP0-RECON-CLOSE-OUT.md`: *"I path-qualified `copy.ts` to the wrong file"*). ⚠ **`SlotHeader.tsx` is ANNEXED — contended with #341**, which owns its arrangement (rows 20/21) and names the rest of `composer/**` *"POLISH.4 territory"*; the allow-list is finalised after #341 lands |
...
| **Tier 2** | canon §4 **rulings 2+3** — Option A: no slippage warning, no tolerance control; price-impact modal **named-retired**; **sell is never clamped**; cap clamp **buy-only** · canon §7 item 2 (split-bar triggers · **composer opens in the opposite slot**) · item 3 (**Đ BET** — canon reads *"app-wide, **plus the W2.8 entry relabel**"*; ⚠ **the second clause was omitted here and is restored at POLISH.4 PR A commit A3 under `O-9`** — re-read at HEAD while re-fencing the Cross-surface cell. It matters: #341 relabels `SlotHeader`'s **entry** trigger to `Buy` (its row 20), so after #341 lands, `COMPOSER_COPY.header`/`.submit` staying `Đ BET` is **canon-correct, not a residue**, and `.4` files no row against them — the omitted clause is what makes that legible) · values-log §1 item 4 (engaged-slot backlight — ⚠ **implements ONLY on `.3`'s files**, `DebateColumn.tsx` + `DebateView.tsx`; `.4` cites a tier-2 item it cannot execute — `PD-4-10`) |
...
| **Cross-surface** | **"Đ BET"** — ⛔ **RE-FENCED BY SYMBOL (`O-8`) at POLISH.4 PR A (`PD-4-03`, commit A3): `COMPOSER_COPY.header`** (`"Place your Đ BET"`) **and `COMPOSER_COPY.submit`** (`"PLACE Đ BET"`), in **`debate/composer/copy.ts`**. ⚠ ~~*(pre-verified — `copy.ts:40,43`)*~~ **NEITHER COORDINATE RESOLVED** — measured at `35da436` and re-measured at `8db535d`: `:40` holds `sell: "Sell",` and `:43` is inside the `overCapStrip` docblock. The **claim** was true and the **coordinate** was stale, which is a **`V-3` false receipt**: marked *pre-verified*, it read as checked · side badge |
...
| ~~**Do not run in parallel**~~ | ~~GRAPH-DEFER · GRAPH-PERF~~ — ⛔ **STRUCK at commit 0 X7, 2026-08-14, per `D7`** (*"`GRAPH-DEFER` / `GRAPH-PERF` STRUCK — one occurrence tree-wide, no definition"*). **Premise re-verified at this commit before striking:** `grep -rn 'GRAPH-DEFER\|GRAPH-PERF'` returns **exactly ONE** occurrence tree-wide — **this line itself** — unchanged from anchor-pack A13's measurement. Neither name has a plan, a log, a register row, a `docs/parked.md` section, a tracker row, or an owner anywhere in the repository, so **the constraint bound nothing**. ⛔ **Deliberately NOT docketed.** `docs/parked.md` is for work that is *deferred*; these are dangling references to things that were **never defined**, and a section for each would mint two rows whose content is *"this name appears once and means nothing"* — a phantom promoted rather than removed. ⚠ **A13 frames the choice as binary — strike, or docket. It is binary, and `D7` already chose**; the anchor pack predates the ruling, and where they disagree the ruling wins |
