...
│   │                               #   ⛔ AND phone/ (MOBILE-2 / ADR-0050) — EIGHT files, and
...
- **⛔⛔ MOBILE-2 / ADR-0050 — THE CONVENTION ABOVE HAS A FLOOR, AND `src/components/debate/phone/` IS WHERE IT WAS HIT.** `/m/[slug]`'s desktop composition is a two-column arena whose composer opens in the column OPPOSITE the one being bet; a stacked arena has no opposite column, so there is no phone shape to reflow INTO. Below 640px the desktop tree therefore **hides** (one appended `max-mobile:hidden` on `DebateView`'s root) and a second tree renders over the SAME four props `DebateView` takes. **This does not loosen ADR-0045** — override-never-replace still governs every existing component, and a phone-only leaf may be minted only when the desktop composition has no phone shape, and only if it (a) lives under `phone/`, (b) consumes the read model / viewer context / existing callbacks and nothing else, (c) contains **no write path** — every bet goes through the reused `BetComposer`, every signed-out prompt through the reused `AuthGateSlot` — and (d) uses no default-breakpoint variant.
  ...
  - ⛔⛔ **THE SCROLL MODEL IS NESTED-SNAP-WITH-A-DOCUMENT-SCROLLER, AND IT WAS MEASURED RATHER THAN CHOSEN (MOBILE-2c R-12, ADR-0050 A2 D-3).** `PhoneFeedTrack`'s track is `overflow-x-auto snap-x snap-mandatory overflow-y-hidden [touch-action:pan-x_pan-y]`; the two panes are `overflow-y-auto overscroll-contain` but have **no height chain behind them**, so they are content-height and `<html>` is the only vertical scroller. ⚠ **That looks like a defect and is not.** MOBILE-2c drove the whole matrix through Chrome's own input pipeline (`Input.synthesizeScrollGesture` + explicit `Input.dispatchTouchEvent` sequences) on Pixel-class Chromium at 360/393/430, on Chromium-iPhone, and on WebKit: **102 of 105 cells WORKS, 0 DEAD**, in both the feed and the thread arm, with the remaining three being a thread page shorter than the viewport. A document-scroll replacement with translated panes and a hand-written angle-locked swipe detector was ruled as the fallback and is **not built**, because the model it would replace passes. ⛔ **NO `touchmove`/`touchstart` HANDLER UNDER `phone/` MAY CALL `preventDefault`** — it takes scrolling away from whatever is under the finger — and `tests/unit/design/phone-touch-handlers.test.ts` scans for it with the comments stripped, because six previous negative source scans in this repo matched the comment explaining the absence.
  - ⛔⛔ **ONE SHEET SHELL, CONTENT-HEIGHT, AND THERE IS NO HEIGHT PROP (MOBILE-2c R-5, ADR-0050 A2 D-3).** `PhoneSheet` is `max-h-[92dvh]`, bottom-anchored, `rounded-t-4xl` (= `--radius-4xl`, 26px, the largest token in the `@theme` scale and the largest actually used), over a 60% `--overlay` backdrop, with `env(safe-area-inset-bottom)` on its body. ⚠ **It used to take `fullHeight`, and `PhoneDebateView` passed `viewer !== null` — so the sheet was the height of the viewport for exactly the readers who can bet, its own backdrop was behind its own panel, and NOT ONE PIXEL of backdrop was reachable.** A tap outside did nothing, so the sheet stayed, and a `fixed inset-0` sheet over a `body{overflow:hidden}` page takes away vertical scrolling AND the tab taps at once — two symptoms of one modal the reader cannot tell is up. That was MOBILE-2c's P0 and it reproduced on **every** engine, not only Android. `tests/unit/debate/phone/phone-sheet-dismissal.test.tsx` pins the absence of any full-height branch; the reachability itself is a browser fact and lives in the B11 matrix.
