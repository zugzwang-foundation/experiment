...
- **Not installed yet:** `commitlint`. *(Playwright landed E2E-1 — §9.)*
...
Test scripts (in `package.json`): `pnpm test:invariants` (`vitest run tests/invariants/`), `pnpm test:integration` (`vitest run tests/integration/`), `pnpm test:scale` (opt-in, its own config), `pnpm test:e2e` (`playwright test` — real-browser tests, own config + own local Postgres/Redis, §9), plus identity-pool seed/verify and staging migrate/seed/smoke scripts. There is **no `just db:up`** and **no all-in-one test recipe** — run `just test-db` and the `pnpm test:*` scripts as needed.
...
- **Accessibility:** `aria-label` on icon-only buttons and YES/NO toggles; Tab-reachable; focus-trap via shadcn `Dialog`; `aria-live="polite"` for price/status; pair colour with icon/text. *(Playwright is installed [§9], but there is no dedicated axe/accessibility-audit project — manual review for now.)*

---

## 9. Testing — Vitest + Playwright

**Vitest is the primary runner.** Playwright landed E2E-1 (2026-08) for real-browser tests under `tests/e2e/` — see the dedicated bullet below. Real layout under a dedicated `tests/` dir (not colocated):
...
├── e2e/           PLAYWRIGHT, not Vitest — real-browser tests against a real local dev server. See the dedicated bullet below. `pnpm test:e2e`; excluded from `vitest.config.ts` (`tests/e2e/**`) since Playwright's own `.spec.ts` files would otherwise collide with Vitest's include glob
...
- **E2E (Playwright, real browser)** — `tests/e2e/`, landed E2E-1, closed out same-project. `pnpm test:e2e` runs `playwright.config.ts`'s `globalSetup` (environment fence → local DB reset+re-migrate → engine-driven world seed → forged session with a round-trip proof against `auth.api.getSession()`) then the numbered spec files (`01-discovery`, `02-market-detail`, `03-bet-place`, `04-reply-floor`, `05-api-checks` — direct API requests bypassing the composer entirely, immune to UI-selector churn — plus `profile`, `sign-in`, `debate-export`, `debate-interactions`). `admin-login.spec.ts` was deliberately deleted at close-out — admin is out of scope, and it was burning real per-IP rate-limit budget (10/hour, real even against the local bridge) on every run for no return. Fixtures live in `tests/e2e/_fixtures/`: `env-guard.ts` (refuses a shared DB/Redis/R2 — proven by `tests/unit/e2e/env-guard.test.ts`'s 18 refusal + acceptance cases), `reset-local-db.ts`, `generate-local-world.vitest.ts` (drives `createMarket`/`openMarket`/`place`/`createOAuthUser`/`acceptTosAction` directly — never the HTTP endpoint, so it never touches moderation — under `vitest.e2e-fixtures.config.ts`, isolated from the main Vitest run the same way `tests/staging/**` is), `session.ts` (the forging code; import-guarded by `tests/unit/e2e/no-src-imports-session-fixture.test.ts` so `src/` can never reach it), `local-upstash-bridge.ts` (a real local Redis, not an unreachable address — idempotency fails CLOSED on missing Redis by design, ADR-0015, so pointing Redis nowhere silently 503s every successful bet while leaving rejection tests green; a generic protocol relay, round-trip-verified against the real `@upstash/redis`/`@upstash/ratelimit` clients including the sliding-window script — this is also what the admin-login rate limit above is genuinely enforced against, locally, once wired in). `tests/e2e/.auth/` (forged session + seeded world) is gitignored — never commit it. Three tests stay deliberately `test.fixme`'d, reason string in the call, not deleted or hidden: bet success, reply-at-floor success, and the profile position check, all blocked on the same invalid `OPENAI_API_KEY` in this environment (moderation fails closed by design). Two features the original E2E-1 plan assumed exist were checked against the live code and confirmed absent — Discovery ranking-mode buttons (retired, ADR-0017 Patch P3; replaced with a capital-neutrality test — SPEC.1's front-door selection is recency-ordered, deliberately not stake-ordered) and a mounted Sell flow (`SellModule` built, not wired to any surface — awaiting POSREV-1) — neither got a faked test.
