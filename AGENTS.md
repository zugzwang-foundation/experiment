...
- **Operational runners** (opt-in, the LIVE staging database): `tests/staging/` holds non-test operational artifacts that borrow the Vitest harness for module resolution — ADR-0036. `reset.staging.test.ts` is the guarded staging reset (ADR-0035); `_lib/` holds its guard predicate, constants and mechanism. They run **only** via `pnpm staging:reset` (which wraps `vitest.staging.config.ts` in `doppler run --config stg`), and the default config **excludes `tests/staging/**`** exactly as it excludes `tests/scale/**`, so no bare `vitest run` — local, CI, or a subagent's — can reach a live database. Each runner additionally refuses to start unless the four-guard contract passes (target · environment · live connection · post-run verification). Isolation is asserted by `tests/unit/staging/runner-isolation.test.ts`, and the reset's gating shape by `runner-gating.test.ts`.
