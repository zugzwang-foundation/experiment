...
│   ⛔ `proxy.ts` LIVES UNDER `src/` (AWS-MIGRATION-3). It sat at the repo root until 2026-09-25 and
│   Next resolves the proxy/middleware file relative to `src/` when the app dir is there — so it was
│   NEVER BUILT: `.next/server/middleware.js` was a 223-byte stub and `middleware-manifest.json` was
│   empty on every build, measured on staging with the write-pause deployed and inert. The admin
│   redirect it carries was dead the whole time; `assertAdminActor` (server-side) is what guarded
│   `/admin`. Moving the file is the fix; the manifest is the claim.
```
...
- **`--breakpoint-mobile: 640px` is the repo's ONE minted breakpoint, and every mobile rule is an ADDITIVE `max-mobile:` override behind a `mobileResponsive` prop** (MOBILE-1 Phase A / ADR-0045, the first breakpoint this repo has ever minted; ADR-0045 calls the convention "a new convention future work must follow consistently"). It lives in the plain `@theme` block beside the branded ramp — a `--breakpoint-*` outside `@theme` is an ordinary custom property that resolves in `var()` and generates **no variant at all**, silently. It generates `mobile:*` (≥640px) and `max-mobile:*` (<640px). **Rules, and each exists because its violation fails quietly — ⚠ this read *"Four rules"* against SIX sub-bullets, stale by two. The count is deleted rather than corrected, which is the exit the `tests/unit/design/` figure and the `*.test.tsx` figure have both already taken in this file (`O-15`); the bullets below are the claim:**
  ...
  - **⛔ GATE IT ON A PROP, AND THREAD THE PROP THE WHOLE WAY DOWN.** `GlobalHeader` is mounted by BOTH `(public)/layout.tsx` and `(auth)/layout.tsx`, so every reflow class is `cn(base, mobileResponsive && "max-mobile:…")` and the prop **defaults `false`** — a mount that forgets it inherits the desktop render rather than an accidental reflow. ⚠ **BOTH MOUNTS NOW OPT IN.** This line read *"ADR-0045 leaves auth/join surfaces `gated, not made responsive`… and only `(public)` opts in"* until MOBILE-1 · Job A, and **ADR-0048 supersedes exactly that carve-out** — a phone participant is allowed to join, so `(auth)/layout.tsx` passes the prop and `/sign-in`, `/sign-in/otp` and `/onboarding` reflow at the 640px tier (measured: 379px of document overflow → 0px, header min-content 754px → 375px). ⛔ **The default is NOT thereby dead, and deleting it is the wrong inference from "both callers pass it".** The polarity is a property of the default rather than of who currently passes it: it is what makes a THIRD mount safe by omission, and ADR-0048 `:84` considers removing the prop and rejects it for now. ⚠ **The gate is the prop chain, not the file boundary** — a class left unconditional two components down reaches `(auth)` exactly as surely as one written in the layout's own file, which is how `OnboardingDeck` shipped three ungated classes onto `/sign-in` through `GlobalHeader → RulesControl → OnboardingDeck`. That deck now reflows on the auth routes **by founder ruling**, which is a change in what is wanted, not in the discipline: the classes are still gated and still threaded. Guarded by `tests/unit/shell/global-header-mobile-reflow.test.ts`. ⚠⚠ **`HeaderNav` JOINED THAT CHAIN AT MOBILE-2m (ADR-0051 A9 D-3) AND IS THE FIRST CONSUMER THAT HIDES A NAVIGATION CONTROL RATHER THAN A DECORATIVE ONE** — Back does not render below 640 on any route. The five before it drop a wordmark, a visitor count, two utility controls and a Đ readout; this one removes a way to move. That is legal under the same rule for the same reason the rule exists: a phone has a system back gesture that knows the real stack, which the `history.length` heuristic this control hedges against does not, and the phone tier carries its own Back in `PhoneTitleStrip` for the one screen needing an in-page one. ⚠ Hidden, not unmounted — the handler and the depth probe are untouched, so a resize across the tier restores the control with its state intact. ⚠ Several `src/` docblocks on that chain still state the superseded position and are a known, deferred debt — see `docs/plans/MOBILE-1-JOB-A.md` OQ-6a. ⚠⚠ **AND AT MOBILE-2n (ADR-0051 A10 D-5) THE CHAIN CARRIES ITS FIRST GATED *MOUNT* RATHER THAN A GATED HIDE** — `GlobalHeader` renders `GitHubIconControl` only when the prop is true, so a third mount inherits the header it has today instead of silently GAINING a control. The polarity is the same and the direction is the opposite: the six before it decide whether an existing node is painted, this one decides whether a node exists. ⛔ **The class gate is still needed beside it and is not redundant** — both current mounts pass the prop at EVERY width, so `mobileResponsive` says *this surface reflows*, never *this viewport is a phone*; `hidden max-mobile:inline-flex` is what keeps 1440 unchanged. ⛔⛔ **AND THAT HIDE MUST GO THROUGH `cn()`.** The shared 34px icon register (`shell/header-control.ts`) OPENS with its own `inline-flex`; written as a template literal the element carries both `inline-flex` and `hidden` unprefixed, two single-class selectors at equal specificity, and Tailwind emits `inline-flex` LAST — so the cascade paints the control at 1440. Measured: 33.99x33.99 at x 1216.72 signed out, widening the right zone 165.29 → 199.29px and moving JOIN 34px on the DESKTOP, with every source scan green throughout. `twMerge` resolves the two as one display group and keeps the last; the variant lands in a different group and survives.
...
- **Component / render** (jsdom, no IO): `jsdom` + `@testing-library/react`, enabled **per file** by a `// @vitest-environment jsdom` docblock on line 1 — `find tests -name '*.test.tsx' | wc -l` — **RUN IT; there is no number in this sentence to read.** The figure has now been written out and gone stale **five** times — 30, 79, 84, 106, 108 — the last of them measured 2026-09-04 against a live 133 ten days later. Five passes have established that the count is the part that rots while the command never does, so MOBILE-2l deletes it rather than writing a sixth (the precedent is SYNC-5, which removed the spec versions from CLAUDE.md §1 on exactly this argument and has not gone stale since). `O-15`: a number written into prose decays regardless of how loudly the prose says not to trust it. The files sit mostly under `tests/unit/**/render/` plus `tests/server/admin/*.component.test.tsx`. **There is no `jest-dom`**, so `toBeInTheDocument()` / `toBeDisabled()` and that whole matcher set are UNAVAILABLE — assert against plain DOM (`getAttribute`, `textContent`, `querySelector`). Fake timers + `act()` for interval/effect behaviour; page visibility is exercised by stubbing `document.hidden` and dispatching `visibilitychange` (F-DEBATE-4). *Recorded at F-DEBATE-4 because this harness has existed since UI.0 and §9 never named it, so successive plans inherited a false "the UI cannot be tested" premise.*
  ...
  - ⚠ **jsdom SHIPS NO `DragEvent`, SO A DRAG TEST SILENTLY LOSES THE ONE PROPERTY THAT DECIDES IT.** Measured at DND-1: `typeof DragEvent === "undefined"` here, so dom-testing-library falls back to a plain `Event` — which means `fireEvent.dragLeave(el, { relatedTarget })` constructs an event **with no `relatedTarget`**, and a handler that distinguishes "left the element" from "crossed onto a child" takes the wrong arm against code that is behaving correctly. ⇒ Install it by hand: `createEvent.dragLeave(...)` then `Object.defineProperty(event, "relatedTarget", { value: node })`. `dataTransfer` is the exception and needs no such treatment — dom-testing-library special-cases it (and `clipboardData`) by `defineProperty` precisely because jsdom omits it too, so `{ dataTransfer: { files, types } }` DOES arrive. ⛔ **The two halves read identically in the test source and behave oppositely**, which is why they are stated together. `tests/unit/composer/render/attach-drop.test.tsx` carries the working shape.
