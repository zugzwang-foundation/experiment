...
├── staging/       OPERATIONAL RUNNERS, not tests — reset.staging.test.ts + _lib/ (STAGING-PARITY Slice A, ADR-0035/0036). Points at the LIVE staging DB; opt-in only, see the Operational-runners bullet below
...
- **Operational runners** (opt-in, the LIVE staging database): `tests/staging/` holds non-test operational artifacts that borrow the Vitest harness for module resolution — ADR-0036. `reset.staging.test.ts` is the guarded staging reset (ADR-0035); `_lib/` holds its guard predicate, constants and mechanism. They run **only** via `pnpm staging:reset` (which wraps `vitest.staging.config.ts` in `doppler run --config stg`), and the default config **excludes `tests/staging/**`** exactly as it excludes `tests/scale/**`, so no bare `vitest run` — local, CI, or a subagent's — can reach a live database. Each runner additionally refuses to start unless the FIVE-guard contract passes (intent · target · environment · live connection · post-run verification — the G-5 intent token is the ADR-0035 Addendum's addition to primitive 6's four). Isolation is asserted by `tests/unit/staging/runner-isolation.test.ts`, and the reset's gating shape by `runner-gating.test.ts`.
