...
- **The seed runner** (opt-in, CAN WRITE PRODUCTION — ADR-0053): `tests/prod-seed/seed.prod-seed.test.ts` under its own `vitest.prod-seed.config.ts`, excluded from the default config. It drives the engine over a validated seed table (`tests/prod-seed/_lib/table.ts`, SEED-1 §4.3's rules). The table comes from `scripts/seed/generate-table.ts`, the images folder is filled by `scripts/seed/prepare-images.ts`, and the only entry point is `scripts/seed-prod.ts --env local|staging|prod` (`--preflight-only`, `--limit N`, `--window-live`, `--until-done`). Its target guard is a POSITIVE match on the production ref plus `ZUGZWANG_ENV=prod` plus an acknowledgement, and local/staging modes still run P-17. It re-does the route's freeze, ban and reply-parent checks that `place()` skips, resumes only over receipts whose content fingerprint, market and user match the table, reads `--limit N` as the first N table rows, and rebuilds `manifest-<mode>.json` from the database beside the table. ⚠ It does not touch `tests/staging/` guards; the one shared change is `_lib/write-guard.ts` learning Windows stack frames, which previously made every engine write unattributable on Windows.
