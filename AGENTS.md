...
- **`--breakpoint-mobile: 640px` is the repo's ONE minted breakpoint, and every mobile rule is an ADDITIVE `max-mobile:` override behind a `mobileResponsive` prop** (MOBILE-1 Phase A / ADR-0045, the first breakpoint this repo has ever minted; ADR-0045 calls the convention "a new convention future work must follow consistently"). It lives in the plain `@theme` block beside the branded ramp — a `--breakpoint-*` outside `@theme` is an ordinary custom property that resolves in `var()` and generates **no variant at all**, silently. It generates `mobile:*` (≥640px) and `max-mobile:*` (<640px). **Three rules, and each exists because its violation fails quietly:**
  ...
  - **⛔ GATE IT ON A PROP, AND THREAD THE PROP THE WHOLE WAY DOWN.** `GlobalHeader` is mounted by BOTH `(public)/layout.tsx` and `(auth)/layout.tsx`, and ADR-0045 leaves auth/join surfaces *"gated, not made responsive."* So every reflow class is `cn(base, mobileResponsive && "max-mobile:…")`, the prop **defaults `false`** (a mount that forgets it inherits the desktop render rather than an accidental reflow), and only `(public)` opts in. ⚠ **The gate is the prop chain, not the file boundary** — a class left unconditional two components down reaches `(auth)` exactly as surely as one written in the layout's own file, which is how `OnboardingDeck` shipped three ungated classes onto `/sign-in` through `GlobalHeader → RulesControl → OnboardingDeck`. Guarded by `tests/unit/shell/global-header-mobile-reflow.test.ts`.
    - ⚠ **THERE IS EXACTLY ONE UNGATED BREAKPOINT CLASS IN THIS SUBTREE AND IT IS DELIBERATE — `IdentityCluster.tsx`'s JOIN link (MOBILE-1 Phase B).** It carries `max-mobile:hidden touch-primary:hidden` with no `mobileResponsive` anywhere in the file, which is what the rule above forbids on its face. The rule says every ***reflow*** class, and the distinction is the whole of the exception: a reflow makes an auth surface responsive, which ADR-0045 rules out; this class **is** ADR-0045's gate, which the same ADR puts on "every route" — so `(auth)` is where it is *most* needed, not where it must not reach. Gated, `/sign-in` would render a JOIN button at phone width directly above the card that now says *"Sign-up only works on a computer right now."* ⇒ The test row that used to assert `IdentityCluster.tsx` carried **no** responsive token was **inverted rather than deleted** (the WARLI-MOUNT precedent): it now pins the vocabulary as *exactly* `{max-mobile:hidden, touch-primary:hidden}`, requires both inside the `if (!viewer)` branch, requires their absence on the signed-in chip branches, and requires `mobileResponsive` to be absent. A third token, a different spelling, or a hide that reaches the identity chip all still redden. **Do not read the ungated class as the defect this bullet exists to catch, and do not add a second one on its precedent** — the exception is one CTA, argued from the ADR, and guarded.
  - **`touch-primary` is the repo's ONE custom variant besides `dark`** (`@custom-variant touch-primary (@media (hover: none) and (pointer: coarse));`, MOBILE-1 Phase B). It is width-INDEPENDENT on purpose and is not a synonym for anything: a tablet is ~1024px wide, so no phone breakpoint reaches it. `hover: none` **and** `pointer: coarse` together, not Tailwind's built-in `pointer-coarse` alone, which is less precise against hybrid / 2-in-1 devices. ⛔ **Verify it in the BUILT stylesheet after a `just clean`, never after a warm build** — measured at MOBILE-1 Phase B: `pnpm build` on an existing `.next` emitted **zero** `touch-primary` rules while emitting `max-mobile:*` normally, and `just clean` + rebuild then emitted all three correctly inside `@media (hover:none) and (pointer:coarse)`. The variant was fine the whole time; the warm build was not. This is the §9 stale-utility trap one layer up — there a dev server served a stale sheet, here a production build did — and it is the same failure that made Phase A ship inert.
