 ...
 * missing rules" would pass identically against a `compareStripRules` that
...
describe("strip-rule completeness · against the LIVE registry", () => {
	...
	it("does not throw on the live registry", () => {
		expect(() => assertStripRulesComplete()).not.toThrow();
	});
	...
	it("reads the RUNTIME array — 24 types, not a regex count", () => {
		...
		expect(verifyStripRuleCompleteness().eventTypes).toHaveLength(24);
		expect(EVENT_TYPES).toHaveLength(24);
	});

	it("records the seven market.* types as explicitly nothing-to-strip", () => {
		// `[]` is an answered question. Absence is an unasked one. The two
		// must not share a representation, or the guard cannot distinguish
		// "reviewed, nothing to strip" from "never reviewed".
		expect([...verifyStripRuleCompleteness().noStripRequired].sort()).toEqual([
			"market.closed",
			"market.corrected",
			"market.created",
			"market.opened",
...
describe("strip-rule completeness · POSITIVE CONTROLS (the injected gap)", () => {
	it("THE WRONG ANSWER — a new event type with no rule is detected", () => {
		...
		const report = compareStripRules(withNewType, PAYLOAD_STRIP_KEYS);
	...
	it("detects a rule orphaned by a removed event type", () => {
		...
		const report = compareStripRules(shrunk, PAYLOAD_STRIP_KEYS);
	...
	it("detects a gap even when most of the table is fine", () => {
		...
		const { "bet.placed": _dropped, ...missingOne } = PAYLOAD_STRIP_KEYS;

		const report = compareStripRules(EVENT_TYPES, missingOne);
		expect(report.missingRules).toEqual(["bet.placed"]);
	});
});

describe("strip-rule completeness · the thrown error", () => {
	it("assertStripRulesComplete names the offending types", () => {
	...
	it("the error tells the operator what to DO, not just what is wrong", () => {
		...
		try {
			...
			const stub = { ...PAYLOAD_STRIP_KEYS } as Record<
				string,
				readonly string[]
			>;
			delete stub["comment.placed"];
			const r = compareStripRules(EVENT_TYPES, stub);
			if (r.missingRules.length > 0) {
				throw new EgressContractGapError(
					`EVENT_TYPES: ${r.missingRules.join(", ")}`,
					"Add an entry to PAYLOAD_STRIP_KEYS — '[]' if genuinely nothing " +
						"needs stripping — and amend SPEC.2 §19.4.1 in the same commit.",
				);
			}
			throw new Error("expected a contract gap");
		} catch (e) {
			expect((e as Error).message).toContain("PAYLOAD_STRIP_KEYS");
