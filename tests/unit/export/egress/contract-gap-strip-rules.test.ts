 ...
 * `assertShipRulesComplete` never throwing at all, and with its message being
 * the empty string — the assertion is about a literal in the test, not about
 * anything in `src/`. Mutating `assertShipRulesComplete`'s body to `return;`
 * leaves the whole export suite green.
 *
 * The failing shape has to be constructed at the INPUT, not at the assertion.
 * `assertShipRulesComplete()` takes no arguments and reads the runtime
...
vi.mock("@/server/events/event-types", async (importOriginal) => {
	...
	return {
		...actual,
		// The defect, constructed: a 25th event type with no PAYLOAD_SHIP_KEYS
...
describe("strip-rule completeness · the REAL throw, not a reconstruction", () => {
	it("assertShipRulesComplete THROWS when a live event type has no rule", async () => {
		const { assertShipRulesComplete } = await import(
		...
		expect(() => assertShipRulesComplete()).toThrow(EgressContractGapError);
	});
	...
	it("the REAL message names the type and carries the remedy", async () => {
		...
		expect(message).toContain("PAYLOAD_SHIP_KEYS");
		expect(message).toContain("§19.4.1");
		// '{}' is a legal answer. Without saying so, the fastest-looking fix for
		// an event type that ships nothing is to delete the guard. (Was '[]'
		// until the DATASET.3 inversion made the declaration an object.)
		expect(message).toContain("'{}'");
	});

	it("verifyShipRuleCompleteness reports the gap against the LIVE array", async () => {
		const { verifyShipRuleCompleteness } = await import(
			"@/server/export/egress/completeness"
		);
	...
	it("buildDataset REFUSES to read a row while the gap exists", async () => {
		...
		// deleting `assertShipRulesComplete()` from `buildDataset` leaves every
