...
function post(
	...
	const built: PostSubstrate = {
		...
		// side counts) and the meter's display numerator. The §13 worked example
		// below states no flag anywhere, so `b` reads exactly what it read
		// pre-ADR and §13 reproduces unchanged — which is the identity ADR-0058
		// Driver 5 promises, checked here by the spec's own example.
		endorseCount: 0,
		contestCount: 0,
...
describe("ranking::top-ties (§3.4)", () => {
	it("equal topScore → higher author stake ranks first", () => {
		...
		const hi = post({
			...
			supportCount: 10,
			counterCount: 10,
			supportCountTotal: 10,
			counterCountTotal: 10,
			supportDharma: "500",
			counterDharma: "500",
			createdAt: new Date("2026-09-01T00:00:00.000Z"),
			authorStake: "300",
