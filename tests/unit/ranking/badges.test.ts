...
function post(
	...
	const built: PostSubstrate = {
		...
		// side counts, and the meter's display numerator. NO case in this file
		// states either, which is the point: with no flag anywhere the balance
		// term reads exactly what it read before the ADR, so every badge
		// calibration here must survive FF-1 untouched.
		endorseCount: 0,
		contestCount: 0,
		friendlyFireDharma: "0",
		createdAt: new Date("2026-09-01T00:00:00.000Z"),
		authorStake: "0",
		authorStakeOriginal: "0",
		authorSold: false,
		priceAtBet: "0.5",
		...over,
	};
	// The zero-flag identity, applied AFTER the spread so it reads each case's
	// own side counts rather than the zeros above.
...
describe("ranking::lane-dominance-badge-highest-margin (§5.1.3)", () => {
	it("a post dominating two badge lanes wears the higher-margin one", () => {
		...
		const p1 = post({
			...
			supportCount: 19,
			counterCount: 21,
			supportCountTotal: 19,
			counterCountTotal: 21,
			supportDharma: "600",
			counterDharma: "600",
			authorStake: "300",
		});
