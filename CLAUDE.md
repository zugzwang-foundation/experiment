...
*Rebuilt at SYNC.8 (Jun 2, 2026) against live repo `27216fc` + SPEC.1 v1.9.0-draft + SPEC.2 + ADRs 0003–0031 (**the range as it stood on that date — a SYNC.8 snapshot, not a current fact; §1 carries the live ceiling**); descriptive drift reconciled at BC.1 (Jul 1, 2026) against `248e02f`; SPEC.1/SPEC.2 version citations reconciled at the SYNC sweep (Jul 7, 2026), then at SYNC-LITE (Jul 16), and again at **SYNC-1 (Aug 8, 2026) — SPEC.1 1.0.29, SPEC.2 1.0.22, the live values ON THAT DATE**. ⚠ **RPLY-CLOSE (Aug 26, 2026) re-read all three: SPEC.1 is `1.0.44` (this task bumped it twice, 1.0.42 → 1.0.44), SPEC.2 is `1.0.24`, the ADR ceiling is `0040` across 38 numbered files.** ⛔ **AND THE CLAUSE THAT FOLLOWED THOSE FIGURES — *"§1 carries them"* — WAS THE STALE PART, NOT THE NUMBERS.** §1 carries SPEC.2's version and the ADR ceiling, both of which were already correct; it deliberately carries **no** SPEC.1 version at all, ruling instead that *"version lives in the file's own header; read it there"*. So a pointer promising §1 held a number §1 had stopped holding sent a reader looking in the one place guaranteed not to answer — the `O-13` shape, one document over. The dated snapshots above are left exactly as they are: they record what was true at SYNC-1 and were never wrong; overwriting them with today's figures would manufacture a reading that pass never took (`V-9`). Folded: reply-as-bet, ranking → 0017, two-floor economy → 0018, RLS → 0019, **CC → Opus 4.8** (**likewise a SYNC.8 snapshot, not a current fact — the live pin is Opus 5, §6, repinned at MODEL-REPIN 2026-07-31**). Corrected against recon: hooks/skills/`settings.json` not installed; critical-path naming; schema at `src/db/`. SYNC-1 (Aug 8, 2026) added §5.14 standing review checks and §8 O-space, and executed the D.4 V-renumber. Advisory, not enforcement. Maintained per `docs/maintenance.md`.*
