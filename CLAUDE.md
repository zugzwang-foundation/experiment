...
**The state, which is not the rule:** there are **346** pre-convention commits, and **341 carry a note** — the whole of `n=1–346` except the five below. ⚠ **`git notes list | wc -l` returns 343, and the gap is not drift in this figure.** Two POST-convention commits — `6ebbfcc` and `44924a7` — carry a note because their own messages carry no block, which is the repair path above being used exactly as it is written. The corpus count and the ref total are two different measurements and this sentence is about the first; a reader running the command and reading 343 against 341 is looking at the second. The sentence above says where that reasoning goes; it does not say it is already there. A claim nothing on disk can falsify is not a guarantee, and this file should not read like one. Update the count when it moves.
