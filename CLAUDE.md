...
**Subagents** — auto-discovered from `.claude/agents/*.md`; all four are tracked, each `model: claude-opus-5` / `effort: max` with a "MUST BE USED" routing description. Agent definitions are read at session start and are **not hot-reloaded** — a mid-session edit to `.claude/agents/*.md` does not affect the running session (MODEL-REPIN). Full briefings live in those files; invocation policy is §5.11.
