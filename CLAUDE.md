...
**The state, which is not the rule:** there are **346** pre-convention commits, and **341 carry a note** — the whole of `n=1–346` except the five below. The sentence above says where that reasoning goes; it does not say it is already there. A claim nothing on disk can falsify is not a guarantee, and this file should not read like one. Update the count when it moves.
