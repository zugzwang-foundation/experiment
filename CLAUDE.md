...
- **Source of truth:** `SPEC.1` (product, v1.9.0-draft) + `SPEC.2` (technical) + `docs/adr/0003–0026` are canonical. `tracker_v11.html` is planning/sequencing only. On conflict, spec/ADR wins — note the drift once, don't block.
...
SPEC.1 §5. Tests at `tests/invariants/I-<AREA>-NNN.<slug>.spec.ts`; the triggers in `drizzle/migrations/0003_append_only_triggers.sql` are storage-layer ground truth. *(Canonical tests on disk: INV-1 → `I-ATOMICITY-001`, INV-2 → `I-NO-OVERDRAFT-001`, INV-3 → `I-SIDE-BIND-001`, INV-4 → `I-APPEND-ONLY-001`; plus five invariant-class spec-rule specs — `I-DAILY-ONCE-001`, `I-GRANT-ONCE-001`, `I-NO-OVERSELL-001`, `I-RESOLVE-ONCE-001`, `I-SINGLE-SIDE-001`. Storage-layer backstop remains `tests/db/triggers/` + the schema.)*
