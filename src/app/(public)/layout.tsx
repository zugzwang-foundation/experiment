 ...
 * `router.refresh()` every `POLL_INTERVAL_MS_DEBATE_VIEW` (30 s — widened
 * from 15 s, frontend-optimization-notes item 1; no per-tab jitter, see that
 * constant's own docblock for why), and a refresh re-executes the LAYOUT as
 * well as the page — `docs/logs/F-DEBATE-4.md:213` measured the tick at
 * 12–14 round-trips "because the refresh re-executes the layout as well as
 * the page". So for a signed-in viewer holding that tab open this pair costs
 * 2 reads / 30 s / tab, not 2 per navigation. Still two
	...
	return (
		<div className="flex min-h-full flex-col">
			...
			{/* O1-DECK — the first-login deck, a SIBLING of `<main>` and never a
			    ...
			    `router.refresh()` every 30 s on `/m/[slug]` (widened from
			    15 s — frontend-optimization-notes item 1), and a refresh
